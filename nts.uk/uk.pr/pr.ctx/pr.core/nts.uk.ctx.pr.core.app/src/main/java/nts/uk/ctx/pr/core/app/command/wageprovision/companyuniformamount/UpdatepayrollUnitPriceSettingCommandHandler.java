...
@Stateless
@Transactional
public class UpdatepayrollUnitPriceSettingCommandHandler extends CommandHandler<PayrollUnitPriceSettingCommand>
{
    ...
    protected void handle(CommandHandlerContext<PayrollUnitPriceSettingCommand> context) {
        ...
        repository.update(new PayrollUnitPriceSetting(command.getHistoryId(), command.getAmountOfMoney(), command.getTargetClass(), command.getMonthSalaryPerDay(), command.getADayPayee(), command.getHourlyPay(), command.getMonthlySalary(), command.getSetClassification(), command.getNotes()));
