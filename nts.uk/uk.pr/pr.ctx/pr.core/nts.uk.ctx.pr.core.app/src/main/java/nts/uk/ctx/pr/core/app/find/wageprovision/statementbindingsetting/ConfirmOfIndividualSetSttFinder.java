        ...
        employeeIds.stream().forEach(itemEmployeeId -> {
                    ...
                    if (mStateUseUnitSetting.get().getIndividualUse().value == SettingUseClassification.USE.value) {
                        ...
                        if (!mStateCorrelationHisIndividual.isPresent()) {
                            stateCorrelationHisIndividual = mStateLinkSettingIndividualRepository.getStateLinkSettingIndividualById(mStateCorrelationHisIndividual.get().getHistory().get(FIRST_HISTORY).identifier());
    ...
    private Optional<StateLinkSettingMaster> getAcquireMasterLinkedStatement(UsageMaster type, String employeeId, String hisId, GeneralDate baseDate, EmployeeInformationImport dataRequestList) {
        ...
        switch (type) {
            case DEPARMENT: {
                ...
                Optional<StateLinkSettingMaster> mStateLinkSettingMaster = mStateLinkSettingMasterRepository.getStateLinkSettingMasterById(historyId, dataRequestList.getDepartment().getDepartmentCode());
            ...
            case EMPLOYEE: {
                ...
                /*ドメインモデル「明細書紐付け設定（マスタ）」を取得する*/
                Optional<StateLinkSettingMaster> mStateLinkSettingMasterVer2 = mStateLinkSettingMasterRepository.getStateLinkSettingMasterById(hisId, mEmploymentHisExport.get().lstEmpCodeandPeriod.stream().filter(x -> x.getHistoryID().equals(hisId)).findFirst().get().getEmploymentCode());
                if(!mStateLinkSettingMasterVer2.isPresent()){
                    return Optional.empty();
                }
                return mStateLinkSettingMasterVer2;
            ...
            case POSITION: {
                ...
                /*ドメインモデル「明細書紐付け設定（マスタ）」を取得する*/
                Optional<StateLinkSettingMaster> mStateLinkSettingMasterVer2 = mStateLinkSettingMasterRepository.getStateLinkSettingMasterById(hisId, mJobTitleInfo.stream().filter(x -> x.getJobTitleId().equals(employeeJobHistExport.get().getJobTitleID())).findFirst().get().getJobTitleCode());
                if(!mStateLinkSettingMasterVer2.isPresent()){
                    return Optional.empty();
                }
                return mStateLinkSettingMasterVer2;
