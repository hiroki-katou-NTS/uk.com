...
@Stateless
@Transactional
public class DeleteHealthInsuranceCommandHandler extends CommandHandler<AddHealthInsuranceCommand>{
	...
	protected void handle(CommandHandlerContext<AddHealthInsuranceCommand> context) {
		healthInsuranceService.deleteHistory(context.getCommand().getOfficeCode(), context.getCommand().getYearMonthHistoryItem().fromCommandToDomain());
