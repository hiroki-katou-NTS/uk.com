...
@NoArgsConstructor
@Entity
@Table(name = "QBTMT_EMP_BON_PAY_INFO")
public class QbtmtEmpBonPayInfo extends UkJpaEntity {
	...
	public QbtmtEmpBonPayInfo(String employeeId, String historyId, YearMonth start, YearMonth end) {
		super();
	...
	public static List<QbtmtEmpBonPayInfo> fromDomain(EmployeeBonusPaymentInfor domain) {
		List<YearMonthHistoryItem> periods = domain.items();
		return periods.stream()
				.map(p -> new QbtmtEmpBonPayInfo(domain.getEmployeeId(), p.identifier(), p.start(), p.end()))
				.collect(Collectors.toList());
	}

	public static EmployeeBonusPaymentInfor toDomain(List<QbtmtEmpBonPayInfo> listEntity) {
