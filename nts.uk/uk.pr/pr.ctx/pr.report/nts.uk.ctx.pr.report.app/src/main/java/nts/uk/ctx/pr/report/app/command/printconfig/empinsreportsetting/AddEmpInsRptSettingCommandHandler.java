...
@Stateless
@Transactional
public class AddEmpInsRptSettingCommandHandler extends CommandHandler<EmpInsRptSettingCommand> {
    ...
    protected void handle(CommandHandlerContext<EmpInsRptSettingCommand> commandHandlerContext) {
        ...
        repository.insert(new EmpInsReportSetting(AppContexts.user().companyId(), AppContexts.user().userId(), command.getSubmitNameAtr(), command.getOutputOrderAtr(), command.getOfficeClsAtr(), command.getMyNumberClsAtr(), command.getNameChangeClsAtr()));
    }
}
