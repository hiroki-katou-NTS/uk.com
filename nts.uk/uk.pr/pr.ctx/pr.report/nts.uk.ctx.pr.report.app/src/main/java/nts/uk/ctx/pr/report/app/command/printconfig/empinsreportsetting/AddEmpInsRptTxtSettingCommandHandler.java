...
@Stateless
@Transactional
public class AddEmpInsRptTxtSettingCommandHandler extends CommandHandler<EmpInsRptTxtSettingCommand> {
    ...
    protected void handle(CommandHandlerContext<EmpInsRptTxtSettingCommand> commandHandlerContext) {
        ...
        repository.insert(new EmpInsReportTxtSetting(AppContexts.user().companyId(), AppContexts.user().userId(), command.getOfficeAtr(), command.getFdNumber(), command.getLineFeedCode()));
    }
}
