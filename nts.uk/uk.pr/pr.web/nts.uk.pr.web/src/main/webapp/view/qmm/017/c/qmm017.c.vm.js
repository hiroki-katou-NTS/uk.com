...
(function (nts) {
    ...
    (function (qmm017) {
        ...
        var TextEditor = (function () {
            ...
            TextEditor.prototype.validateAfterAtSign = function (tagsJapaneseChar) {
                var self = this;
                var _loop_1 = function(tagOrder) {
                    if (tagsJapaneseChar[tagOrder].innerText.indexOf('＠') !== -1) {
                        var splitByAtSign = tagsJapaneseChar[tagOrder].innerText.split('＠');
                        var correctItemName = _.find(self.itemsBag, function (item) { return item.name == tagsJapaneseChar[tagOrder].innerText.trim(); });
                        if (!correctItemName) {
            ...
            TextEditor.prototype.isContainsComparator = function (content) {
                var self = this;
                for (var i = 0; i < self.listComparator.length; i++) {
            ...
            TextEditor.prototype.isBooleanType = function (content) {
                var self = this;
                return (content.indexOf('関数＠かつ') !== -1 || content.indexOf('関数＠または') !== -1 || self.isContainsComparator(content));
            };
            TextEditor.prototype.isYearMonthType = function (content) {
            ...
            TextEditor.prototype.validateFunction = function (contentFunction) {
                var self = this;
                var inputContent = [];
                var splitedContent = contentFunction.split('');
                var startIndex = 0;
                var endIndex = 0;
                var openCount = 0;
                var closeCount = 0;
                var lstFunctionString = [];
                for (var position = 0; position < splitedContent.length; position++) {
            ...
            TextEditor.prototype.markErrorTreeObject = function (treeObject, message, param) {
                var self = this;
