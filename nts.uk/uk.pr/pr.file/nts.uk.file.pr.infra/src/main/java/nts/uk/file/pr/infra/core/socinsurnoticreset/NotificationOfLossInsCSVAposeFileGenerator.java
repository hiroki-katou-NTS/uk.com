...
public class NotificationOfLossInsCSVAposeFileGenerator extends AsposeCellsReportGenerator implements NotificationOfLossInsCSVFileGenerator {
    ...
    private void fillPensionOffice(List<InsLossDataExport> healthInsLoss, Worksheet worksheet,
        ...
        for(int i = 0; i < healthInsLoss.size(); i++){
            ...
            if(i == 0) {
                ...
                cells.get(startRow, OFFICE_SYMBOL).setValue(ins.getBusinessArrSymbol() == BussEsimateClass.HEAL_INSUR_OFF_ARR_SYMBOL ? data.getOfficeNumber2().substring(0, 4) : data.getWelfOfficeNumber2().substring(0, 4));
                cells.get(startRow, SERIAL_NUMBER).setValue(ins.getFdNumber());
                ...
                cells.get(startRow, 11).setValue(company.getPhoneNum().length() > 11 ? company.getPhoneNum().substring(6, 10) : company.getPhoneNum().substring(6, company.getPhoneNum().length()));
                cells.get(startRow, 12).setValue(company.getPhoneNum().length() > 16 ? company.getPhoneNum().substring(11, 16) : company.getPhoneNum().substring(11, company.getPhoneNum().length()));
                startRow++;
    ...
    private void fillPensionEmployee(InsLossDataExport data, Cells cells,
        ...
        cells.get(startRow, 2).setValue(ins.getBusinessArrSymbol() == BussEsimateClass.HEAL_INSUR_OFF_ARR_SYMBOL ? data.getOfficeNumber1().substring(0, 2) : data.getWelfOfficeNumber1().substring(0, 2));
        cells.get(startRow, 3).setValue(ins.getBusinessArrSymbol() == BussEsimateClass.HEAL_INSUR_OFF_ARR_SYMBOL ? data.getOfficeNumber2().substring(0, 4) : data.getWelfOfficeNumber2().substring(0, 4));
        cells.get(startRow, 4).setValue(ins.getBusinessArrSymbol() == BussEsimateClass.HEAL_INSUR_OFF_ARR_SYMBOL ? data.getOfficeNumber() : data.getWelfOfficeNumber());
    ...
    private void fillHealthInsAssociationOffice(List<InsLossDataExport> healthInsAssociation, Worksheet worksheet,List<SocialInsurancePrefectureInformation> infor, CompanyInfor company, SocialInsurNotiCreateSet ins, GeneralDate baseDate){
        ...
        for(int i = 0; i < healthInsAssociation.size(); i++){
            ...
            if(i == 0) {
                ...
                cells.get(startRow, 1).setValue(Objects.toString(ins.getFdNumber(), "001"));
                ...
                cells.get(startRow, 5).setValue(data.getHealInsInherenPr().length() > 20 ? data.getHealInsInherenPr().substring(10, 20) : data.getHealInsInherenPr().substring(10, data.getHealInsInherenPr().length()));
                cells.get(startRow, 6).setValue(data.getHealInsInherenPr().length() > 30 ? data.getHealInsInherenPr().substring(20, 30) : data.getHealInsInherenPr().substring(20, data.getHealInsInherenPr().length()));
                cells.get(startRow, 7).setValue(data.getHealInsInherenPr().length() > 40 ? data.getHealInsInherenPr().substring(30, 40) : data.getHealInsInherenPr().substring(30, data.getHealInsInherenPr().length()));
