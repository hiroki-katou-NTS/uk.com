    ...
    class Kaf018IViewModel extends ko.ViewModel {
        ...
        created() {
            ...
            self.tabs = ko.observableArray([
				...
				{ id: 'tab-6', title: text("KAF018_456"), content: '.tab-content-6', enable: self.checkH3, visible: self.checkH3 },
            ...
            self.selectedTab.subscribe((newValue) => {
                ...
                switch (newValue) {
                    ...
                    case 'tab-3':
                        ...
                        break;
					case 'tab-6':
                        self.screenEditMode(self.monthlyUnconfirmByPrincipal.editMode());
                        break;
                    case 'tab-4':
		...
		startPage(): JQueryPromise<any> {
            ...
            let dfd = $.Deferred();
            block.invisible();
            self.$ajax('at', API.getUseSetting).done(function(useSetting) {
                self.useSetting = useSetting;
                self.$ajax('at', API.getMailTemp).done(function(data: any) {
        ...
        private registerApprovalStatusMail(): void {
            ...
            self.$ajax('at', API.registerMail, listMail).done(function() {
                //画面モード　＝　更新
        ...
        sendTestMail() {
            ...
            self.$ajax('at', API.confirmSenderMail).done(function(data: any) {
                //メッセージ（Msg_800）を表示する
                confirm({ messageId: "Msg_800", messageParams: [data] }).ifYes(() => {
                    ...
                    block.invisible();
                    self.$ajax('at', nts.uk.text.format(API.sendTestMail, mailType)).done(function(result: any) {
