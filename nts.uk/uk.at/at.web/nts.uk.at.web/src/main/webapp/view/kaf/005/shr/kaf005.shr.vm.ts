			...
			<table id="fixed-overtime-hour-table">
				...
				<tbody data-bind="foreach: overTime">
					<tr data-bind="if: visible()">
						...
						<!--A6_8 残業申請時間入力-->
						<td data-bind="style: {'background-color': $parent.visibleModel.c15_3() ? backgroundColor() : (applicationTime() > 0 ? '#F69164' : '')}"><input
							tabindex="12" class="right-content overtimeHoursCheck"
							data-bind=" 
								style: {'background-color': $parent.visibleModel.c15_3() ? backgroundColor() : (applicationTime() > 0 ? '#F69164' : '')},
								ntsTimeEditor: { 
			...
			<table id="fixed-table-holiday">
				...
				<tbody data-bind="foreach: holidayTime">
					<tr data-bind="if: visible()">
						...
						<td class="header" data-bind="text: displayNo"></td>
						<!--A5_6 開始時刻-->
						<td data-bind="style: {'background-color': $parent.visibleModel.c15_3() ? backgroundColor() : (start() > 0 ? '#F69164' : '')}">
						<input tabindex="12" class="right-content"
							data-bind="
								style: {'background-color': $parent.visibleModel.c15_3() ? backgroundColor() : (start() > 0 ? '#F69164' : '')},
								ntsTimeEditor: {
									name: '#[KAF005_337]', 
									value: start, 
									constraint:'OvertimeAppPrimitiveTime',
