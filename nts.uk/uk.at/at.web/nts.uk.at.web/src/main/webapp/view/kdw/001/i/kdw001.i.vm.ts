module nts.uk.at.view.kdw001.i {
    ...
    export module viewmodel {
        export class ScreenModel {
            ...
            listExecutionContentName: Array<string> = [];
            ...
            getByEmpCalAndSumExeLogId(empCalAndSumExeLogId: string) {
                ...
                service.getByEmpCalAndSumExeLogId(empCalAndSumExeLogId).done(function(data: modelkdw001f.IEmpCalAndSumExeLog): any {
                    ...
                    if (self.listExecutionLog().length > 0) {
                        //let dataExecutionLogs = self.empCalAndSumExecLog().executionLogs;
                        let count = 0;
                        _.map(self.listExecutionLog(), 'executionContentName');
                        for (let i = 0; i < self.listExecutionLog().length; i++) {
                            if (!_.map(self.listExecutionLog(), 'executionContentName')[i].equals("月別集計")) {
                                count++;
                            }
                        }
                        if (count) {
