...
public class JpaShiftTableRuleForOrganizationRepo extends JpaRepository implements ShiftTableRuleForOrganizationRepo {
	...
	public void update(String companyId, ShiftTableRuleForOrganization domain) {
		Optional<KrcmtShiftTableRuleForOrg> oldEntity = this.queryProxy()
				.query(SELECT_BY_KEY, KrcmtShiftTableRuleForOrg.class).setParameter("companyId", companyId)
		...
		if (oldEntity.isPresent()) {
			...
			if (oldEntity.get().useWorkAvailabilityAtr == 1) {
				if (newEntity.useWorkAvailabilityAtr == 1) {
					...
					oldEntity
							.get().krcmtShiftTableRuleForOrgAvai.holidayAtr = newEntity.krcmtShiftTableRuleForOrgAvai.holidayAtr;
					oldEntity
							.get().krcmtShiftTableRuleForOrgAvai.shiftAtr = newEntity.krcmtShiftTableRuleForOrgAvai.shiftAtr;
					oldEntity
							.get().krcmtShiftTableRuleForOrgAvai.timeSheetAtr = newEntity.krcmtShiftTableRuleForOrgAvai.timeSheetAtr;
					oldEntity
							.get().krcmtShiftTableRuleForOrgAvai.fromNoticeDays = newEntity.krcmtShiftTableRuleForOrgAvai.fromNoticeDays;
					oldEntity
							.get().krcmtShiftTableRuleForOrgAvai.periodUnit = newEntity.krcmtShiftTableRuleForOrgAvai.periodUnit;

					if (newEntity.krcmtShiftTableRuleForOrgAvai.periodUnit == WorkAvailabilityPeriodUnit.MONTHLY.value) {
						oldEntity
								.get().krcmtShiftTableRuleForOrgAvai.dateCloseDay = newEntity.krcmtShiftTableRuleForOrgAvai.dateCloseDay;
						oldEntity
								.get().krcmtShiftTableRuleForOrgAvai.dateCloseIsLastDay = newEntity.krcmtShiftTableRuleForOrgAvai.dateCloseIsLastDay;
						oldEntity
								.get().krcmtShiftTableRuleForOrgAvai.dateDeadlineDay = newEntity.krcmtShiftTableRuleForOrgAvai.dateDeadlineDay;
						oldEntity
								.get().krcmtShiftTableRuleForOrgAvai.dateDeadlineIsLastDay = newEntity.krcmtShiftTableRuleForOrgAvai.dateDeadlineIsLastDay;
						oldEntity
								.get().krcmtShiftTableRuleForOrgAvai.dateHDUpperlimit = newEntity.krcmtShiftTableRuleForOrgAvai.dateHDUpperlimit;
						...
						oldEntity
								.get().krcmtShiftTableRuleForOrgAvai.weekSetStart = newEntity.krcmtShiftTableRuleForOrgAvai.weekSetStart;
						oldEntity
								.get().krcmtShiftTableRuleForOrgAvai.weekSetDeadlineAtr = newEntity.krcmtShiftTableRuleForOrgAvai.weekSetDeadlineAtr;
						oldEntity
								.get().krcmtShiftTableRuleForOrgAvai.weekSetDeadlineWeek = newEntity.krcmtShiftTableRuleForOrgAvai.weekSetDeadlineWeek;
					} else {
						oldEntity.get().krcmtShiftTableRuleForOrgAvai.dateCloseDay = null;
						oldEntity.get().krcmtShiftTableRuleForOrgAvai.dateCloseIsLastDay = null;
						oldEntity.get().krcmtShiftTableRuleForOrgAvai.dateDeadlineDay = null;
					...
					oldEntity.get().krcmtShiftTableRuleForOrgAvai = null;
				}
			} else {
				if (newEntity.useWorkAvailabilityAtr == 1) {
					oldEntity.get().krcmtShiftTableRuleForOrgAvai = newEntity.krcmtShiftTableRuleForOrgAvai;
				} else {
					oldEntity.get().krcmtShiftTableRuleForOrgAvai = null;
				}
	...
	public void delete(String companyId, TargetOrgIdenInfor targetOrg) {
		...
		if (data.isPresent()) {
			this.commandProxy().remove(KrcmtShiftTableRuleForOrg.class,
