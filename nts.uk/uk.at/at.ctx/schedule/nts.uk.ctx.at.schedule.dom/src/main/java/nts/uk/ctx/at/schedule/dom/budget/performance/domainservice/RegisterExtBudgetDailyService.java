...
public class RegisterExtBudgetDailyService {
		 ...
		 * @return AtomTask
		 */
	public static AtomTask signUp(Require require,TargetOrgIdenInfor targetOrg,ExtBudgetActItemCode itemCode,GeneralDate ymd, Optional<ExtBudgetActualValue> extBudgetActualValue){
		return null;
	...
	public static interface Require {
		/**
		 * [R-1] 外部予算実績を追加する
		 * 日次の外部予算実績Repository.insert(日次の外部予算実績)
		 * ExtBudgetDailyRepository
		 * @param extBudgetDaily
		 */
		void insert(ExtBudgetDaily extBudgetDaily);
		/**[R-2] 外部予算実績を削除する
		 * 日次の外部予算実績Repository.delete(対象組織識別情報, 外部予算実績項目コード, 年月日)
		 * ExtBudgetDailyRepository
		 * @param extBudgetDaily
		 */
