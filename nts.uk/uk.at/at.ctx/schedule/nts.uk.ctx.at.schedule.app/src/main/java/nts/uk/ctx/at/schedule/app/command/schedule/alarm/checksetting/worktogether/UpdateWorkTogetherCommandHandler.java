...
/**
 * UKDesign.UniversalK.就業.KSM_スケジュールマスタ.KSM008 スケジュールのアラームチェック設定.B: 同時出勤指定.メニュー別OCD.同時出勤指定を変更する
 */
@Stateless
public class UpdateWorkTogetherCommandHandler extends CommandHandler<AddWorkTogetherCommand> {
    ...
    protected void handle(CommandHandlerContext<AddWorkTogetherCommand> context) {
        ...
        workTogetherRepo.delete(command.getSid());
        workTogetherRepo.insert(command.toDomain());
    }
}
