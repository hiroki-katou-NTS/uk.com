	...
	public PrepareWorkOutput getWorkInfo(ScheduleCreatorExecutionCommand command, ScheduleCreator creator,
		...
		// 入力パラメータ「作成方法区分」を確認する	
			...
			if (command.getContent().getSpecifyCreation().getCreationMethod().value == CreationMethod.SCHEDULE_COPY.value) {
				prepareWorkOutput = this.copyPastSchedule(command, carrier, targetPeriod, dateInPeriod, masterCache, prepareWorkOutput, creator);
			}
				...
				if (itemDto.get().getMonthlyPattern().isPresent()) {
					...
					if (monthlySetting.isPresent()) {
						...
						WorkingCode workTimeCode = this.getWorkingCode(command, masterCache, itemDto, 
								monthlySetting.get().getWorkInformation().getWorkTimeCode() != null ? new WorkingCode(monthlySetting.get().getWorkInformation().getWorkTimeCode().v()) : null, workType.isPresent() ? workType.get() : null, dateInPeriod, creator);
