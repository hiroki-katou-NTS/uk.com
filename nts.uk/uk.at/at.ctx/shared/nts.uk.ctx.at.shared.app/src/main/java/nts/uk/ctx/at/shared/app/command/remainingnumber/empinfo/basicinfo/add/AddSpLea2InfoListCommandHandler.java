...
@Stateless
public class AddSpLea2InfoListCommandHandler extends CommandHandlerWithResult<List<AddSpecialleave2informationCommand>, List<PeregAddCommandResult>>
implements PeregAddListCommandHandler<AddSpecialleave2informationCommand>{
	@Inject
	...
	protected List<PeregAddCommandResult> handle(
			CommandHandlerContext<List<AddSpecialleave2informationCommand>> context) {
		String cid = AppContexts.user().companyId();
		List<AddSpecialleave2informationCommand> cmd = context.getCommand();
		List<SpecialLeaveBasicInfo> domains = cmd.stream().map(c ->{return new SpecialLeaveBasicInfo(cid, c.getSID(), SpecialLeaveCode.CS00026.value,
