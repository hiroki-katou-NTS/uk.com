...
@Stateless
public class UpdateSpLea18InfoListCommandHandler extends CommandHandlerWithResult<List<UpdateSpecialleave18informationCommand>, List<MyCustomizeException>>
	...
	@Override
	protected List<MyCustomizeException> handle(CommandHandlerContext<List<UpdateSpecialleave18informationCommand>> context) {
		String cid = AppContexts.user().companyId();
		List<UpdateSpecialleave18informationCommand> cmd = context.getCommand();
		List<SpecialLeaveBasicInfo> domains = cmd.stream().map(c ->{return new SpecialLeaveBasicInfo(cid, c.getSID(), SpecialLeaveCode.CS00056.value,
