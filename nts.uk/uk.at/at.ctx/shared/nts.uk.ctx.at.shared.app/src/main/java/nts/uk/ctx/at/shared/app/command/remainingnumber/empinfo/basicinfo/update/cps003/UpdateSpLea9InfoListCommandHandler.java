...
@Stateless
public class UpdateSpLea9InfoListCommandHandler extends CommandHandlerWithResult<List<UpdateSpecialleave9informationCommand>, List<MyCustomizeException>>
	...
	@Override
	protected List<MyCustomizeException> handle(CommandHandlerContext<List<UpdateSpecialleave9informationCommand>> context) {
		String cid = AppContexts.user().companyId();
		List<UpdateSpecialleave9informationCommand> cmd = context.getCommand();
		List<SpecialLeaveBasicInfo> domains = cmd.stream().map(c ->{return new SpecialLeaveBasicInfo(cid, c.getSID(), SpecialLeaveCode.CS00033.value,
