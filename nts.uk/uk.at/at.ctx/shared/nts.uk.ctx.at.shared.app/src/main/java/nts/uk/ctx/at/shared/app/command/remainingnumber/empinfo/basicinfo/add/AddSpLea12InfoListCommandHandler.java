...
@Stateless
public class AddSpLea12InfoListCommandHandler extends CommandHandlerWithResult<List<AddSpecialleave12informationCommand>, List<PeregAddCommandResult>>
implements PeregAddListCommandHandler<AddSpecialleave12informationCommand>{
	@Inject
	...
	public Class<?> commandClass() {
		return AddSpecialleave12informationCommand.class;
	}

	@Override
	protected List<PeregAddCommandResult> handle(
			CommandHandlerContext<List<AddSpecialleave12informationCommand>> context) {
		String cid = AppContexts.user().companyId();
		List<AddSpecialleave12informationCommand> cmd = context.getCommand();
		List<SpecialLeaveBasicInfo> domains = cmd.parallelStream().map(c ->{return new SpecialLeaveBasicInfo(cid, c.getSID(), SpecialLeaveCode.CS00050.value,
