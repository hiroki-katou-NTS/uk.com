...
@Stateless
public class UpdateSpLea4InfoListCommandHandler extends CommandHandlerWithResult<List<UpdateSpecialleave4informationCommand>, List<MyCustomizeException>>
	...
	@Override
	protected List<MyCustomizeException> handle(CommandHandlerContext<List<UpdateSpecialleave4informationCommand>> context) {
		String cid = AppContexts.user().companyId();
		List<UpdateSpecialleave4informationCommand> cmd = context.getCommand();
		List<SpecialLeaveBasicInfo> domains = cmd.stream().map(c ->{return new SpecialLeaveBasicInfo(cid, c.getSID(), SpecialLeaveCode.CS00028.value,
