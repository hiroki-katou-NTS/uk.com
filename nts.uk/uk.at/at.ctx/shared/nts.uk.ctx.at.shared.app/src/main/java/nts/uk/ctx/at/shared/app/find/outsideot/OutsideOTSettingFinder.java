...
public class OutsideOTSettingFinder {
    ...
    public OutsideOTDto findByIdNew() {
        ...
        Optional<OutsideOTSetting> overtimeSetting = this.repository.findById(companyId);
        // call repository find data
        Optional<SuperHD60HConMed> superHoliday = this.superHD60HConMedRepository.findById(companyId);
        val superHD60HConMed = superHoliday.map(SuperHD60HConMedDto::of).orElse(new SuperHD60HConMedDto());
        val overtimeSettingDto = overtimeSetting.map(OutsideOTSettingDto::of).orElse(new OutsideOTSettingDto());

        return new OutsideOTDto(overtimeSettingDto.getBreakdownItems(),
                overtimeSettingDto.getOvertimes(),
                superHD60HConMed.getRoundingTime(),
                superHD60HConMed.getRounding(),
                superHD60HConMed.getSuperHolidayOccurrenceUnit());
    }
}
