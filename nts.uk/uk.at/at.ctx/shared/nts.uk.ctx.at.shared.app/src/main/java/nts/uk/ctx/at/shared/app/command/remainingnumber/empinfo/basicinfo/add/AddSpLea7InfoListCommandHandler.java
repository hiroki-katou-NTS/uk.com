...
@Stateless
public class AddSpLea7InfoListCommandHandler extends CommandHandlerWithResult<List<AddSpecialleave7informationCommand>, List<PeregAddCommandResult>>
implements PeregAddListCommandHandler<AddSpecialleave7informationCommand>{
	@Inject
	...
	public Class<?> commandClass() {
		return AddSpecialleave7informationCommand.class;
	}

	@Override
	protected List<PeregAddCommandResult> handle(
			CommandHandlerContext<List<AddSpecialleave7informationCommand>> context) {
		String cid = AppContexts.user().companyId();
		List<AddSpecialleave7informationCommand> cmd = context.getCommand();
		List<SpecialLeaveBasicInfo> domains = cmd.parallelStream().map(c ->{return new SpecialLeaveBasicInfo(cid, c.getSID(), SpecialLeaveCode.CS00031.value,
