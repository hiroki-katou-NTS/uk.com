...
@Stateless
public class AddSpLea20InfoListCommandHandler extends CommandHandlerWithResult<List<AddSpecialleave20informationCommand>, List<PeregAddCommandResult>>
implements PeregAddListCommandHandler<AddSpecialleave20informationCommand>{
	@Inject
	...
	protected List<PeregAddCommandResult> handle(
			CommandHandlerContext<List<AddSpecialleave20informationCommand>> context) {
		String cid = AppContexts.user().companyId();
		List<AddSpecialleave20informationCommand> cmd = context.getCommand();
		List<SpecialLeaveBasicInfo> domains = cmd.stream().map(c ->{return new SpecialLeaveBasicInfo(cid, c.getSID(), SpecialLeaveCode.CS00058.value,
