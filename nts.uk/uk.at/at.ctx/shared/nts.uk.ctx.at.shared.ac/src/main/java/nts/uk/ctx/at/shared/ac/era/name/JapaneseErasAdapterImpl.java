...
public class JapaneseErasAdapterImpl implements JapaneseErasAdapter {
	...
	public JapaneseEras getAllEras() {
		...
		List<JapaneseEraName> jpEraNames = eraDomList.stream().map(e -> new JapaneseEraName(e.getEraName().toString(), e.getSymbol().toString(), e.getStartDate(), e.getEndDate())).collect(Collectors.toList());
