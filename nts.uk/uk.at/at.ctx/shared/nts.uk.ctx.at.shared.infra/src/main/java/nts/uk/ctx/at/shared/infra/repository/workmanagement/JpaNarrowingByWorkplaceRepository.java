...
import nts.uk.ctx.at.shared.infra.entity.workmanagement.worknarrowingdown.metamodel.KsrmtTaskAssignWkpPk_;
import nts.uk.ctx.at.shared.infra.entity.workmanagement.worknarrowingdown.metamodel.KsrmtTaskAssignWkp_;
...
public class JpaNarrowingByWorkplaceRepository extends JpaRepository implements NarrowingByWorkplaceRepository {
    ...
    @Override
    public List<NarrowingDownWorkByWorkplace> getListWorkByCid(String cid) {
        EntityManager entityManager = this.getEntityManager();
        CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
    ...
    public List<NarrowingDownWorkByWorkplace> getListWork(List<String> workPlaceIds, TaskFrameNo taskFrameNo) {
        ...
        Root<KsrmtTaskAssignWkp> root = criteriaQuery.from(KsrmtTaskAssignWkp.class);
        criteriaQuery.select(root);
        List<NarrowingDownWorkByWorkplace> rs = new ArrayList<>();
    ...
    @Override
    public List<NarrowingDownWorkByWorkplace> getListWorkByWpl(String workPlaceId) {
        EntityManager entityManager = this.getEntityManager();
        CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
        CriteriaQuery<KsrmtTaskAssignWkp> criteriaQuery = criteriaBuilder.createQuery(KsrmtTaskAssignWkp.class);
        Root<KsrmtTaskAssignWkp> root = criteriaQuery.from(KsrmtTaskAssignWkp.class);
    ...
    private List<NarrowingDownWorkByWorkplace> toDomains(List<KsrmtTaskAssignWkp> entitys) {
        ...
        listMap.keySet().forEach(e -> {
            ...
            if (wkpPkList.size() > 0) {
                rs.add(new NarrowingDownWorkByWorkplace(
                        e,
