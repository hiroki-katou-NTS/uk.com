package nts.uk.ctx.at.shared.infra.repository.scherec.dailyattdcal.premiumitem;
...
import nts.uk.ctx.at.shared.infra.entity.scherec.dailyattdcal.premiumitem.KmldpPremiumAttendancePK;
import nts.uk.ctx.at.shared.infra.entity.scherec.dailyattdcal.premiumitem.KmldtPremiumAttendance;
import nts.uk.ctx.at.shared.infra.entity.scherec.dailyattdcal.premiumitem.KmlmpPersonCostCalculationPK;
import nts.uk.ctx.at.shared.infra.entity.scherec.dailyattdcal.premiumitem.KmlmtPersonCostCalculation;
import nts.uk.ctx.at.shared.infra.entity.scherec.dailyattdcal.premiumitem.KmlspPremiumSetPK;
import nts.uk.ctx.at.shared.infra.entity.scherec.dailyattdcal.premiumitem.KmlstPremiumSet;
...
//	}
	...
	private PersonCostCalculation toSimpleDomain(KmlmtPersonCostCalculation kmlmtPersonCostCalculation){
		return new PersonCostCalculation(
				...
				new PersonCostRoundingSetting(
						new UnitPriceRoundingSetting(UnitPriceRounding.valueOf(kmlmtPersonCostCalculation.unitPrice)),
						new AmountRoundingSetting(
								AmountUnit.valueOf(kmlmtPersonCostCalculation.costUnit),
	...
	private KmlmtPersonCostCalculation toPersonCostCalculationEntity(PersonCostCalculation personCostCalculation) {
		return new KmlmtPersonCostCalculation(
				...
				personCostCalculation.getRoundingSet().getUnitPrice().getRounding().value,
				personCostCalculation.getRoundingSet().getCost().getUnit().value,
				personCostCalculation.getRoundingSet().getCost().getRounding().value);
	}
...
//				.setParameter("date", date).getList(c -> toDomainPersonCostCalculation(c));
	...
	public List<PersonCostCalculation> findByCompanyIDAndDisplayNumberNotFull(String companyID, DatePeriod date, List<Integer> itemNos) {
		StringBuilder query = new StringBuilder();
		query.append("SELECT a.HIS_ID, a.START_DATE, a.END_DATE, a.UNITPRICE_ATR, a.MEMO, a.UNIT_PRICE_ROUNDING, a.COST_UNIT, a.COST_ROUNDING");
		query.append(" b.PREMIUM_RATE, b.PREMIUM_NO, c.ATTENDANCE_ID FROM KMLMT_COST_CALC_SET a ");
		...
		try (PreparedStatement statement = this.connection().prepareStatement(query.toString())) {
			...
			return new NtsResultSet(statement.executeQuery()).getList(rec -> {
				...
				val.put("UNITPRICE_ATR", rec.getInt("UNITPRICE_ATR"));
				val.put("MEMO", rec.getString("MEMO"));
				val.put("UNIT_PRICE_ROUNDING", rec.getInt("UNIT_PRICE_ROUNDING"));
				val.put("COST_UNIT", rec.getInt("COST_UNIT"));
				val.put("COST_ROUNDING", rec.getInt("COST_ROUNDING"));
				return val;
			...
			.stream().map(et -> {
				...
				return new PersonCostCalculation(
						...
						UnitPrice.valueOf((String) et.getValue().get(0).get("UNITPRICE_ATR")),
						new Memo((String) et.getValue().get(0).get("MEMO")),
						premiumSettings,
						new PersonCostRoundingSetting(
								new UnitPriceRoundingSetting(UnitPriceRounding.valueOf((Integer) et.getValue().get(0).get("UNIT_PRICE_ROUNDING"))),
								new AmountRoundingSetting(
										AmountUnit.valueOf((Integer) et.getValue().get(0).get("COST_UNIT")),
										AmountRounding.valueOf((Integer) et.getValue().get(0).get("COST_ROUNDING")))));
			}).collect(Collectors.toList());
