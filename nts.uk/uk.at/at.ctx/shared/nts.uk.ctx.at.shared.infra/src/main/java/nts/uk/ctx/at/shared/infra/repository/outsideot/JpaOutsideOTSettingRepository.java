...
public class JpaOutsideOTSettingRepository extends JpaRepository
    ...
    public Optional<OutsideOTSetting> findById(String companyId) {
        ...
        Optional<TimeRoundingOfExcessOutsideTime> optionalExcessOutsideTime = getOutRound(companyId);

        // check exist data
        if (kshstOutsideOtSet.isPresent()) {
            return Optional
                    .ofNullable(this.toDomain(kshstOutsideOtSet.get(), entityOvertimeBRDItem, entityOvertime, optionalExcessOutsideTime));
        }
        // default data
        return Optional.ofNullable(
                this.toDomain(new KshmtOutsideSet(), entityOvertimeBRDItem, entityOvertime, optionalExcessOutsideTime));
    ...
    public void save(OutsideOTSetting domain) {
        ...
        Optional<TimeRoundingOfExcessOutsideTime> optionalExcessOutsideTime = getOutRound(companyId);
        val outsideTimeOptional = domain.getTimeRoundingOfExcessOutsideTime();
        if (outsideTimeOptional.isPresent()) {
            val outsideTime = outsideTimeOptional.get();
            if (optionalExcessOutsideTime.isPresent()) {
                updateOutRound(outsideTime, companyId, UPDATE);
            } else {
                updateOutRound(outsideTime, companyId, CREATE);
            }
        }
        // check exist data
    ...
    private OutsideOTSetting toDomain(KshmtOutsideSet entity, List<KshmtOutsideDetail> entityOvertimeBRDItems,
                                      List<KshmtOutside> entityOvertime, Optional<TimeRoundingOfExcessOutsideTime> optionalExcessOutsideTime) {

        return new OutsideOTSetting(entity.getCid(), new OvertimeNote(entity.getNote()),
                entityOvertimeBRDItems.stream().map(c -> c.domain()).collect(Collectors.toList()),
                EnumAdaptor.valueOf(entity.getCalculationMethod(), OutsideOTCalMed.class),
                entityOvertime.stream().map(c -> c.domain()).collect(Collectors.toList()), optionalExcessOutsideTime);
    }
    ...
    /**
     * Get DOMAIN TimeRoundingOfExcessOutsideTime FROM TABLE KRCMT_CALC_M_OUTSIDE_RND (KMK010)
     *
     * @param companyId
     * @return
     */
    @SneakyThrows
    private Optional<TimeRoundingOfExcessOutsideTime> getOutRound(String companyId) {
        String sqlJdbc = "SELECT * FROM KRCMT_CALC_M_OUTSIDE_RND KOOS "
                + "WHERE KOOS.CID = ?";
        ...
        try (PreparedStatement stmt = this.connection().prepareStatement(sqlJdbc)) {
            ...
            return new NtsResultSet(stmt.executeQuery())
                    .getSingle(rec -> {
                        TimeRoundingOfExcessOutsideTime entity =
                                TimeRoundingOfExcessOutsideTime.of(EnumAdaptor.valueOf(rec.getInt("ROUND_UNIT")
                                        , Unit.class), EnumAdaptor.valueOf(rec.getInt("ROUND_PROC")
                                        , RoundingProcessOfExcessOutsideTime.class));
                        return entity;
                    });
        }
    }
    ...
    /**
     * UPDATE,INSERT KRCMT_CALC_M_OUTSIDE_RND (KMK010)
     *
     * @param outsideTime
     * @param companyId
     * @param act
     */
    @SneakyThrows
    private void updateOutRound(TimeRoundingOfExcessOutsideTime outsideTime, String companyId, String act) {
        String sqlJdbc = null;
        switch (act) {
            case UPDATE:
                sqlJdbc = " UPDATE KRCMT_CALC_M_OUTSIDE_RND  "
                        + " SET ROUND_UNIT =  ? "
                        + " , ROUND_PROC =  ? "
                        + " WHERE CID = ? ";
                break;
            case CREATE:
                sqlJdbc = " INSERT INTO KRCMT_CALC_M_OUTSIDE_RND  "
        ...
        Integer roundingUnit = outsideTime.getRoundingUnit() != null ? outsideTime.getRoundingUnit().value : null;
        Integer roundingProcess = outsideTime.getRoundingProcess() != null ? outsideTime.getRoundingProcess().value : null;
        try (PreparedStatement stmt = this.connection().prepareStatement(sqlJdbc)) {
            if (roundingUnit != null && roundingProcess != null) {
                stmt.setInt(1, roundingUnit);
                stmt.setInt(2, roundingProcess);
