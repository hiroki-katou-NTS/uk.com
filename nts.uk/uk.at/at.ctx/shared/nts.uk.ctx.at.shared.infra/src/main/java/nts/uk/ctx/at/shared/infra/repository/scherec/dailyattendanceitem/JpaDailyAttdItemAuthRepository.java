...
public class JpaDailyAttdItemAuthRepository extends JpaRepository implements DailyAttdItemAuthRepository {
	...
	public Optional<DailyAttendanceItemAuthority> getDailyAttdItem(String companyID, String authorityDailyId) {
		List<DisplayAndInputControl> data = this.queryProxy().query(SELECT_BY_AUTHORITY_DAILY_ID,KshstDailyServiceTypeControl.class)
				.setParameter("companyID", companyID)
				.setParameter("authorityDailyID", authorityDailyId)
				.getList(c->c.toDomain());
		if(data.isEmpty())
			return Optional.empty();
		DailyAttendanceItemAuthority dailyAttendanceItemAuthority = new DailyAttendanceItemAuthority(
				companyID,authorityDailyId,data);
		...
		public void updateDailyAttdItemAuth(DailyAttendanceItemAuthority dailyAttendanceItemAuthority) {
			List<KshstDailyServiceTypeControl> newEntity = dailyAttendanceItemAuthority.getListDisplayAndInputControl().stream()
					.map(c->KshstDailyServiceTypeControl.toEntity(
							dailyAttendanceItemAuthority.getCompanyID(),
							dailyAttendanceItemAuthority.getAuthorityDailyId(), c))
					.collect(Collectors.toList());
			List<KshstDailyServiceTypeControl> updateEntity = this.queryProxy().query(SELECT_BY_AUTHORITY_DAILY_ID,KshstDailyServiceTypeControl.class)
