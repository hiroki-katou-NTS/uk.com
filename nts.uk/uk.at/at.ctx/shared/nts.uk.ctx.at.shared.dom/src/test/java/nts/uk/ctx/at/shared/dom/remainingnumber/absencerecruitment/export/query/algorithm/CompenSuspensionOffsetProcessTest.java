...
public class CompenSuspensionOffsetProcessTest {
	...
	public void test() {
		...
		List<SeqVacationAssociationInfo> resultActual = CompenSuspensionOffsetProcess.process(require, CID, SID,
		...
		assertThat(lstAccDetail)
				.extracting(x -> (x.getUnbalanceCompensation().isPresent()
						? x.getUnbalanceCompensation().get().getDigestionCate()
						: Optional.empty()))
				.containsExactly(Optional.empty(), Optional.empty(), Optional.empty(), DigestionAtr.USED);
	}
