 ...
 */
package nts.uk.ctx.at.shared.dom.alarmList.worktime;
...
public class PlanActualWorkTime extends WorkTimeCondition {
	...
	public WorkCheckResult checkWorkTime(WorkInfoOfDailyAttendance workInfo, Optional<SnapShot> snapshot) {
		...
		if(this.getComparePlanAndActual() == FilterByCompare.NOT_SELECTED){
			if(workInfo.getRecordInfo().getWorkTimeCode() == null && scheWorkTime == null) {
				compareTypeError = WorkCheckResult.NOT_ERROR;
			} else if(workInfo.getRecordInfo().getWorkTimeCode() == null || scheWorkTime == null) {
				compareTypeError = WorkCheckResult.ERROR;
			} else if(workInfo.getRecordInfo().getWorkTimeCode().equals(scheWorkTime)){
				compareTypeError = WorkCheckResult.NOT_ERROR;
			} else {
				compareTypeError = WorkCheckResult.ERROR;
			}
