 ...
 */
package nts.uk.ctx.at.shared.dom.alarmList.worktype;
	...
	public WorkCheckResult checkWorkType(WorkInfoOfDailyAttendance workInfo, Optional<SnapShot> snapshot) {
		if (this.targetWorkType != null) {
			if(this.targetWorkType.isUse() && !this.targetWorkType.getLstWorkType().isEmpty()){
				if(workInfo.getRecordInfo().getWorkTypeCode().equals(snapshot.map(c -> c.getWorkInfo().getWorkTypeCode()).orElse(null))
