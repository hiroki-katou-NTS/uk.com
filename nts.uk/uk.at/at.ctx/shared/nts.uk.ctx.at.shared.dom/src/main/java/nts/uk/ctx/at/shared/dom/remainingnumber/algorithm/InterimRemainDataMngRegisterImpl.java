	...
	private void registryInterimResereLeave(RegistryInterimResereLeaveDataInput dataInput) {
		...
		if(!lstAnnualHolidayCreate.isEmpty()
			...
			case ANNUAL:
				...
				break;
			case FUNDINGANNUAL:
				TempReserveLeaveManagement resereDataCreate = earchData.getResereData().get();
//				resereDataCreate.setResereId(mngId);
		...
		if(lstAnnualHolidayCreate.isEmpty()
			...
			case ANNUAL:
				...
				break;
			case FUNDINGANNUAL:
				resereLeave.remove(annualHolidayRemainBefore.getSID(), annualHolidayRemainBefore.getYmd());
				break;
			case PAUSE:
