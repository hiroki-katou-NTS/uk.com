...
public class CheckExistenceMasterDomainService {
    ...
    public static void checkExistenceTaskMaster(Require require, TaskFrameNo taskFrameNo, List<TaskCode> childTaskList) {
        ...
        val workList = require.getTasks(cid, taskFrameNo);
        // $除外するコード一覧 = $作業一覧: map $.コード
        Map<String, Tasks> frameNoTaskMap = workList.stream().collect(Collectors.toMap(e -> e.getCode().v(), e -> e));
    ...
    public interface Require {
        // R-1] 作業を取得する
        List<Tasks> getTasks(String cid, TaskFrameNo taskFrameNo);
    }
}
