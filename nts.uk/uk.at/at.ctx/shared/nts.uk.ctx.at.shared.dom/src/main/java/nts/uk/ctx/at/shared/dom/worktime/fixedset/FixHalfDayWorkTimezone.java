...
public class FixHalfDayWorkTimezone extends DomainObject {
	...
	public void validate() {
		...
		List<String> lstWorkTime = this.workTimezone.getLstWorkingTimezone().stream().map(item -> item.getTimezone().toString())
				.collect(Collectors.toList());
		List<String> lstOTTTime = this.workTimezone.getLstOTTimezone().stream().map(item -> item.getTimezone().toString())
				.collect(Collectors.toList());
