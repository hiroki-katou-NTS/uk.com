...
public class WorkingConditionItemServiceIpm implements WorkingConditionItemService {
	...
	/* (non-Javadoc)
	 * @see nts.uk.ctx.at.shared.dom.workingcondition.WorkingConditionItemService#getHolidayWorkSchedule(java.lang.String, java.lang.String, nts.arc.time.GeneralDate, java.lang.String)
	 */
	@Override
	public Optional<SingleDaySchedule> getHolidayWorkSchedule(String companyId, String employeeId, GeneralDate baseDate,
			String workTypeCode) {
		
		Optional<WorkingConditionItem> optWorkingCondItem = this.repositoryWorkingConditionItem.findWorkingConditionItemByPersWorkCat(employeeId, baseDate);
		...
		if(optWorkingCondItem.isPresent()){
			...
			if(!optpublicHoliday.isPresent()){
				Optional<HolidaySetting> optHolidaySetting = this.repositoryHolidaySetting.findBy(companyId);
	 ...
	 *
	 * @param persWorkCategory the pers work category
	 * @return the boolean
	 */
