...
public class AnnualWorkScheduleExportGenerator extends AsposeCellsReportGenerator implements AnnualWorkScheduleGenerator {
	...
	public void generate(FileGeneratorContext fileContext, ExportDataSource dataSource) {
		try (AsposeCellsReportContext reportContext = this.createContext(TEMPLATE_FILE)) {
			...
			reportContext.setHeader(0, dataSource.getData().getHeader().getTitle());
	...
	private void print(RangeCustom range, EmployeeData emp, boolean isNewPage) {
		...
		range.cell("month1st").putValue("10:00");
		range.cell("month2nd").putValue("20:00");
		range.cell("month3rd").putValue("30:00");
	}
