 ...
 * @author thanh_nx
 *
 *         NRWeb照会日別実績XMLとHTMLに変換
 */
public class NRWebQueryDailyXmlHtml {

	// 情報処理
	public static Response process(Require require, NRWebQuerySidDateParameter param) {

		List<NRWebScheduleRecordData> dto = GetNRWebQueryDaily.process(require, param);
	...
	private static String createHtml(List<NRWebScheduleRecordData> dto) {
		...
		builder.append(NRWebQueryMenuName.DAILY.value);
	...
	private static String createXml(List<NRWebScheduleRecordData> dto) {
		StringBuilder builder = new StringBuilder();
		builder.append("<kindata type='2'><item type='head'>");
		if (!dto.isEmpty()) {
	...
	public static interface Require extends GetNRWebQueryDaily.Require {
