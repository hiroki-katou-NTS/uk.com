...
public class NRQueryOvertimeAppImport extends NRQueryAppImport {
	...
	public String createXml() {
		...
		this.flexOvertime.ifPresent(data -> builder
				.append(String.format("<subitem title='フレックス超過時間' value='%s'  align='1' valign='1' />", data)));

		this.overTimeWoktypeName.ifPresent(
				data -> builder.append(String.format("<subitem title='勤務種類' value='%s'  align='1' valign='1' />", data)));
		this.overTimeZoneName.ifPresent(data -> builder
				.append(String.format("<subitem title='就業時間帯' value='%s'  align='1' valign='1' />", data)));
		this.anyItemLst.forEach(data -> builder.append(String
				.format("<subitem title='%s' value='%s'  align='1' valign='1' />", data.getName(), data.getValue())));
	...
	public String createHtml() {
		...
		this.overtimeQuotalst.forEach(data -> builder
				.append(String.format("<DT>%s</DT><DD>%s</DD>", data.getOvertimeQuotaName(), data.getOvertimeHours())));
		this.midnightIimeOutside.ifPresent(data -> builder.append(String.format("<DT>外深夜時間</DT><DD>%s</DD>", data)));
		this.flexOvertime.ifPresent(data -> builder.append(String.format("<DT>フレックス超過時間</DT><DD>%s</DD>", data)));
		this.overTimeWoktypeName.ifPresent(data -> builder.append(String.format("<DT>勤務種類</DT><DD>%s</DD>", data)));
		this.overTimeZoneName.ifPresent(data -> builder.append(String.format("<DT>就業時間帯</DT><DD>%s</DD>", data)));
		this.anyItemLst.forEach(
				data -> builder.append(String.format("<DT>%s</DT><DD>%s</DD>", data.getName(), data.getValue())));
