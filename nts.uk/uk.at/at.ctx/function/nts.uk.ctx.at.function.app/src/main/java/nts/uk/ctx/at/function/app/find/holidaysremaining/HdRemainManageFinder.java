...
public class HdRemainManageFinder {
	...
	public DateHolidayRemainingDto getDate(){
		...
		if (employmentHisOptional.isPresent()) {
			...
			Optional<ClosureEmployment> closureEmployment = closureEmploymentRepository.findByEmploymentCD(companyId, employmentHisOptional.get().getEmploymentCode());
			// アルゴリズム「当月の年月を取得する」を実行する (thực hiện thuật toán 「当月の年月を取得する」)
			YearMonth ym = closureRepository.findById(companyId, closureEmployment.get().getClosureId()).get().getClosureMonth().getProcessingYm();
			// 当月の期間を算出する	(Tính thời gian của tháng này 当月)
