...
@Stateless
public class UpdateAlarmListExtraProcessStatusCmdHandler extends CommandHandler<AlarmListExtraProcessStatusCmd> {
	...
	protected void handle(CommandHandlerContext<AlarmListExtraProcessStatusCmd> context) {
		...
		if(data.isPresent()) {
			this.repo.updateAlListExtaProcess(AlarmListExtraProcessStatusCmd.toDomain(command));
