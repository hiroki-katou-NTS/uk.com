...
public class ItemOutTblBookDto {
	...
	*/
	...
	public static ItemOutTblBookDto fromDomain(ItemOutTblBook domain) {
		return new ItemOutTblBookDto(domain.getCid(), domain.getCd().v(), domain.getSetOutCd(), domain.getSortBy(),
									 domain.getHeadingName().v(), domain.getUseClass(), domain.getValOutFormat().value);
	}
}
