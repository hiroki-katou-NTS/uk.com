...
public class JpaWorkLocationRepository extends JpaRepository implements WorkLocationRepository {
	...
	private static final String SELECT_ALL_BY_COMPANY = SELECT + " WHERE c.kwlmtWorkLocationPK.contractCode = :contractCode order by c.kwlmtWorkLocationPK.workLocationCD asc";
	...
	public void updateWorkLocation(WorkLocation workLocation) {
		Optional<KrcmtWorkLocation> oldData = this.queryProxy()
				.query(SELECT_SINGLE, KrcmtWorkLocation.class)
				.setParameter("contractCode", workLocation.getContractCode().v())
				.setParameter("workLocationCD", workLocation.getWorkLocationCD().v())
				.getSingle();
	...
	public List<Ipv4Address> getIPAddressByStartEndIP(String contractCode, int net1, int net2, int host1, int host2,
		...
		List<Ipv4Address> datas = this.queryProxy().query(SELECT_IPADDRESS_BY_START_END_IP, KrcmtIP4Address.class)
				.setParameter("contractCode", contractCode)
				.setParameter("net1", net1)
				.setParameter("net2", net2)
				.setParameter("host1", host1)
				.setParameter("host2", host2)
