...
//				.getList().stream().map(r-> r.pk.roleId).collect(Collectors.toList());
		...
		CommonSettingsStampInput domain = toDomain(entityOpt.get());
		
		return Optional.of(domain);
	}

	public CommonSettingsStampInput toDomain(KrcmtStampFunction entity) {
		return new CommonSettingsStampInput(entity.cid,
				entity.googleMapUseArt == 1 ? true : false,
				entity.mapAddress == null ? Optional.empty():Optional.of(new MapAddress(entity.mapAddress)),
				EnumAdaptor.valueOf(entity.supportUseArt, NotUseAtr.class));
	}
