...
/**
 * リポジトリ実装：社員の変形労働時間勤務の法定内集計設定
 * @author shuichi_ishida
 */
@Stateless
public class JpaLegalAggrSetOfIrgForSya extends JpaRepository implements LegalAggrSetOfIrgForSyaRepository {

	/** 追加 */
	@Override
	public void insert(String companyId, String employeeId, LegalAggrSetOfIrg legalAggrSetOfIrg) {
		this.commandProxy().insert(toEntity(companyId, employeeId, legalAggrSetOfIrg, false));
	}
	
	/** 更新 */
	@Override
	public void update(String companyId, String employeeId, LegalAggrSetOfIrg legalAggrSetOfIrg) {
		this.toEntity(companyId, employeeId, legalAggrSetOfIrg, true);
	}
	 ...
	 */
	private KrcstMonsetSyaIrgAggr toEntity(String companyId, String employeeId,
		...
		KrcstMonsetSyaIrgAggr entity;
		if (execUpdate) {
			entity = this.queryProxy().find(key, KrcstMonsetSyaIrgAggr.class).get();
		}
		else {
			entity = new KrcstMonsetSyaIrgAggr();
			entity.PK = key;
		}
