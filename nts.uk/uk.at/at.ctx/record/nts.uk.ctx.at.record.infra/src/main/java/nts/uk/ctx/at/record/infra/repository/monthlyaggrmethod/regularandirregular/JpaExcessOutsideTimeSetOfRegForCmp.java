...
public class JpaExcessOutsideTimeSetOfRegForCmp extends JpaRepository implements ExcessOutsideTimeSetOfRegForCmpRepository {
	...
	/** 追加 */
	@Override
	public void insert(String companyId, ExcessOutsideTimeSet excessOutsideTimeSet) {
		this.commandProxy().insert(toEntity(companyId, excessOutsideTimeSet, false));
	}
	
	/** 更新 */
	@Override
	public void update(String companyId, ExcessOutsideTimeSet excessOutsideTimeSet) {
		this.toEntity(companyId, excessOutsideTimeSet, true);
	}
	 ...
	 */
	private KrcstMonsetCmpRegExot toEntity(String companyId,
		...
		KrcstMonsetCmpRegExot entity;
		if (execUpdate){
			entity = this.queryProxy().find(key, KrcstMonsetCmpRegExot.class).get();
		}
		else {
			entity = new KrcstMonsetCmpRegExot();
			entity.PK = key;
		}
