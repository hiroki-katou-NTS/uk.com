...
public class MSConversionInfo implements StampInfoConversion {
	...
	private StampTypeOutput getStampOutput(StampClassifi stampClassifi) {
		...
		switch (stampDes.get()) {
		case ATTENDANCE:
			return new StampTypeOutput(ChangeClockAtr.GOING_TO_WORK, Optional.empty());
		case START_IN:
			return new StampTypeOutput(ChangeClockAtr.OVER_TIME, Optional.empty());
		case SUPPORT_ATTENDANCE:
			return new StampTypeOutput(ChangeClockAtr.GOING_TO_WORK, Optional.empty());
		case LEAV:
			return new StampTypeOutput(ChangeClockAtr.WORKING_OUT, Optional.empty());
		case START_OUT:
			return new StampTypeOutput(ChangeClockAtr.BRARK, Optional.empty());
		case PRIVATE_OUT:
			return new StampTypeOutput(ChangeClockAtr.GO_OUT, Optional.of(GoingOutReason.PRIVATE));
		case PUBLIC_OUT:
			return new StampTypeOutput(ChangeClockAtr.GO_OUT, Optional.of(GoingOutReason.PUBLIC));
		case PAID_OUT:
			return new StampTypeOutput(ChangeClockAtr.GO_OUT, Optional.of(GoingOutReason.COMPENSATION));
		case UNION_OUT:
			return new StampTypeOutput(ChangeClockAtr.GO_OUT, Optional.of(GoingOutReason.UNION));
		case IN_SUPPORT_START:
			return new StampTypeOutput(ChangeClockAtr.START_OF_SUPPORT, Optional.empty());
		case OUT_SUPPORT_END:
			return new StampTypeOutput(ChangeClockAtr.END_OF_SUPPORT, Optional.empty());
		case GO_OUT:
			return new StampTypeOutput(ChangeClockAtr.RETURN, Optional.empty());
