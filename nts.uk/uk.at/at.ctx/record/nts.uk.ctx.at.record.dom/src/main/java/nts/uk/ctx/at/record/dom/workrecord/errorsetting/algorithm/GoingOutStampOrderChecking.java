	...
	public void goingOutStampOrderChecking(String companyId, String employeeId, GeneralDate processingDate,
		...
		if (outingTimeOfDailyPerformance != null && !outingTimeOfDailyPerformance.getOutingTimeSheets().isEmpty()) {
			...
			for (OutingTimeSheet outingTimeSheet : outingTimeSheets) {
				if (outingTimeSheet.getComeBack().getStamp().isPresent() && outingTimeSheet.getGoOut().getStamp().isPresent()) {
					if (outingTimeSheet.getGoOut().getStamp().get().getTimeWithDay()
						...
						TimeWithDayAttr stampStartTimeFirstTime = outingTimeSheet.getGoOut().getStamp().get()
								.getTimeWithDay();
						TimeWithDayAttr endStartTimeFirstTime = outingTimeSheet.getComeBack().getStamp().get()
								.getTimeWithDay();
						...
						for (OutingTimeSheet timeSheet : outingTimeSheetsTemp) {
							TimeWithDayAttr stampStartTimeSecondTime = timeSheet.getGoOut().getStamp().get()
									.getTimeWithDay();
							TimeWithDayAttr endStartTimesecondTime = timeSheet.getComeBack().getStamp().get()
									.getTimeWithDay();
...
//				.findByKey(employeeID, processingDate);
		...
		if (timeLeavingOfDailyPerformance != null && !timeLeavingOfDailyPerformance.getTimeLeavingWorks().isEmpty()) {
			...
			for (TimeLeavingWork timeLeavingWork : timeLeavingWorks) {
				...
				TimeWithDayAttr stampStartTimeSecondTime = timeLeavingWork.getAttendanceStamp().getStamp().get()
						.getTimeWithDay();
				TimeWithDayAttr endStartTimesecondTime = timeLeavingWork.getLeaveStamp().getStamp().get()
						.getTimeWithDay();
				TimeSpanForCalc timeSpanSecondTime = new TimeSpanForCalc(stampStartTimeSecondTime,
						endStartTimesecondTime);
				DuplicateStateAtr duplicateStateAtr = this.rangeOfDayTimeZoneService
...
//							.findByKey(employeeID, processingDate);
					if (temporaryTimeOfDailyPerformance != null && !temporaryTimeOfDailyPerformance.getTimeLeavingWorks().isEmpty()) {
						...
						for (TimeLeavingWork leavingWork : leavingWorks) {
							TimeWithDayAttr newStampStartTimeSecondTime = leavingWork.getAttendanceStamp().getStamp()
									.get().getTimeWithDay();
							TimeWithDayAttr newEndStartTimesecondTime = leavingWork.getLeaveStamp().getStamp().get()
									.getTimeWithDay();
