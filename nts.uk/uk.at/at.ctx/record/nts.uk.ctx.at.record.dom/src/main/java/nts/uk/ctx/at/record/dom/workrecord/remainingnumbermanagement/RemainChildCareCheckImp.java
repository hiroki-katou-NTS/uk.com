...
public class RemainChildCareCheckImp implements RemainChildCareCheck {
    ...
    @Inject
    private ChildNursingSettingCheck childNursingSettingCheck;
    ...
    @Override
    public List<ChildCareNurseErrors> checkRemainChildCare(RemainChildCareCheckParam param) {
        val require = childCareNurseRequireImplFactory.createRequireImpl();
        val checkRequire = new RequireImp();
        
        // 子の看護設定が存在するかチェック
        boolean settingExist = childNursingSettingCheck.check(checkRequire, param.getSID());
        ...
        if (settingExist) {
            ...
            if (aggrResultOfChildCareNurse.getChildCareNurseErrors().size() > 0) {
...
//                List<EmployeeMonthlyPerError> employeeMonthlyPerError = createChildCareErrors.createChildCareErrors(CreateChildCareErrorsParam);
                ...
                return aggrResultOfChildCareNurse.getChildCareNurseErrors();
            }
        } else {
