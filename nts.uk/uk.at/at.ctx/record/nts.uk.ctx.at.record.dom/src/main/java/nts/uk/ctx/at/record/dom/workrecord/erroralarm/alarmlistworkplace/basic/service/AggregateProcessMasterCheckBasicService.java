        ...
        for (BasicFixedExtractionCondition basic : basicConditions) {
            ...
            switch (no) {
                ...
                case NO_REF_TIME_SET:
                    // 基準時間を確認する。
                    extractResults = refTimeSetCfmService.confirm(cid, basicItem.getName(), basic.getDisplayMessage(), ymPeriod);
                    break;
                case EST36_TIME_NOT_SET:
                    // ３６協定目安時間確認
                    extractResults = est36TimeCfmService.confirm(cid, basicItem.getName(), basic.getDisplayMessage(), ymPeriod);
                    break;
                case UNSET_PUBLIC_HD:
                    // 公休日数を確認する。
                    extractResults = publicHdCfmService.confirm(cid, basicItem.getName(), basic.getDisplayMessage(), ymPeriod);
                    break;
                case EST_TIME_AMOUNT_NOT_SET:
                    // 目安時間・金額を確認する。
                    extractResults = estTimeAmountCfmService.confirm(cid, basicItem.getName(), basic.getDisplayMessage(), ymPeriod);
                    break;
            }

            // アラームリスト抽出情報（職場）を作成してList＜アラームリスト抽出情報（職場）＞を追加
            List<AlarmListExtractionInfoWorkplaceDto> results = extractResults.stream().map(x ->
                    new AlarmListExtractionInfoWorkplaceDto(basic.getId(), 0, x))
