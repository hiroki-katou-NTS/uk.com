...
//	}
	...
	public static WithinWorkTimeFrame createWithinWorkTimeFrame(WithinWorkTimeFrame duplicateTimeSheet,
		...
		//遅刻時間帯の作成
		LateTimeSheet lateTimeSheet = new LateTimeSheet(Optional.empty(),Optional.empty(),workNo,Optional.empty());
...
//												   		  coreTimeSetting);
		...
		//早退時間帯の作成
		LeaveEarlyTimeSheet LeaveEarlyTimeSheet = new LeaveEarlyTimeSheet(Optional.empty(), Optional.empty(), workNo, Optional.empty());
...
//														  					coreTimeSetting);		
	...
	public boolean jugmentDeductLateEarly(PremiumAtr premiumAtr,NotUseAtr deductLateLeaveEarly) {
		...
		if(premiumAtr.isRegularWork()&&deductLateLeaveEarly.isDonot()) {
			return true;
