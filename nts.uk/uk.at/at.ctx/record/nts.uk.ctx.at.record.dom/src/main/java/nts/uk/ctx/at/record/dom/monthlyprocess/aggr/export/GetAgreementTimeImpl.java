...
public class GetAgreementTimeImpl implements GetAgreementTime {
	...
	private AgreementTimeOutput getAverageAndYearProc(String companyId, String employeeId, YearMonth averageMonth,
		...
		Map<YearMonth, AgreementTimeOfManagePeriod> agreTimeMap =
				this.getAggrAgreementTime(companyId, employeeId, procYmList, scheRecAtr, closure, companySets, employeeSets);
		...
		if (workingCondItemOpt.isPresent()) {
			val workingSystem = workingCondItemOpt.get().getLaborSystem();
			val basicSet = this.repositories.getAgreementDomainService().getBasicSet(
	 ...
	 */
	private Map<YearMonth, AgreementTimeOfManagePeriod> getAggrAgreementTime(
			String companyId, String employeeId, List<YearMonth> ymList, ScheRecAtr scheRecAtr, Closure closure,
		...
		// 最新の締め状態管理を取得
		Optional<ClosureStatusManagement> closureStatusMngOpt =
				this.repositories.getClosureStatusMng().getLatestByEmpId(employeeId);
		...
		if (loadYmList.size() > 0) {
			List<AgreementTimeOfManagePeriod> agreementTimeList =
					this.agreementTimeOfMngPeriodRepo.findBySidsAndYearMonths(employeeIds, loadYmList);
		...
		for (YearMonth procYm : ymList) {
			...
			if (procYm.lessThanOrEqualTo(latestYm)) {
				...
				// 集計に必要な日別実績データを取得する
				MonthlyCalculatingDailys monthlyCalcDailys = MonthlyCalculatingDailys.loadDataForAgreement(
						employeeId, aggrPeriod.getPeriod(), attendanceTimeOfDailys, this.repositories);
