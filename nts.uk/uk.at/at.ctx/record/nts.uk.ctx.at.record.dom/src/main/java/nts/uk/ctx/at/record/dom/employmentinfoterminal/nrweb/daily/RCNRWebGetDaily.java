 ...
 * @author thanh_nx
 *
 *         勤怠項目と社員IDと期間から日別実績の値を取得
 */
public class RCNRWebGetDaily {
	...
	private RCNRWebGetDaily() {
	};

	public static List<RCNRWebDailyData> getRecord(Require require, String cid, String employeeId, DatePeriod period,
			List<Integer> attendanceId) {
		// 社員IDと期間から「勤務予定」を取得
		List<IntegrationOfDaily> lstDomainDaily = require.get(employeeId, period);

		// DailyRecordToAttendanceItemConverterにセットする
		Map<Pair<String, GeneralDate>, List<ItemValue>> mapDataRecord = new HashMap<>();
		...
		lstDomainDaily.stream().forEach(domainDaily -> {
			...
			mapDataRecord.put(Pair.of(domainDaily.getEmployeeId(), domainDaily.getYmd()),
		...
		// 勤怠項目のマスタの取得
		List<AllMasterAttItem> allMasterName = require.getAllMaster(cid, attendanceId, period.end());
		...
		// 勤怠項目IDと値を作る
		List<RCNRWebDailyData> lstResultData = mapDataRecord.entrySet().stream().map(x -> {
			IntegrationOfDaily daily = mapDomainDaily.get(x.getKey());
			// daily.getEditState().stream().filter(x -> x.get)
			List<RCAttendanceItemAndValue> value = x.getValue().stream().map(itemDetail -> {
				...
				return new RCAttendanceItemAndValue(itemName.getAttendanceItemDisplayNumber(), itemDetail.getItemId(), // id
			...
			return new RCNRWebDailyData(x.getKey().getLeft(), x.getKey().getRight(), value);
		}).collect(Collectors.toList());

		return lstResultData;
	}

	private static String getName(List<AllMasterAttItem> allMasterName, Optional<DailyAttendanceItem> type,
		...
		List<MasterAttItemDetail> lstDetail = allMasterName.stream()
