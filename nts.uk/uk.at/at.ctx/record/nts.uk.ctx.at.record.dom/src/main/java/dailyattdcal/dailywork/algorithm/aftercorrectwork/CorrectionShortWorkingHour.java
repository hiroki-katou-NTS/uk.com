...
import nts.uk.ctx.at.record.dom.dailyperformanceprocessing.repository.ReflectShortWorkingTimeDomainServiceimpl;
...
public class CorrectionShortWorkingHour {
	...
	public IntegrationOfDaily correct(String companyId, IntegrationOfDaily domainDaily) {
		...
		String sid = domainDaily.getWorkInformation().getEmployeeId();
		GeneralDate date = domainDaily.getWorkInformation().getYmd();
		// 短時間勤務時間帯を反映するか確認する
		ConfirmReflectWorkingTimeOuput rShotWT = new ReflectShortWorkingTimeDomainServiceimpl()
				.confirmReflectWorkingTime(UUID.randomUUID().toString(), companyId, date, sid,
