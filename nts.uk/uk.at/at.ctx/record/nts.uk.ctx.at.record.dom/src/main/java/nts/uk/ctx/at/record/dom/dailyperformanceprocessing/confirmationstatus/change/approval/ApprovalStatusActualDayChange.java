...
public class ApprovalStatusActualDayChange {
	...
	public List<ApprovalStatusActualResult> processApprovalStatus(String companyId, String empTarget, List<String> employeeIds,
			Optional<DatePeriod> periodOpt, Optional<YearMonth> yearMonthOpt, int mode) {
		...
		ApprovalInfoResult approvalInfoResultAll = approvalInfoAcqProcess.getApprovalInfoAcp(companyId, empTarget, employeeIds, periodOpt, yearMonthOpt);
		if(approvalInfoResultAll == null) return Collections.emptyList(); 
		// ドメインモデル「本人確認処理の利用設定」を取得する
		Optional<IdentityProcessUseSet> optIndentity = iFindDataDCRecord.findIdentityByKey(companyId);
		return processApprovalStatusCommon(companyId, empTarget, employeeIds, approvalInfoResultAll, optIndentity, optApprovalUse, mode);
	}
	
	public List<ApprovalStatusActualResult> processApprovalStatusCommon(String companyId, String tartgetEmp, List<String> employeeIds,
			ApprovalInfoResult approvalInfoResultAll, Optional<IdentityProcessUseSet> optIndentity, Optional<ApprovalProcessingUseSetting> approvalUseSettingOpt, int mode) {
		List<ApprovalStatusActualResult> resultResult = new ArrayList<>();
		Optional<ApprovalProcessingUseSetting> optApprovalUse = approvalUseSettingOpt.isPresent() ? approvalUseSettingOpt : iFindDataDCRecord.findApprovalByCompanyId(companyId);
		// 社員の指定期間中の所属期間を取得する
				List<StatusOfEmployeeExport> statusOfEmps = approvalInfoResultAll.getStatusOfEmps();
				...
				statusOfEmps.stream().forEach(statusOfEmp -> {
					...
					ApprovalInfoResult approvalInfoResult = approvalInfoResultAll.getDataWithEmp(statusOfEmp.getEmployeeId());
...
//										Arrays.asList(employeeId), 1);
						List<ApproveRootStatusForEmpImport> lstApprovalStatus = approvalInfoResult.getLstApprovalDayStatus();
						//対応するImported「基準社員の承認対象者」を取得する
						List<ApprovalRootOfEmployeeImport> lstApprovalRootDay = approvalInfoResult.getLstApprovalRootDaily();
						Map<Pair<String, GeneralDate>, Pair<ApprovalStatus, ApproverEmployeeState>> mapApprovalRoot = lstApprovalRootDay.stream()
								.flatMap(x -> x.getApprovalRootSituations().stream())
						...
						if (mode == ModeData.NORMAL.value) {
							...
							if(!lstApprovalRootDay.isEmpty()) {
								lstApprovalRootDay.stream().flatMap(x -> x.getApprovalRootSituations().stream()).filter(x -> x.getTargetID().equals(employeeId)).forEach(x ->{
						...
						Optional<IdentityProcessUseSet> identitySetOpt = optIndentity.isPresent() ? optIndentity : iFindDataDCRecord.findIdentityByKey(companyId);
						if (!identitySetOpt.isPresent() || !identitySetOpt.get().isUseConfirmByYourself()) {
							...
							List<Identification> lstIdentity = approvalInfoResult.getIndentities();
...
//						List<ClosurePeriod> lstMergePeriod = confirmStatusActualDay.mergePeriod(closureStartOpt, closureEndOpt);
						for (AggrPeriodEachActualClosure mergePeriodClr : approvalInfoResult.getAggrPeriods()) {
							...
							// 対応するImported「（就業．勤務実績）承認対象者の承認状況」をすべて取得する
							List<ApproveRootStatusForEmpImport> lstApprovalStatus = approvalInfoResult.getLstApprovalMonthStatus();
