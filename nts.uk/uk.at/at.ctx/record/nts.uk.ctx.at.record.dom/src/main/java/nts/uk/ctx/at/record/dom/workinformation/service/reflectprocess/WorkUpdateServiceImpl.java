...
public class WorkUpdateServiceImpl implements ScheWorkUpdateService {
		...
		lstItem.stream().forEach(z -> {
			...
			if (optItemData.isPresent()) {
				...
				EditStateOfDailyPerformance data = new EditStateOfDailyPerformance(itemData.getEmployeeId(),
						itemData.getAttendanceItemId(), itemData.getYmd(), EditStateSetting.REFLECT_APPLICATION);
				lstDaily.add(data);
			} else {
				EditStateOfDailyPerformance insertData = new EditStateOfDailyPerformance(employeeId, z, dateData,
	...
	public void updateStartTimeOfReflect(TimeReflectParameter para) {
		...
		// 日別実績の勤務情報
		Optional<WorkInfoOfDailyPerformance> optDailyPerfor = workRepository.find(para.getEmployeeId(),
				para.getDateData());
		...
		if (para.isPreCheck()) {
			timeSheet = new ScheduleTimeSheet(timeSheetFrameNo.getWorkNo().v(), para.getTime(),
					timeSheetFrameNo.getLeaveWork().v());
		} else {
			timeSheet = new ScheduleTimeSheet(timeSheetFrameNo.getWorkNo().v(), timeSheetFrameNo.getAttendance().v(),
					para.getTime());
		}
	...
	public void updateReflectStartEndTime(TimeReflectParameter para) {
		// 開始時刻を反映する
		Optional<TimeLeavingOfDailyPerformance> optTimeLeavingOfDaily = timeLeavingOfDaily
				.findByKey(para.getEmployeeId(), para.getDateData());
		...
		WorkStamp stampTmp = new WorkStamp(stamp.getAfterRoundingTime(), new TimeWithDayAttr(para.getTime()),
				stamp.getLocationCode().isPresent() ? stamp.getLocationCode().get() : null, stamp.getStampSourceInfo());
		TimeActualStamp timeActualStam = new TimeActualStamp(
				timeAttendance.getActualStamp().isPresent() ? timeAttendance.getActualStamp().get() : null, stampTmp,
				timeAttendance.getNumberOfReflectionStamp());
		TimeLeavingWork timeLeavingWorkTmp;
		if (para.isPreCheck()) {
			timeLeavingWorkTmp = new TimeLeavingWork(timeLeavingWork.getWorkNo(), timeActualStam,
					(timeLeavingWork.getLeaveStamp().isPresent() && timeLeavingWork.getLeaveStamp() != null)
							? timeLeavingWork.getLeaveStamp().get() : null);
		} else {
			timeLeavingWorkTmp = new TimeLeavingWork(timeLeavingWork.getWorkNo(),
					(timeLeavingWork.getAttendanceStamp().isPresent() && timeLeavingWork.getAttendanceStamp() != null)
		...
		TimeLeavingOfDailyPerformance tmpData = new TimeLeavingOfDailyPerformance(para.getEmployeeId(),
				timeLeavingOfDailyData.getWorkTimes(), lstTimeLeavingWorksTmp, para.getDateData());
	...
	public void updateTimeShiftNight(String employeeId, GeneralDate dateData, Integer timeNight, boolean isPre) {
		...
		ExcessOfStatutoryMidNightTime tmp = new ExcessOfStatutoryMidNightTime(exMidNightTime.getTime(),
				new AttendanceTime(timeNight));
		if (isPre) {
	...
	public void updateRecordWorkType(String employeeId, GeneralDate dateData, String workTypeCode, boolean scheUpdate) {
		...
		if (scheUpdate) {
			lstItem.add(1);
			dailyPerfor.setScheduleInfo(new WorkInformation(dailyPerfor.getScheduleInfo().getWorkTimeCode() == null
					? null : dailyPerfor.getScheduleInfo().getWorkTimeCode().v(), workTypeCode));
			workRepository.updateByKeyFlush(dailyPerfor);
		} else {
			lstItem.add(28);
			dailyPerfor.setRecordInfo(new WorkInformation(dailyPerfor.getRecordInfo().getWorkTimeCode() == null ? null
					: dailyPerfor.getScheduleInfo().getWorkTimeCode().v(), workTypeCode));
			workRepository.updateByKeyFlush(dailyPerfor);
		}
		// 日別実績の編集状態
		this.updateEditStateOfDailyPerformance(employeeId, dateData, lstItem);
