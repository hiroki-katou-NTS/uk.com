...
public class CheckApprovalDayComplete {
	...
	public Optional<ApprovalDayComplete> checkApprovalDayComplete(String employeeId, DatePeriod date){
		...
		List<GeneralDate> dates = appRoots.stream().filter(x -> x.getApprovalStatus().value == ApprovalStatusForEmployee.UNAPPROVED.value).map(x -> x.getAppDate()).collect(Collectors.toList());
