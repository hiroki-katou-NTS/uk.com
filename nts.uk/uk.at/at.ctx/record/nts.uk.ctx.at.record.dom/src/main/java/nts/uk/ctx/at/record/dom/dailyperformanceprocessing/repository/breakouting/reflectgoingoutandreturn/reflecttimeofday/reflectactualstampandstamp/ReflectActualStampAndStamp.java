package nts.uk.ctx.at.record.dom.dailyperformanceprocessing.repository.breakouting.reflectgoingoutandreturn.reflecttimeofday.reflectactualstampandstamp;
...
public class ReflectActualStampAndStamp {
	...
	public void reflect(TimeActualStamp timeActualStamp,Stamp stamp,boolean isReflectTimeStamp,TimeFrame timeFrame,GeneralDate ymd,WorkTimeCode workTimeCode) {
		...
		if(!(!isReflectTimeStamp && timeActualStamp.getActualStamp().isPresent())) {
			...
			timeStampCopy.getActualStamp().get().setTimeDay(timeDay);
			timeStampCopy.getActualStamp().get().setAfterRoundingTime(timeWithDayAttr);
			timeStampCopy.getActualStamp().get().setLocationCode(stamp.getRefActualResults().getWorkLocationCD());
