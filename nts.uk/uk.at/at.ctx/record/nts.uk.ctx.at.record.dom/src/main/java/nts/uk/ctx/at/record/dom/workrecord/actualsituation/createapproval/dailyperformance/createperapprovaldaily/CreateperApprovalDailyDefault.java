...
public class CreateperApprovalDailyDefault implements CreateperApprovalDailyService {
	...
	public boolean createperApprovalDaily(String companyId, String executionId, List<String> employeeIDs,
		...
		if(processExecType ==0) {
			//RequestList211
			List<AffCompanyHistImport> listAffCompanyHistImport = syCompanyRecordAdapter.getAffCompanyHistByEmployee(employeeIDs, new DatePeriod(startDateClosure, GeneralDate.today()));
			...
			for(String employeeID : employeeIDs) {
				if(createNewEmp ==1) {
					...
					if(flagError) {
						...
						AppDataInfoDaily appDataInfoDaily = new AppDataInfoDaily(employeeID, executionId, new ErrorMessageRC(TextResource.localize(errorMessage)));
