...
@Stateless
public class UpdateSpLea1InfoCommandHandler extends CommandHandler<UpdateSpecialleave1informationCommand>
implements PeregUpdateCommandHandler<UpdateSpecialleave1informationCommand>{
	...
	@Override
	protected void handle(CommandHandlerContext<UpdateSpecialleave1informationCommand> context) {
		
		val command = context.getCommand();
		
		String cid = AppContexts.user().companyId();
		SpecialLeaveBasicInfo domain = new SpecialLeaveBasicInfo(cid,command.getSID(),SpecialLeaveCode.CS00025.value, command.getUseAtr().intValue(), command.getAppSet().intValue(), command.getGrantDate(),
