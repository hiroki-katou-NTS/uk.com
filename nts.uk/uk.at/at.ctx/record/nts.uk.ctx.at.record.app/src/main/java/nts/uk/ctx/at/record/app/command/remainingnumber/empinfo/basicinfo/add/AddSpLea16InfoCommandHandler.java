...
@Stateless
public class AddSpLea16InfoCommandHandler extends CommandHandlerWithResult<AddSpecialleave16informationCommand, PeregAddCommandResult>
implements PeregAddCommandHandler<AddSpecialleave16informationCommand>{
	...
	@Override
	protected PeregAddCommandResult handle(CommandHandlerContext<AddSpecialleave16informationCommand> context) {
		
		val command = context.getCommand();
		
		String cid = AppContexts.user().companyId();
		
		SpecialLeaveBasicInfo domain = new SpecialLeaveBasicInfo(cid, command.getSID(), SpecialLeaveCode.CS00054.value, command.getUseAtr().intValue(), command.getAppSet().intValue(), command.getGrantDate(),
