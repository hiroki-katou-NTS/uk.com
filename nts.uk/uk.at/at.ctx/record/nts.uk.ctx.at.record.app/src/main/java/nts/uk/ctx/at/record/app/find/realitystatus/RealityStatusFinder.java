...
public class RealityStatusFinder {
	...
	public List<RealityStatusActivityDto> getStatusActivity(RealityStatusActivityData wkpInfoDto) {
		...
		for (String wkpId : listWorkplaceId) {
			...
			List<ApprovalStatusEmployeeImport> listStatusEmp = approvalStatusAdapter.getApprovalStatusEmployee(wkpId, startDate,
					endDate, wkpInfoDto.getListEmpCd());
			
			// アルゴリズム「承認状況取得職場実績確認」を実行する
			SumCountOutput count = realityStatusService.getApprovalSttConfirmWkpResults(listStatusEmp, wkpId, useSeting);
			// 保持内容．未確認データ確認
