...
@Stateless
public class AddSpLea8InfoCommandHandler extends CommandHandlerWithResult<AddSpecialleave8informationCommand, PeregAddCommandResult>
implements PeregAddCommandHandler<AddSpecialleave8informationCommand>{
	...
	protected PeregAddCommandResult handle(CommandHandlerContext<AddSpecialleave8informationCommand> context) {
		...
		SpecialLeaveBasicInfo domain = new SpecialLeaveBasicInfo(cid, command.getSID(), SpecialLeaveCode.CS00032.value, command.getUseAtr().intValue(), command.getAppSet().intValue(), command.getGrantDate(),
