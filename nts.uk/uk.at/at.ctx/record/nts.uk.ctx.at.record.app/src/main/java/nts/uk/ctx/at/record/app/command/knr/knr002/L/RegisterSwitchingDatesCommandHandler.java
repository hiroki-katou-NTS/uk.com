 ...
 * @author dungbn
 *  切替日の一括登録する
 *	UKDesign.ドメインモデル.NittsuSystem.UniversalK.就業.contexts.勤務実績.就業情報端末.端末情報.APP.切替日の一括登録する.切替日の一括登録する
 */
@Stateless
public class RegisterSwitchingDatesCommandHandler extends CommandHandler<RegisterSwitchingDatesCommand> {
	...
	protected void handle(CommandHandlerContext<RegisterSwitchingDatesCommand> context) {
		...
		if (command.getTimeSwitchUKMode().equals("")) {
			this.timeRecordReqSettingRepository.updateSwitchDates(new ContractCode(contractCode), null);
		} else {
			GeneralDateTime datetime 			  = GeneralDateTime.fromString(command.getTimeSwitchUKMode(), datetimeFormat);
			this.timeRecordReqSettingRepository.updateSwitchDates(new ContractCode(contractCode), datetime);
		}
	}
}
