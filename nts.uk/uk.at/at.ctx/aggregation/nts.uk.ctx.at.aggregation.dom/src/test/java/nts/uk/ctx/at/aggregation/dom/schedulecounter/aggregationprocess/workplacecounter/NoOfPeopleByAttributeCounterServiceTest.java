...
public class NoOfPeopleByAttributeCounterServiceTest {
	...
	public void countingEachEmployments() {
		...
		val noEmpByEmplCd = new HashMap<EmploymentCode, BigDecimal>() {
			private static final long serialVersionUID = 1L;
			{
				put(new EmploymentCode("empl_01"), new BigDecimal("10"));
				put(new EmploymentCode("empl_02"), new BigDecimal("20"));
			}
		};
		...
		new MockUp<NumberOfEmployeesByAttributeCountingService>() {
			@Mock
			public Map<String, BigDecimal> count(Require require, AggregationUnitOfEmployeeAttribute unit, List<WorkInfoWithAffiliationInfo> values) {
				return unit == AggregationUnitOfEmployeeAttribute.EMPLOYMENT
						? noEmpByEmplCd.entrySet().stream().collect(Collectors.toMap( c -> c.getKey().v(), Map.Entry::getValue ))
		...
		Map<EmploymentCode, BigDecimal> value2 = result.get(GeneralDate.ymd(2021, 1, 2));
	...
	public void countingEachClassification() {
		...
		val noEmpByClsCd = new HashMap<ClassificationCode, BigDecimal>() {
			private static final long serialVersionUID = 1L;
			{
				put(new ClassificationCode("cls_01"), new BigDecimal("10"));
				put(new ClassificationCode("cls_02"), new BigDecimal("20"));
			}
		};
		...
		new MockUp<NumberOfEmployeesByAttributeCountingService>() {
			@Mock
			public Map<String, BigDecimal> count(Require require, AggregationUnitOfEmployeeAttribute unit, List<WorkInfoWithAffiliationInfo> values) {
				return unit == AggregationUnitOfEmployeeAttribute.CLASSIFICATION
						? noEmpByClsCd.entrySet().stream().collect(Collectors.toMap( c -> c.getKey().v(), Map.Entry::getValue ))
						: Collections.emptyMap();
			}
		};
	...
	public void countingEachJobTitle() {
		...
		val noByJobTilte = new HashMap<String, BigDecimal>() {
			private static final long serialVersionUID = 1L;
			{
				put("jobTitle_01", new BigDecimal("10"));
	...
	public void countingEachAttribute() {
		...
		val noEmpByJobTilte = new HashMap<String, BigDecimal>() {
			private static final long serialVersionUID = 1L;
			{
				put("jobTitle_01", new BigDecimal("10"));
				put("jobTitle_02", new BigDecimal("20"));
		...
		new MockUp<NumberOfEmployeesByAttributeCountingService>() {
			...
			public Map<String, BigDecimal> count(Require require, AggregationUnitOfEmployeeAttribute unit, List<WorkInfoWithAffiliationInfo> values) {
				switch( unit ) {
					case CLASSIFICATION:	return noEmpByClsCd.entrySet().stream().collect(Collectors.toMap( c -> c.getKey().v(), Map.Entry::getValue ));
					case EMPLOYMENT:		return noEmpByEmplCd.entrySet().stream().collect(Collectors.toMap( c -> c.getKey().v(), Map.Entry::getValue ));
					case JOB_TITLE:			return noEmpByJobTilte;
				}
				return Collections.emptyMap();
			}
		};
		...
		assertThat(value1.entrySet())
				.extracting( Map.Entry::getKey, Map.Entry::getValue )
				.containsExactlyInAnyOrder(
						tuple(new ClassificationCode("cls_01").v(), new BigDecimal("10"))
					,	tuple(new ClassificationCode("cls_02").v(), new BigDecimal("20"))
				);
		...
		assertThat(value1.entrySet())
				.extracting( Map.Entry::getKey, Map.Entry::getValue )
				.containsExactlyInAnyOrder(
						tuple(new EmploymentCode("empl_01").v(), new BigDecimal("10"))
					,	tuple(new EmploymentCode("empl_02").v(), new BigDecimal("20"))
				);
