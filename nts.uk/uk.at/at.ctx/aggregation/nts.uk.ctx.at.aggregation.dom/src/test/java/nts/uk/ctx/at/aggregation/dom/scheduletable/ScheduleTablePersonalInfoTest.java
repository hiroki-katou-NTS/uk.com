...
public class ScheduleTablePersonalInfoTest {
	...
	public void testCreate_onlyEmployeeName() {
		...
		EmpLicenseClassification empLicenseClass = EmpLicenseClassification.createEmpLicenseClassification("employee-id");
	...
	public void testCreate_all() {
		...
		EmpLicenseClassification empLicenseClass = EmpLicenseClassification.createEmpLicenseClassification("employee-id", Hepler.createNurseClass(LicenseClassification.NURSE, false));
	...
	public static class Hepler {
		 ...
		 * 看護区分(dummy)を作成する
		 * @param license 免許区分
		 * @param isManager 看護管理者か
		 * @return 看護区分(dummy)
		 */
		public static NurseClassification createNurseClass(LicenseClassification license, boolean isManager) {
			return NurseClassification.create(new CompanyId("company-id")
						,	new NurseClassifiCode(String.format("%02d", license.value))
						,	new NurseClassifiName(license.name())
						,	license
						,	false
						,	isManager
					);
