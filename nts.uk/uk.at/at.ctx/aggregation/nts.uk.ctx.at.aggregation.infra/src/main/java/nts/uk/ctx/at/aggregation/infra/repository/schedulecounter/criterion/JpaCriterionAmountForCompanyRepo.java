...
import nts.uk.ctx.at.aggregation.infra.entity.schedulecounter.criterion.KagmtCriterionAmountForCompany;
 ...
 */
@Stateless
public class JpaCriterionAmountForCompanyRepo extends JpaRepository implements CriterionAmountForCompanyRepository {

	private static final String GET_BY_CID = "SELECT a FROM KagmtCriterionAmountForCompany a WHERE a.pk.companyId = :companyId ";

	@Override
	public void insert(String cid, CriterionAmountForCompany criterion) {
		this.commandProxy().insertAll(KagmtCriterionAmountForCompany.toEntity(cid, criterion));

	}

	@Override
	public void update(String cid, CriterionAmountForCompany criterion) {
		List<KagmtCriterionAmountForCompany> dataOld = this.queryProxy()
				.query(GET_BY_CID, KagmtCriterionAmountForCompany.class).setParameter("companyId", cid).getList();
		List<KagmtCriterionAmountForCompany> dataNew = KagmtCriterionAmountForCompany.toEntity(cid, criterion);

		for (KagmtCriterionAmountForCompany temp : dataOld) {
	...
	public Optional<CriterionAmountForCompany> get(String cid) {
		List<KagmtCriterionAmountForCompany> result = this.queryProxy()
				.query(GET_BY_CID, KagmtCriterionAmountForCompany.class).setParameter("companyId", cid).getList();

		if (result.isEmpty()) {
			return Optional.empty();
		}
		return Optional.of(KagmtCriterionAmountForCompany.toDomain(result));
	}
