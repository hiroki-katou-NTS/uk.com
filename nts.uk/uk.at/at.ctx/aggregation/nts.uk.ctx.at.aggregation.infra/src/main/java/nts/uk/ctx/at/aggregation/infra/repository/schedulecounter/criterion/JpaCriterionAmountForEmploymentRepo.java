...
import nts.uk.ctx.at.aggregation.infra.entity.schedulecounter.criterion.KagmtCriterionAmountForEmployment;
 ...
 */
@Stateless
public class JpaCriterionAmountForEmploymentRepo extends JpaRepository
		implements CriterionAmountForEmploymentRepository {

	private static final String GET_BY_CID = "SELECT a FROM KagmtCriterionAmountForEmployment a WHERE a.pk.companyId = :companyId ";
	...
	public void insert(String cid, CriterionAmountForEmployment criterion) {
		this.commandProxy().insertAll(KagmtCriterionAmountForEmployment.toEntity(cid, criterion));

	}

	@Override
	public void update(String cid, CriterionAmountForEmployment criterion) {
		List<KagmtCriterionAmountForEmployment> dataOld = this.queryProxy()
				.query(GET_BY_CID, KagmtCriterionAmountForEmployment.class).setParameter("companyId", cid).getList();
		List<KagmtCriterionAmountForEmployment> dataNew = KagmtCriterionAmountForEmployment.toEntity(cid, criterion);

		for (KagmtCriterionAmountForEmployment temp : dataOld) {
	...
	public void delete(String cid, EmploymentCode employmentCd) {
		List<KagmtCriterionAmountForEmployment> result = this.queryProxy()
				.query(GET_BY_CID_AND_CODE, KagmtCriterionAmountForEmployment.class).setParameter("companyId", cid)
	...
	public Optional<CriterionAmountForEmployment> get(String cid, EmploymentCode employmentCd) {
		List<KagmtCriterionAmountForEmployment> result = this.queryProxy()
				.query(GET_BY_CID_AND_CODE, KagmtCriterionAmountForEmployment.class)
				.setParameter("companyId", cid)
				.setParameter("employmentCode", employmentCd.v()).getList();
		if (result.isEmpty()) {
			return Optional.empty();
		}
		return Optional.of(KagmtCriterionAmountForEmployment.toDomain(employmentCd.v(), result));
	}
	...
	public List<CriterionAmountForEmployment> getAll(String cid) {
		...
		List<KagmtCriterionAmountForEmployment> result = this.queryProxy()
				.query(GET_BY_CID, KagmtCriterionAmountForEmployment.class)
				.setParameter("companyId", cid).getList();
		Map<String, List<KagmtCriterionAmountForEmployment>> mapData = result.stream()
				.collect(Collectors.groupingBy(c -> c.pk.employmentCode));
		mapData.forEach((k, v) -> {
			datas.add(KagmtCriterionAmountForEmployment.toDomain(k, v));
		});
		return datas;
	}
