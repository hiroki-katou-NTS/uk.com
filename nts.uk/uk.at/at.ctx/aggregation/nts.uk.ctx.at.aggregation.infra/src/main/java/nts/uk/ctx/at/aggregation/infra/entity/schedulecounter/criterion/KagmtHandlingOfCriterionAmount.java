...
public class KagmtHandlingOfCriterionAmount extends ContractUkJpaEntity implements Serializable {
	private static final long serialVersionUID = 1L;

	@EmbeddedId
	public KagmtHandlingOfCriterionAmountPK pk;
	...
	public static List<KagmtHandlingOfCriterionAmount> toEntity(String cid, HandlingOfCriterionAmount domain) {
		return domain.getList().stream()
				.map(c -> new KagmtHandlingOfCriterionAmount(
						new KagmtHandlingOfCriterionAmountPK(cid, c.getFrameNo().v()), c.getBackgroundColor().v()))
				.collect(Collectors.toList());
	}

	public static HandlingOfCriterionAmount toDomain(List<KagmtHandlingOfCriterionAmount> entitys) {
		List<HandlingOfCriterionAmountByNo> list = new ArrayList<>();
		for (KagmtHandlingOfCriterionAmount temp : entitys) {
