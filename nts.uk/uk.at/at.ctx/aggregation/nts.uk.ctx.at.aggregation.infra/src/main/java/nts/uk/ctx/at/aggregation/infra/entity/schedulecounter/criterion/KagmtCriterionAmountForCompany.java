...
public class KagmtCriterionAmountForCompany extends ContractUkJpaEntity implements Serializable {

	private static final long serialVersionUID = 1L;

	@EmbeddedId
	public KagmtCriterionAmountForCompanyPK pk;
	...
	public static List<KagmtCriterionAmountForCompany> toEntity(String companyId, CriterionAmountForCompany domain) {
		List<KagmtCriterionAmountForCompany> result = new ArrayList<>();
		// 0：年間目安金額
		for (CriterionAmountByNo temp : domain.getCriterionAmount().getYearly().getList()) {
			result.add(new KagmtCriterionAmountForCompany(
					new KagmtCriterionAmountForCompanyPK(companyId, 0, temp.getFrameNo().v()), temp.getAmount().v()));
		}
		// 1：月間目安金額
		for (CriterionAmountByNo temp : domain.getCriterionAmount().getMonthly().getList()) {
			result.add(new KagmtCriterionAmountForCompany(
					new KagmtCriterionAmountForCompanyPK(companyId, 1, temp.getFrameNo().v()), temp.getAmount().v()));
		}
		return result;
	}

	public static CriterionAmountForCompany toDomain(List<KagmtCriterionAmountForCompany> entitys) {
		List<KagmtCriterionAmountForCompany> listYearly = new ArrayList<>();
		List<KagmtCriterionAmountForCompany> listMonthly = new ArrayList<>();
		for (KagmtCriterionAmountForCompany ent : entitys) {
			// 0：年間目安金額
