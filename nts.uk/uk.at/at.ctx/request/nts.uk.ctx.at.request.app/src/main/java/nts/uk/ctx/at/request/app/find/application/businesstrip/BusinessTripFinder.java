...
public class BusinessTripFinder {
	...
	public DetailStartScreenInfoDto initKAF008(ParamStart paramStart) {
		...
		List<GeneralDate> dateList = paramStart.getDateLst().stream().map(i -> GeneralDate.fromString(i, "yyyy/MM/dd"))
		...
		BusinessTripInfoOutputDto businessTripInfoOutputDto = this.businessScreenInit_New(cid,
				paramStart.getApplicantList(), dateList, appDispInfoStartupOutput);
		// 申請対象日リスト全ての日付に対し「表示する実績内容」が存在する
		// Check xem có ngày nào không có content không, nếu có add Error msg 1695 +
		// date
		List<ConfirmMsgOutput> listErrors = this
				.checkDateWithContent(businessTripInfoOutputDto.getBusinessTripActualContent());
...
//        return dummy;
	...
	// create and start screen A on mobile
	...
	private List<ConfirmMsgOutput> checkDateWithContent(List<BusinessTripActualContentDto> contents) {
		...
		if (!contents.isEmpty()) {
			List<BusinessTripActualContentDto> itemNotHaveConent = contents.stream()
					.filter(i -> i.getOpAchievementDetail() == null).collect(Collectors.toList());
			if (!itemNotHaveConent.isEmpty()) {
				ConfirmMsgOutput msg = new ConfirmMsgOutput();
				msg.setMsgID("Msg_1695");
				msg.setParamLst(
						itemNotHaveConent.stream().map(i -> i.getDate().toString()).collect(Collectors.toList()));
				result.add(msg);
			}
		}
		return result;
	 ...
	 */
	private BusinessTripInfoOutputDto businessScreenInit_New(String cid, List<String> applicantList,
			List<GeneralDate> appDate, AppDispInfoStartupOutput appDispInfoStartupOutput) {
		...
		Optional<AppEmploymentSet> opEmploymentSet = appDispInfoStartupOutput.getAppDispInfoWithDateOutput()
				.getOpEmploymentSet();
		// 出張申請勤務種類を取得する
		List<WorkType> workDays = businessTripService.getBusinessAppWorkType(opEmploymentSet.get(),
		...
		List<WorkType> holidayWorkType = businessTripService.getBusinessAppWorkType(opEmploymentSet.get(),
				EnumAdaptor.valueOf(BusinessTripAppWorkType.HOLIDAY.value, BusinessTripAppWorkType.class),
				new ArrayList<>(Arrays.asList(WorkTypeClassification.Holiday, WorkTypeClassification.HolidayWork,
						WorkTypeClassification.Shooting)));
		// アルゴリズム「出張申請未承認申請を取得」を実行する
		List<ActualContentDisplay> actualContentDisplays = businessTripService.getBusinessTripNotApproved(sid, appDate,
				appDispInfoStartupOutput.getAppDispInfoWithDateOutput().getOpActualContentDisplayLst());

		// Actual content get từ AppDispInfoStartupOutput đang default là optional empty
		Optional<List<ActualContentDisplay>> appActualContent = appDispInfoStartupOutput.getAppDispInfoWithDateOutput()
...
//        if (appActualContent.isPresent()) {
		if (!actualContentDisplays.isEmpty()) {
			List<String> cds = actualContentDisplays.stream().filter(i -> i.getOpAchievementDetail().isPresent())
					.map(i -> i.getOpAchievementDetail().get().getWorkTypeCD()).distinct().collect(Collectors.toList());
			// ドメインモデル「勤務種類」を取得する
			Map<String, WorkType> mapWorkCds = wkTypeRepo.getPossibleWorkType(cid, cds).stream()
					.collect(Collectors.toMap(i -> i.getWorkTypeCode().v(), i -> i));
			businessTripWorkTypes = actualContentDisplays.stream()
					.map(i -> new BusinessTripWorkTypes(i.getDate(),
							i.getOpAchievementDetail().isPresent()
									? mapWorkCds.get(i.getOpAchievementDetail().get().getWorkTypeCD())
									: null))
					.collect(Collectors.toList());
		}
		// 取得した情報をOUTPUT「出張申請の表示情報」にセットしてを返す
		BusinessTripInfoOutput output = new BusinessTripInfoOutput(
				tripRequestSet.isPresent() ? tripRequestSet.get() : null, appDispInfoStartupOutput,
				Optional.of(workDays), Optional.of(holidayWorkType), Optional.of(actualContentDisplays),
				Optional.of(businessTripWorkTypes), Optional.empty());
	 ...
	 * @return
	 */
	public List<ConfirmMsgOutput> checkBeforeRegister(ErrorFlagImport errorFlag, Application application,
			BusinessTrip businessTripApp, BusinessTripInfoOutput tripRequestInfoOutput) {
		String sid = application.getEmployeeID() == null ? AppContexts.user().employeeId()
		...
		DatePeriod period = new DatePeriod(appStartDate.get().getApplicationDate(),
				appEndDate.get().getApplicationDate());
		List<GeneralDate> lstDate = period.datesBetween();

		// アルゴリズム「2-1.新規画面登録前の処理」を実行する
		confirmMsgOutputs = processBeforeRegister.processBeforeRegister_New(AppContexts.user().companyId(),
				EmploymentRootAtr.APPLICATION, false, application, null, errorFlag, null,
		...
		if (confirmMsgOutputs.isEmpty()) {
			...
			businessTripApp.getInfos().stream().forEach(i -> {
				...
				// 申請の矛盾チェック
				this.commonAlgorithm.appConflictCheck(cid, employeeInfoImports.get(0), lstDate,
	 ...
	 * @return
	 */
	public DetailStartScreenInfoDto updateAppDate(BusinessTripInfoOutputDto businessTripInfoOutputDto,
		...
		Application application = Application.createFromNew(
				...
				new ApplicationDate(GeneralDate.fromString(applicationDto.getAppDate(), "yyyy/MM/dd")), loginSid,
				Optional.empty(), Optional.empty(),
				Optional.ofNullable(
						new ApplicationDate(GeneralDate.fromString(applicationDto.getOpAppStartDate(), "yyyy/MM/dd"))),
				Optional.ofNullable(
						new ApplicationDate(GeneralDate.fromString(applicationDto.getOpAppEndDate(), "yyyy/MM/dd"))),
		...
		DatePeriod dates = new DatePeriod(application.getOpAppStartDate().get().getApplicationDate(),
				application.getOpAppEndDate().get().getApplicationDate());
		List<GeneralDate> inputDates = dates.datesBetween();

		List<ActualContentDisplay> appActualContents = businessTripService.getBusinessTripNotApproved(loginSid,
				inputDates, appDispInfoStartupOutput.getAppDispInfoWithDateOutput().getOpActualContentDisplayLst());
...
//                    .distinct()
//                    .collect(Collectors.toList());
			List<String> cds = dummyActualContent.stream().map(i -> i.getOpAchievementDetail().getWorkTypeCD())
...
//            )).collect(Collectors.toList());
			businessTripWorkTypes = dummyActualContent.stream()
					.map(i -> new BusinessTripWorkTypes(GeneralDate.fromString(i.getDate(), "yyyy/MM/dd"),
							i.getOpAchievementDetail().getWorkTypeCD() != null
									? mapWorkCds.get(i.getOpAchievementDetail().getWorkTypeCD())
									: null))
					.collect(Collectors.toList());
		}
	...
	public BusinessTripInfoOutputDto changeWorkTypeCode(ChangeWorkCodeParam changeWorkCodeParam) {
		...
		Optional<BusinessTripWorkTypes> currentDateWorkType = businessTripInfoOutput.getWorkTypeBeforeChange().get()
				.stream().filter(i -> i.getDate().equals(inputDate)).findFirst();
		if (currentDateWorkType.isPresent()) {
			workTypesBeforeChange = getBusinessTripWorkChangeInfo(businessTripInfoOutput,
					currentDateWorkType.get().getWorkType());
		}
		// 取得した勤務種類リストの中に、INPUT.勤務種類コードが存在する
		Optional<WorkType> inputWorkType = workTypesBeforeChange.stream()
				.filter(i -> i.getWorkTypeCode().v().equals(inputCode)).findFirst();
	 ...
	 * @return
	 */
	private List<WorkType> getBusinessTripWorkChangeInfo(BusinessTripInfoOutput businessTripInfoOutput,
			WorkType workType) {
	...
	private boolean getBusinessTripClsContent(WorkType workType) {
		...
		List<WorkTypeClassification> workDays = Arrays.asList(WorkTypeClassification.Attendance, // 出勤
		...
		List<WorkTypeClassification> holidays = Arrays.asList(WorkTypeClassification.Holiday, // 休日
	...
	public WorkTypeNameDto changeWorkTimeCode(ChangeWorkCodeParam changeWorkCodeParam) {
		...
		val workTimeSet = businessTripInfoOutput.getAppDispInfoStartup().getAppDispInfoWithDateOutput()
				.getOpWorkTimeLst();
		if (workTimeSet.isPresent() && !workTimeSet.get().isEmpty()) {
			Optional<WorkTimeSetting> existWorkTimeSet = workTimeSet.get().stream()
					.filter(i -> i.getWorktimeCode().v().equals(timeCode)).findFirst();
	...
	public DetailScreenDto getDetailKAF008(ParamUpdate param) {
		...
		AppDispInfoStartupOutput appDispInfoStartupOutput = appCommonSetService
				.getCommonSetBeforeDetail(param.getCompanyId(), param.getApplicationId());
		DetailScreenB detailScreen = businessTripService.getDataDetail(param.getCompanyId(), param.getApplicationId(),
	...
	public boolean getFlagStartKDL003(ParamStartKDL003 param) {
		...
		WorkType selectedWorkType = infoOutput.getWorkTypeBeforeChange().get().stream()
				.filter(i -> i.getDate().equals(selectedDate)).findFirst().get().getWorkType();
