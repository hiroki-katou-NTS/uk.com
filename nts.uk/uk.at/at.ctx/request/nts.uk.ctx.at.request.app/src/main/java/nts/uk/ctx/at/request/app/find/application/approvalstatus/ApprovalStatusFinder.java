...
public class ApprovalStatusFinder {
	...
	public List<ApplicationDetailDto> initApprovalSttRequestContentDis(ApprovalSttRequestContentDis appSttContent) {
		...
		GeneralDate appStartDate = GeneralDate.today().addDays(5);
		GeneralDate appEndDate = GeneralDate.today().addDays(6);
		List<Integer> approvalStatus = new ArrayList<>();
		for (int x = 0; x < 3; x++) {
			approvalStatus.add(x);
		}
		listAppDetail.add(new ApplicationDetailDto(0, "残業申請", true, appStartDate, appEndDate,
				"2-16③システム日付(2018/4/7)　<　申請日.終了日(3/31)+8　　ＯＫ", 2, approvalStatus, "Phase1", "Phase1", "Phase1",
				"Phase1", "Phase1"));
		listAppDetail.add(new ApplicationDetailDto(1, "残業申請", false, appStartDate, appEndDate,
				"2-16③システム日付(2018/4/7)　<　申請日.終了日(3/31)+8　　ＯＫ", 4, approvalStatus, "Phase1", "Phase1", "Phase1",
				"Phase1", "Phase1"));
		listAppDetail.add(new ApplicationDetailDto(2, "残業申請", true, appStartDate, appEndDate,
				"2-16③システム日付(2018/4/7)　<　申請日.終了日(3/31)+8　　ＯＫ", 5, approvalStatus, "Phase1", "Phase1", "Phase1",
				"Phase1", "Phase1"));
		listAppDetail.add(new ApplicationDetailDto(3, "残業申請", false, appStartDate, appEndDate,
				"2-16③システム日付(2018/4/7)　<　申請日.終了日(3/31)+8　　ＯＫ", 0, approvalStatus, "Phase1", "Phase1", "Phase1",
				"Phase1", "Phase1"));
		listAppDetail.add(new ApplicationDetailDto(1, "残業申請", true, appStartDate, appEndDate,
				"2-16③システム日付(2018/4/7)　<　申請日.終了日(3/31)+8　　ＯＫ", 1, approvalStatus, "Phase1", "Phase1", "Phase1",
				"Phase1", "Phase1"));
		/*
		 * List<ApprovalSttAppDetail> listAppSttDetail =
		 * appList.getApprovalSttAppDetail(); for (ApprovalSttAppDetail app :
		 * listAppSttDetail) { ApplicationDetailDto detail = new
		 ...
		 * if(applicaton_N.isAppAbsence() || applicaton_N.isAppBusinessTrip() ||
		 * applicaton_N.isAppWkChange()) {
		 ...
		 * 
		 * List<ApprovalPhaseStateImport_New> listApprovalPhase=
		 * app.getAppContent().getApprRootContentExport().getApprovalRootState()
		 * .getListApprovalPhaseState();
		 * listApprovalPhase.sort((ApprovalPhaseStateImport_New x1,
		 * ApprovalPhaseStateImport_New x2) -> x1.getPhaseOrder() -
		 * x2.getPhaseOrder()); List<Integer> appStatus = new ArrayList<>();
		 * for(ApprovalPhaseStateImport_New appPhase: listApprovalPhase) {
		 * ApprovalBehaviorAtrImport_New appBehavior =
		 * appPhase.getApprovalAtr(); switch (appBehavior) { case UNAPPROVED:
		 * appStatus.add(0); break; case APPROVED: appStatus.add(1); break; case
		 * REMAND: appStatus.add(0); break; case DENIAL: appStatus.add(2);
		 * break; default: break; } } detail.setApprovalStatus(appStatus);
		 * List<ApproverOutput> listApprover = app.getListApprover(); for
		 * (ApproverOutput approver : listApprover) { int phase =
		 * approver.getPhase(); String others = approver.getEmpName() +
		 * I18NText.getText("KAF018_47", approver.getNumOfPeople().toString());
		 * switch (phase) { case 1: detail.setPhase1(others); break; case 2:
		 * detail.setPhase2(others); break; case 3: detail.setPhase3(others);
		 * break; case 4: detail.setPhase4(others); break; case 5:
		 * detail.setPhase5(others); break; default: break; } } String
		 * appContent = ""; switch (appType) { // 残業申請 case
		 * OVER_TIME_APPLICATION: appContent = getAppContentOverTime(companyID,
		 * appId); break; // 休暇申請 case ABSENCE_APPLICATION: appContent =
		 * getAbsenceApplication(applicaton_N, companyID, appId); break; //
		 * 勤務変更申請 case WORK_CHANGE_APPLICATION: appContent =
		 * getAppWorkChangeInfo(companyID, appId); // có endDate break; // 出張申請
		 * case BUSINESS_TRIP_APPLICATION: // TODO // có endDate break; //
		 * 直行直帰申請 case GO_RETURN_DIRECTLY_APPLICATION: appContent =
		 * getAppGoBackInfo(companyID, appId); break; // 休出時間申請 case
		 * BREAK_TIME_APPLICATION: appContent = getBreakTimeApp(applicaton_N,
		 * companyID, appId); break; // 打刻申請 case STAMP_APPLICATION: // TODO
		 * break; // 時間年休申請 case ANNUAL_HOLIDAY_APPLICATION:
		 * 
		 * break; // 遅刻早退取消申請 case EARLY_LEAVE_CANCEL_APPLICATION: // TODO
		 * break; // 振休振出申請 case COMPLEMENT_LEAVE_APPLICATION: //Not use anymore
		 * //appContent = getAppCompltLeaveInfo(applicaton_N, companyID, appId);
		 * // Đơn xin đồng bộ break; // 打刻申請（NR形式） case STAMP_NR_APPLICATION: //
		 * TODO break; // 連続出張申請 case LONG_BUSINESS_TRIP_APPLICATION: // TODO
		 * break; // 出張申請オフィスヘルパー case BUSINESS_TRIP_APPLICATION_OFFICE_HELPER:
		 * // TODO break; // ３６協定時間申請 case APPLICATION_36: // TODO break;
		 * 
		 * default: break; } detail.setAppContent(appContent); int reflectState
		 * = applicaton_N.getReflectionInformation().getStateReflectionReal().
	...
	private String getAbsenceApplication(HdAppSetDto hdAppSetDto, Application_New applicaton_N, String companyID, String appId) {
		String appContent = "";
