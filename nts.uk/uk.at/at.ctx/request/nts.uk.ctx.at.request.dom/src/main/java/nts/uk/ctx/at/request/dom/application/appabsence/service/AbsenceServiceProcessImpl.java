...
public class AbsenceServiceProcessImpl implements AbsenceServiceProcess {
	...
	public void checkLimitAbsencePlan(String cID, String sID, String workTypeCD, GeneralDate sDate, GeneralDate eDate,
		...
		List<PlanVacationRuleError> check = planVacationRuleExport.checkMaximumOfPlan(cID, sID, workTypeCD,
				new DatePeriod(sDate, eDate), lstDateIsHoliday);
		if (!check.isEmpty()) {
...
//            return this.holidayTypeChangeProcess(companyID, appAbsenceStartInfoOutput, displayHalfDayValue, alldayHalfDay, holidayType.get());
	...
	@Override
	public AbsenceCheckRegisterOutput checkBeforeRegister(String companyID,
			AppAbsenceStartInfoOutput appAbsenceStartInfoOutput, ApplyForLeave appAbsence, boolean agentAtr,
			boolean isEmptyLeaveList, boolean isEmptyPayoutList) {
		AbsenceCheckRegisterOutput result = new AbsenceCheckRegisterOutput();
		// 紐付けデータをチェックする
		this.checkLinkingData(
				appAbsenceStartInfoOutput.getWorkTypeLst().stream()
						.filter(x -> x.getWorkTypeCode().v()
								.equals(appAbsence.getReflectFreeTimeApp().getWorkInfo().getWorkTypeCode().v()))
						.findFirst().get(),
				...
				appAbsenceStartInfoOutput.getPayoutSubofHDManagements(), isEmptyLeaveList, isEmptyPayoutList);

		// 申請期間から休日の申請日を取得する
		List<GeneralDate> holidayDates = otherCommonAlgorithm.lstDateIsHoliday(appAbsence.getEmployeeID(),
				new DatePeriod(appAbsence.getOpAppStartDate().get().getApplicationDate(),
						appAbsence.getOpAppEndDate().get().getApplicationDate()),
				appAbsenceStartInfoOutput.getAppDispInfoStartupOutput().getAppDispInfoWithDateOutput()
		...
		List<GeneralDate> listDates = new DatePeriod(
				appAbsence.getApplication().getOpAppStartDate().get().getApplicationDate(),
		...
		detailBeforeUpdate.displayWorkingHourCheck(companyID,
				...
				appAbsence.getReflectFreeTimeApp().getWorkInfo().getWorkTimeCodeNotNull().isPresent()
						? appAbsence.getReflectFreeTimeApp().getWorkInfo().getWorkTimeCode().v()
						: null);

		// INPUT．休暇申請起動時の表示情報．申請表示情報．申請設定（基準日関係あり）．表示する実績内容」をチェックする
		Optional<List<ActualContentDisplay>> opActualContentDisplayLst = appAbsenceStartInfoOutput
				.getAppDispInfoStartupOutput().getAppDispInfoWithDateOutput().getOpActualContentDisplayLst();

		List<GeneralDate> listDatesCheck = listDates.stream().filter(x -> !holidayDates.contains(x))
		...
		if (opActualContentDisplayLst.isPresent() && !listDatesCheck.isEmpty()) {
			List<GeneralDate> listActualDates = opActualContentDisplayLst.get().stream()
			...
			for (GeneralDate checkDate : listDatesCheck) {
				if (!listActualDates.contains(checkDate)) {
					// その他場合
					throw new BusinessException("Msg_1715",
							empEmployeeAdapter.findByEmpId(appAbsence.getEmployeeID()).getEmployeeName(),
							checkDate.toString("yyyy/MM/dd"));
				}
			}
		} else {
			// その他場合
			throw new BusinessException("Msg_1715",
					empEmployeeAdapter.findByEmpId(appAbsence.getEmployeeID()).getEmployeeName(),
					listDatesCheck.get(0).toString("yyyy/MM/dd"));
		}
		...
		if (appAbsence.getReflectFreeTimeApp().getTimeDegestion().isPresent()) {
			...
			timeDigestionParam = new TimeDigestionParam(timeDigestApplication.getOvertime60H().v(),
					timeDigestApplication.getNursingTime().v(), timeDigestApplication.getChildTime().v(),
					timeDigestApplication.getTimeOff().v(), timeDigestApplication.getTimeAnnualLeave().v(), 0,
					new ArrayList<TimeLeaveApplicationDetailShare>());
		}
		result.setConfirmMsgLst(newBeforeRegister.processBeforeRegister_New(companyID, EmploymentRootAtr.APPLICATION,
				agentAtr, appAbsence, null,
				appAbsenceStartInfoOutput.getAppDispInfoStartupOutput().getAppDispInfoWithDateOutput()
						.getOpMsgErrorLst().orElse(Collections.emptyList()),
				holidayDates, appAbsenceStartInfoOutput.getAppDispInfoStartupOutput(),
				Arrays.asList(appAbsence.getReflectFreeTimeApp().getWorkInfo().getWorkTypeCode().v()),
				Optional.of(timeDigestionParam),
				appAbsence.getReflectFreeTimeApp().getWorkInfo().getWorkTimeCodeNotNull().map(WorkTimeCode::v), false));
		// 休暇申請登録時チェック処理
		result.getConfirmMsgLst().addAll(
				this.checkAbsenceWhenRegister(true, companyID, appAbsence, appAbsenceStartInfoOutput, holidayDates));
	...
	public CheckDispHolidayType checkDisplayAppHdType(String companyID, String sID, GeneralDate baseDate) {
		...
		try {
			...
			AnnualHolidaySetOutput annualHolidaySetOutput = AbsenceTenProcess.getSettingForAnnualHoliday(require,
					companyID);
			annualLeaveManagement = new AnualLeaveManagement(
					...
					EnumAdaptor.valueOf(annualHolidaySetOutput.isYearHolidayManagerFlg() ? 1 : 0,
							ManageDistinct.class));
		...
		try {
			boolean isYearlyReserve = AbsenceTenProcess.getSetForYearlyReserved(require, cache, companyID, sID,
					baseDate);
		...
		SubstituteLeaveManagement substituteLeaveManagement = new SubstituteLeaveManagement(TimeDigestiveUnit.OneMinute,
				ManageDistinct.NO, ManageDistinct.NO, ManageDistinct.NO);

		if (compensatoryLeaveComSetting != null) {
			try {
				SubstitutionHolidayOutput substituationHoliday = AbsenceTenProcess
						.getSettingForSubstituteHoliday(require, cache, companyID, sID, baseDate);
				substituteLeaveManagement = new SubstituteLeaveManagement(
						...
						substituationHoliday.isSubstitutionFlg() ? compensatoryLeaveComSetting.getLinkingManagementATR()
								: ManageDistinct.NO,
		...
		// 10-5.60H超休の設定を取得する
		Overtime60HManagement overtime60hManagement = new Overtime60HManagement(ManageDistinct.NO,
		...
		NursingCareLeaveManagement nursingCareLeaveManagement = new NursingCareLeaveManagement(ManageDistinct.NO,
				TimeDigestiveUnit.OneMinute, ManageDistinct.NO, TimeDigestiveUnit.OneMinute, ManageDistinct.NO,
				ManageDistinct.NO);
		try {
			// 子看護介護の設定の取得
			NursingLeaveSetting childNursingLeaveSetting = this.getNursingLeaveSetting(companyID,
			...
			nursingCareLeaveManagement = new NursingCareLeaveManagement(childNursingLeaveSetting.getManageType(),
		...
		return new CheckDispHolidayType(annualLeaveManagement, accumulatedRestManagement, substituteLeaveManagement,
				holidayManagement, overtime60hManagement, nursingCareLeaveManagement);
	}
	...
	private NursingLeaveSetting getNursingLeaveSetting(String companyID, NursingCategory nursingType) {
		...
		NursingLeaveSetting nursingLeaveSettings = nursingLeaveSettingRepo.findByCompanyIdAndNursingCategory(companyID,
				nursingType.value);
...
//	}
	 ...
	 */
	@Override
	public List<ConfirmMsgOutput> checkPriorityHoliday(AppliedDate pridigCheck, boolean isSubVacaManage,
			boolean subVacaTypeUseFlg, boolean isSubHdManage, boolean subHdTypeUseFlg, int numberSubHd,
			int numberSubVaca) {
		List<ConfirmMsgOutput> result = new ArrayList<>();
		...
		// それ以外
		// ログインしている会社をもとにドメインモデル「休暇の取得ルール」を取得する (lấy dữ liệu domain 「AcquisitionRule」
		// dựa vào công ty đang đăng nhập)
		...
		// 休暇の取得ルール．年休より優先する休暇．振休を優先＝ false OR 休暇申請対象勤務種類．休暇種類を利用しない（振休）＝ true OR
		// 振休管理設定．管理区分＝管理しない
...
//		条件の追加： 「休暇の取得ルール．取得する順番をチェックする＝設定しない」
		if (annualHoliday.isPrioritySubstitute() && !subVacaTypeUseFlg && isSubVacaManage
			...
			if (numberSubVaca > 0) {// 振休残数>0(restRemaining >0)
				if (pridigCheck.equals(AppliedDate.CHECK_IMPOSSIBLE)) {// 年休より優先消化チェック区分＝チェックする（登録不可）(pridigCheck ==
																		// CHECK_IMPOSSIBLE)
		...
		// 休暇の取得ルール．年休より優先する休暇．代休を優先＝ false OR 休暇申請対象勤務種類．休暇種類を利用しない（代休）＝ true OR
		// 代休管理設定．管理区分＝管理しない
		// 代休使用フラグをチェックする (Check substituteHolidayFlag)
//		2018/09/07 muto upd EA#2660
//		条件の追加： 「休暇の取得ルール．取得する順番をチェックする＝設定しない」
		if (!annualHoliday.isPriorityPause() || subHdTypeUseFlg || !isSubHdManage
				|| rule.getCategory().equals(SettingDistinct.NO)) {
			return result;
		}
	...
	public NumberOfRemainOutput getNumberOfRemaining(String companyID, String employeeID, GeneralDate baseDate,
			ManageDistinct annualLeaveManageDistinct, ManageDistinct accumulatedManage,
			ManageDistinct substituteLeaveManagement, ManageDistinct holidayManagement,
			ManageDistinct overrest60HManagement, ManageDistinct childNursingManagement,
			ManageDistinct longTermCareManagement) {
		val require = requireService.createRequire();
		val cache = new CacheCarrier();
		RequireM11Imp requireM11Imp = new RequireM11Imp(comDayOffManaDataRepo, leaveComDayOffManaRepo,
				leaveManaDataRepo, shareEmploymentAdapter, compensLeaveEmSetRepo, compensLeaveComSetRepo,
				interimBreakDayOffMngRepo, closureEmploymentRepo, closureRepo, empEmployeeAdapter,
				substitutionOfHDManaDataRepo, payoutSubofHDManaRepo, payoutManagementDataRepo, empSubstVacationRepo,
				comSubstVacationRepo, interimRecAbasMngRepo, payoutHdManaRepo);
		...
		if (holidayManagement.equals(ManageDistinct.YES)) {// output．振休管理区分が管理する
			// [No.506]振休残数を取得する
			LeaveRemainingDayNumber subVaca = AbsenceReruitmentMngInPeriodQuery.getAbsRecMngRemain(requireM11Imp, cache,
		...
		if (substituteLeaveManagement.equals(ManageDistinct.YES)) {// output．代休管理区分が管理する
			// [No.505]代休残数を取得する
			NumberConsecutiveVacation numberConsecutiveVacation = BreakDayOffMngInPeriodQuery
					.getBreakDayOffMngRemain(requireM11Imp, cache, employeeID, GeneralDate.today());
		...
		if (annualLeaveManageDistinct.equals(ManageDistinct.YES)) {// output．年休管理区分が管理する
			...
			// 次回年休付与日を取得する
			List<NextAnnualLeaveGrantImport> nextYearHolidays = annualHolidayManagementAdapter
					.acquireNextHolidayGrantDate(companyID, employeeID, GeneralDate.today());
		...
		if (overrest60HManagement.equals(ManageDistinct.YES)) {// output.60H超休管理区分管理する
			...
			AggrResultOfHolidayOver60hImport over60hImport = getHolidayOver60hRemNumWithinPeriodAdapter.algorithm(
					companyID, employeeID, new DatePeriod(closureDate, closureDate.addYears(1).addDays(-1)),
					InterimRemainMngMode.OTHER, baseDate, Optional.of(false), Optional.empty(), Optional.empty());
...
//		            Optional.empty());
			...
			childNursingDayRemain = childNursePeriod.getStartdateDays().getThisYear().getRemainingNumber()
					.getUsedDays();
			if (childNursePeriod.getStartdateDays().getThisYear().getRemainingNumber().getUsedTime().isPresent()) {
				childNursingHourRemain = childNursePeriod.getStartdateDays().getThisYear().getRemainingNumber()
						.getUsedTime().get();
			}
		}
...
//                    Optional.empty());
			...
			if (longtermCarePeriod.getStartdateDays().getThisYear().getRemainingNumber().getUsedTime().isPresent()) {
				nursingHourRemain = longtermCarePeriod.getStartdateDays().getThisYear().getRemainingNumber()
						.getUsedTime().get();
			}
		}
//        return NumberOfRemainOutput.init(yearRemain, subHdRemain, subVacaRemain, stockRemain, yearManage, subHdManage, subVacaManage, retentionManage);
		return new NumberOfRemainOutput(yearDayRemain, yearHourRemain, lastYearRemain, subDayRemain, subHdHourRemain,
				vacaRemain, over60HHourRemain, childNursingDayRemain, childNursingHourRemain, nursingRemain,
				nursingHourRemain, grantDate, grantDays);
	}
...
//		List<DisplayReason> displayReasonLst = displayRep.findDisplayReason(companyID);
		...
		VacationApplicationReflect vacationApplicationReflect = vacationAppReflectRepository
				.findReflectByCompanyId(companyID).orElse(null);
		// 取得した情報を返す
	...
	public RemainVacationInfo getRemainVacationInfo(String companyID, String employeeID, GeneralDate date) {
		...
		// 各休暇の残数を取得する
		NumberOfRemainOutput numberOfRemainOutput = this.getNumberOfRemaining(companyID, employeeID, date,
		...
		return new RemainVacationInfo(checkDispHolidayType.getAnnAnualLeaveManagement(),
				checkDispHolidayType.getAccumulatedRestManagement(),
				checkDispHolidayType.getSubstituteLeaveManagement(), checkDispHolidayType.getHolidayManagement(),
				checkDispHolidayType.getOvertime60hManagement(), checkDispHolidayType.getNursingCareLeaveManagement(),
	...
	public AppAbsenceStartInfoOutput getSpecAbsenceUpperLimit(String companyID,
		...
		CheckWkTypeSpecHdEventOutput checkWkTypeSpecHdEventOutput = specialHolidayEventAlgorithm
				.checkWkTypeSpecHdForEvent(companyID, workTypeCD.get());
		if (!checkWkTypeSpecHdEventOutput.isSpecHdForEventFlag()) {
			return appAbsenceStartInfoOutput;
		}
		if (checkWkTypeSpecHdEventOutput.getSpecHdEvent().get()
				.getMaxNumberDay() == MaxNumberDayType.REFER_RELATIONSHIP) {
			// 指定する特休枠の続柄に対する上限日数を取得する
			List<DateSpecHdRelationOutput> dateSpecHdRelationOutputLst = specialHolidayEventAlgorithm
					.getMaxDaySpecHdByRelaFrameNo(companyID, checkWkTypeSpecHdEventOutput.getFrameNo().get());
			// 指定する特休枠の上限日数を取得する
			MaxDaySpecHdOutput maxDaySpecHdOutput = specialHolidayEventAlgorithm.getMaxDaySpecHd(companyID,
			...
			SpecAbsenceDispInfo specAbsenceDispInfo = new SpecAbsenceDispInfo(
					checkWkTypeSpecHdEventOutput.isSpecHdForEventFlag(), checkWkTypeSpecHdEventOutput.getSpecHdEvent(),
					checkWkTypeSpecHdEventOutput.getFrameNo(), Optional.of(maxDaySpecHdOutput.getMaxDay()),
					Optional.of(maxDaySpecHdOutput.getDayOfRela()), Optional.of(dateSpecHdRelationOutputLst));
			appAbsenceStartInfoOutput.setSpecAbsenceDispInfo(Optional.of(specAbsenceDispInfo));
		} else {
			// 指定する特休枠の上限日数を取得する
			MaxDaySpecHdOutput maxDaySpecHdOutput = specialHolidayEventAlgorithm.getMaxDaySpecHd(companyID,
					checkWkTypeSpecHdEventOutput.getFrameNo().get(),
					checkWkTypeSpecHdEventOutput.getSpecHdEvent().get(), Optional.empty());
			// INPUT．「休暇申請起動時の表示情報」に項目をセットして返す
			SpecAbsenceDispInfo specAbsenceDispInfo = new SpecAbsenceDispInfo(
					checkWkTypeSpecHdEventOutput.isSpecHdForEventFlag(), checkWkTypeSpecHdEventOutput.getSpecHdEvent(),
					checkWkTypeSpecHdEventOutput.getFrameNo(), Optional.of(maxDaySpecHdOutput.getMaxDay()),
					Optional.of(maxDaySpecHdOutput.getDayOfRela()), Optional.empty());
			appAbsenceStartInfoOutput.setSpecAbsenceDispInfo(Optional.of(specAbsenceDispInfo));
		}
		return appAbsenceStartInfoOutput;
	}
	...
	public AppAbsenceStartInfoOutput workTimesChangeProcess(String companyID,
		...
		if (!workTimeCD.isPresent() || Strings.isBlank(workTimeCD.get())) {
			// 「休暇申請起動時の表示情報」を返す
			return appAbsenceStartInfoOutput;
		}
		// 勤務時間初期値の取得
		...
		if (!timeZones.isEmpty()) {
			for (int i = 0; i < timeZones.size(); i++) {
				timezoneUses.add(
						new TimezoneUse(timeZones.get(i).getStart(), timeZones.get(i).getEnd(), UseSetting.USE, i + 1));
			}
			appAbsenceStartInfoOutput.setWorkTimeLst(timezoneUses);
		}
		// ドメイン「就業時間帯の設定」を取得する
		Optional<WorkTimeSetting> workTimeSettingOptional = workTimeSettingRepository.findByCode(companyID,
				workTimeCD.get());
		if (workTimeSettingOptional.isPresent()) {
			// 流動勤務か判断して、セットする
			appAbsenceStartInfoOutput.setFlowWorkFlag(
					workTimeSettingOptional.get().getWorkTimeDivision().getWorkTimeMethodSet().isFluidWork());
		}
	...
	public List<TimeZone> initWorktimeCode(String companyID, String workTypeCode, String workTimeCode) {
		...
		if (WkTypeOpt.isPresent()) {
			...
			WorkStyle workStyle = basicScheduleService.checkWorkDay(companyID,
					WkTypeOpt.get().getWorkTypeCode().toString());
			if (workStyle == null) {
...
//				return workInformation.getPredeterminedTimezone(companyID, workTimeCode, workTypeCode, null);
				AbsenceServiceRequireImpl require = new AbsenceServiceRequireImpl(workTypeRepository,
						workTimeSettingRepository, basicScheduleService, fixedWorkSet, flowWorkSet, flexWorkSet,
						predetemineRepo);
	...
	public AppAbsenceStartInfoOutput workTypeChangeProcess(String companyID, List<String> appDates,
			AppAbsenceStartInfoOutput appAbsenceStartInfoOutput, HolidayAppType holidayType,
			Optional<String> workTypeCD) {
		// INPUT．「休暇申請起動時の表示情報．選択中の勤務種類」にセットする
		appAbsenceStartInfoOutput.setSelectedWorkTypeCD(workTypeCD);
		// 就業時間帯の表示制御フラグを確認する
		boolean controlDispWorkingHours = appAbsenceFourProcess.getDisplayControlWorkingHours(workTypeCD,
				appAbsenceStartInfoOutput.getVacationAppReflect(), companyID);
		...
		if (holidayType == HolidayAppType.SPECIAL_HOLIDAY) {
			...
			AttendanceTime requiredTime = this.calculateTimeRequired(
					appAbsenceStartInfoOutput.getAppDispInfoStartupOutput().getAppDispInfoNoDateOutput()
							.getEmployeeInfoLst().get(0).getSid(),
					appDates.isEmpty() ? Optional.empty()
							: Optional.of(GeneralDate.fromString(appDates.get(0), FORMAT_DATE)),
					workTypeCD,
					appAbsenceStartInfoOutput.isWorkTimeChange() ? appAbsenceStartInfoOutput.getSelectedWorkTimeCD()
							: Optional.empty(),
		...
		if (controlDispWorkingHours) {
			// 就業時間帯変更時処理
			appAbsenceStartInfoOutput = this.workTimesChangeProcess(companyID, appAbsenceStartInfoOutput,
					workTypeCD.get(), appAbsenceStartInfoOutput.getSelectedWorkTimeCD());
		}
		// 返ってきた「休暇申請起動時の表示情報」を返す
		return appAbsenceStartInfoOutput;
	 ...
	 */
	@Override
	public AttendanceTime calculateTimeRequired(String employeeID, Optional<GeneralDate> date,
			Optional<String> workTypeCD, Optional<String> selectedWorkTimeCD,
			Optional<WorkInfoOfDailyAttendance> workInfoDaily, Optional<ScBasicScheduleImport> schedule,
		...
		// 就業時間帯を判断する
		Optional<String> workTimeCD = this.determineWorkingHour(employeeID, date.isPresent() ? date.get() : null,
				selectedWorkTimeCD, workInfoDaily, schedule, workingCondition);
		...
		Optional<PredetemineTimeSetting> predetemineTimeSetting = this.predetemineRepo.findByWorkTimeCode(companyID,
				workTimeCD.get());
	 ...
	 * @param workingCondition
	 */
	private Optional<String> determineWorkingHour(String employeeID, GeneralDate date, Optional<String> workTimeCD,
			Optional<WorkInfoOfDailyAttendance> workInfoDaily, Optional<ScBasicScheduleImport> schedule,
			Optional<WorkingConditionItem> workingCondition) {
		// INPUT．「就業時間帯」をチェックする
		if (workTimeCD.isPresent()) {
		...
		if (workingCondition.isPresent() && workingCondition.get().getWorkCategory().getWorkTime().getWeekdayTime()
				.getWorkTimeCode().isPresent()) {
			// INPUT．「労働条件項目．区分別勤務．平日時．就業時間帯コード」を返す
			return Optional.of(workingCondition.get().getWorkCategory().getWorkTime().getWeekdayTime().getWorkTimeCode()
					.get().v());
		}

		// 社員の労働条件を取得する
		Optional<WorkingConditionItem> opWorkingConditionItem = WorkingConditionService
				.findWorkConditionByEmployee(createRequireM1(), employeeID, date);

		// 取得した「労働条件項目」をチェックする
		if (opWorkingConditionItem.isPresent() && opWorkingConditionItem.get().getWorkCategory().getWorkTime()
				.getWeekdayTime().getWorkTimeCode().isPresent()) {
			return Optional.of(opWorkingConditionItem.get().getWorkCategory().getWorkTime().getWeekdayTime()
...
//	}
	@Override
	public AppAbsenceStartInfoOutput holidayTypeChangeProcess(String companyID,
			AppAbsenceStartInfoOutput appAbsenceStartInfoOutput, List<String> appDates, HolidayAppType holidayType) {
		// INPUT．「休暇申請起動時の表示情報．勤務種類一覧」をクリアする
		appAbsenceStartInfoOutput.setWorkTypeLst(new ArrayList<>());

		// 申請理由表示区分を取得する
		AppReasonOutput appReason = commonAlg.getAppReasonDisplay(companyID,
				nts.uk.ctx.at.request.dom.application.ApplicationType.ABSENCE_APPLICATION, Optional.of(holidayType));

		// 勤務種類を取得する
		List<WorkType> workTypes = appAbsenceThreeProcess.getWorkTypeDetails(companyID, holidayType,
				appAbsenceStartInfoOutput.getAppDispInfoStartupOutput().getAppDispInfoWithDateOutput()
						.getOpEmploymentSet().isPresent()
								? Optional.ofNullable(appAbsenceStartInfoOutput.getAppDispInfoStartupOutput()
										.getAppDispInfoWithDateOutput().getOpEmploymentSet().get()
...
//		}
		...
		// 「休暇申請起動時の表示情報」を更新する
		appAbsenceStartInfoOutput.getAppDispInfoStartupOutput().getAppDispInfoNoDateOutput()
				.setDisplayAppReason(appReason.getDisplayAppReason());
		appAbsenceStartInfoOutput.getAppDispInfoStartupOutput().getAppDispInfoNoDateOutput()
				.setDisplayStandardReason(appReason.getDisplayStandardReason());
		appAbsenceStartInfoOutput.getAppDispInfoStartupOutput().getAppDispInfoNoDateOutput()
				.setReasonTypeItemLst(appReason.getReasonTypeItemLst());
		appAbsenceStartInfoOutput.setWorkTypeLst(workTypes);

		Optional<String> selectedWorkTypeCD = appAbsenceStartInfoOutput.getSelectedWorkTypeCD();
		if (selectedWorkTypeCD.isPresent()) {
			appAbsenceStartInfoOutput.setSelectedWorkTypeCD(appAbsenceStartInfoOutput.getWorkTypeLst().stream()
					.filter(x -> x.getWorkTypeCode().v().equals(selectedWorkTypeCD.get())).collect(Collectors.toList())
					.size() > 0 ? appAbsenceStartInfoOutput.getSelectedWorkTypeCD()
							: (workTypes.size() > 0 ? Optional.of(workTypes.get(0).getWorkTypeCode().v())
									: Optional.empty()));
		} else {
			appAbsenceStartInfoOutput.setSelectedWorkTypeCD(
					workTypes.isEmpty() ? Optional.empty() : Optional.of(workTypes.get(0).getWorkTypeCode().v()));
		}

		// 勤務種類変更時処理
		appAbsenceStartInfoOutput = this.workTypeChangeProcess(companyID, appDates, appAbsenceStartInfoOutput,
				holidayType, appAbsenceStartInfoOutput.getSelectedWorkTypeCD());
		// 返ってきた「休暇申請起動時の表示情報」を返す
		return appAbsenceStartInfoOutput;
	}
	 ...
	 * @return
	 */
	private List<ConfirmMsgOutput> checkContradictionAppDate(String companyID, String employeeID, GeneralDate appDate,
			Integer alldayHalfDay, HolidayApplicationSetting hdAppSet) {
		List<ConfirmMsgOutput> result = new ArrayList<>();
		...
		if (wkType == null) {
			...
			if (appliedDate == AppliedDate.CHECK_AVAILABLE) {
				...
				result.add(new ConfirmMsgOutput("Msg_1520", Arrays.asList(appDate.toString("yyyy/MM/dd"))));
			}
		} else {
			...
			if (error) {
				...
				if (appliedDate == AppliedDate.CHECK_AVAILABLE) {
					// 申請日矛盾区分＝「1: チェックする（登録可）」
					result.add(new ConfirmMsgOutput("Msg_1522",
	...
	public List<ConfirmMsgOutput> inconsistencyCheck(String companyID, String employeeID, GeneralDate startDate,
		...
		for (GeneralDate loopDate = startDate; loopDate.beforeOrEquals(endDate); loopDate = loopDate.addDays(1)) {
			// 7-1_申請日の矛盾チェック
			List<ConfirmMsgOutput> loopResult = this.checkContradictionAppDate(companyID, employeeID, loopDate,
					alldayHalfDay, hdAppSet);
	...
	@Override
	public List<ConfirmMsgOutput> holidayCommonCheck(String companyID, GeneralDate closureStartDate,
			ApplyForLeave appAbsence, AppAbsenceStartInfoOutput appAbsenceStartInfoOutput,
			List<GeneralDate> lstHolidayDate, Optional<TimeDigestApplication> timeDigestApplication) {
		List<ConfirmMsgOutput> result = new ArrayList<>();
		...
		commonAlgorithm.appConflictCheck(companyID, appAbsenceStartInfoOutput.getAppDispInfoStartupOutput()
				.getAppDispInfoNoDateOutput().getEmployeeInfoLst().get(0) // KAF006: -PhuongDV domain fix pending-
																			// confirm input -> auto lấy ở đầu danh sách
				, dateLst, lstWorkType, appAbsenceStartInfoOutput.getAppDispInfoStartupOutput()
						.getAppDispInfoWithDateOutput().getOpActualContentDisplayLst().get());
...
//				timeDigestApplication);
	...
	public List<ConfirmMsgOutput> annualLeaveCheck(boolean mode, String companyID, String employeeID,
		...
		List<ConfirmMsgOutput> result = new ArrayList<>();
		// 休暇の優先順をチェックする
		// todo // KAF006: -PhuongDV domain fix pending- Bên KMF001 team C đang làm phần
		// này, liên lạc vs Hiếu
...
//				.stream().filter(x -> x.getHolidayOrPauseType() == HolidayAppType.ANNUAL_PAID_LEAVE.value).findFirst().orElse(null);
		// AppEmploymentSetting employmentSet =
...
//		result.addAll(confirmLst1);
	...
	public void checkTimeDigestProcess(String companyID, TimeDigestApplication timeDigestApplication,
			...
			Optional<AttendanceTime> requiredTime) {
		// 社員の労働条件を取得する
		Optional<WorkingConditionItem> opWorkingConditionItem = WorkingConditionService
				.findWorkConditionByEmployee(createRequireM1(), employeeId, baseDate);
		...
		if (acqRule.isPresent()) {
			...
			acqRule.get().checkVacationPriority(timeDigestApplicationShares,
					remainVacationInfo.getOver60HHourRemain().isPresent()
							? remainVacationInfo.getOver60HHourRemain().get()
							: null,
					remainVacationInfo.getSubHdRemain().isPresent()
							? remainVacationInfo.getSubHdRemain().get().intValue()
							: null,
	...
	public List<ConfirmMsgOutput> errorCheckByHolidayType(boolean mode, String companyID, ApplyForLeave appAbScene,
		...
		case ANNUAL_PAID_LEAVE:
			// INPUT．「休暇種類」 = 年次有給
			// 年休のチェック処理
			List<ConfirmMsgOutput> confirmMsgLst1 = this.annualLeaveCheck(mode, companyID, appAbScene.getEmployeeID(),
					appAbScene.getOpAppStartDate().get().getApplicationDate(),
					appAbScene.getOpAppEndDate().get().getApplicationDate(),
					appAbScene.getReflectFreeTimeApp().getWorkInfo().getWorkTypeCode().v(), lstDate,
		...
		case SPECIAL_HOLIDAY:
			// INPUT．「休暇種類」 = 特別休暇
			// 特別休暇のチェック処理
			this.checkSpecLeaveProcess(companyID, appAbScene.getOpAppStartDate().get().getApplicationDate(),
					appAbScene.getOpAppEndDate().get().getApplicationDate(), lstDate,
					appAbsenceStartInfoOutput.getSpecAbsenceDispInfo().orElse(null), appAbScene.getVacationInfo()
							.getInfo().getApplyForSpeLeaveOptional().map(x -> x.isMournerFlag()).orElse(false));// //
																												// KAF006:
																												// -PhuongDV
																												// domain
																												// fix
		...
		case DIGESTION_TIME:
			...
			// 時間消化のチェック処理
			/*
			 * public void checkTimeDigestProcess(String companyID, TimeDigestApplication
			 * timeDigestApplication , RemainVacationInfo remainVacationInfo, String
			 * employeeId, GeneralDate baseDate ){
			 */
	...
	@Override
	public AbsenceCheckRegisterOutput checkAppAbsenceRegister(boolean mode, String companyID,
			AppAbsenceStartInfoOutput appAbsenceStartInfoOutput, ApplyForLeave appBeforeChange,
			ApplyForLeave newAbsence) {
		AbsenceCheckRegisterOutput result = new AbsenceCheckRegisterOutput();
		// 4.社員の当月の期間を算出する
		PeriodCurrentMonth periodCurrentMonth = otherCommonAlgorithm.employeePeriodCurrentMonthCalculate(companyID,
				newAbsence.getEmployeeID(), GeneralDate.today());
		// 変更後の申請期間をチェックする
		this.changeAbScenePeriodCheck(appBeforeChange, newAbsence, periodCurrentMonth.getStartDate());
		// 申請期間から休日の申請日を取得する
		List<GeneralDate> holidayDates = otherCommonAlgorithm.lstDateIsHoliday(
				newAbsence.getApplication().getEmployeeID(),
				new DatePeriod(newAbsence.getOpAppStartDate().get().getApplicationDate(),
						newAbsence.getOpAppEndDate().get().getApplicationDate()),
				appAbsenceStartInfoOutput.getAppDispInfoStartupOutput().getAppDispInfoWithDateOutput()
						.getOpActualContentDisplayLst().get());
		// 取得した「休日の申請日<List>」をチェックする
		List<GeneralDate> listDates = new DatePeriod(
				newAbsence.getApplication().getOpAppStartDate().get().getApplicationDate(),
		...
		List<ConfirmMsgOutput> lstConfirmMsg = newBeforeRegister.processBeforeRegister_New(companyID,
				EmploymentRootAtr.APPLICATION, false // KAF006: -PhuongDV domain fix pending- confirm input
				, newAbsence.getApplication(), null, appAbsenceStartInfoOutput.getAppDispInfoStartupOutput()
						.getAppDispInfoWithDateOutput().getOpMsgErrorLst().orElse(Collections.emptyList()) // KAF006:
																											// -PhuongDV
																											// domain
																											// fix
																											// pending-
				...
				Optional.of(timeDigestionParam),
				newAbsence.getReflectFreeTimeApp().getWorkInfo().getWorkTimeCodeNotNull().map(WorkTimeCode::v), false);
		...
		// 勤務種類リスト作成
		List<String> lstWorkType = new ArrayList<String>();
		// KAF006: -PhuongDV domain fix pending- confirm input -> ・申請する勤務種類リスト =
		// INPUT．「休暇申請．反映情報．勤務情報．勤務種類コード」 -> ko co trong input EAP
...
//		result.addAll(confirmMsgLst2);
	...
	@Override
	public List<ConfirmMsgOutput> checkAbsenceWhenRegister(boolean mode, String companyID, ApplyForLeave appAbscene,
		...
		// 4.社員の当月の期間を算出する
		PeriodCurrentMonth periodCurrentMonth = otherCommonAlgorithm.employeePeriodCurrentMonthCalculate(companyID,
				appAbscene.getEmployeeID(), GeneralDate.today());
		// 休暇種類共通エラーチェック
		lstConfirmMsg.addAll(this.holidayCommonCheck(companyID, periodCurrentMonth.getStartDate(), appAbscene,
				appAbsenceStartInfoOutput, lstHolidayDate, appAbscene.getReflectFreeTimeApp().getTimeDegestion()));
		// 休暇種類別エラーチェック
		lstConfirmMsg.addAll(
				this.errorCheckByHolidayType(mode, companyID, appAbscene, lstHolidayDate, appAbsenceStartInfoOutput));
		// 「確認メッセージリスト」を返す
	...
	public void changeAbScenePeriodCheck(ApplyForLeave absceneBefore, ApplyForLeave absceneNew, GeneralDate endDate) {
		...
		if (boforeStartDate.equals(newStartDate) && beforeEndDate.equals(newEndDate)) {
	...
	public AppAbsenceStartInfoOutput getWorkTypeWorkTimeInfo(String companyID, ApplyForLeave appAbsence,
			AppAbsenceStartInfoOutput appAbsenceStartInfoOutput) {
		// 選択可能の勤務種類を取得する
		List<WorkType> workTypeLst = appAbsenceThreeProcess.getWorkTypeDetails(companyID,
				appAbsence.getVacationInfo().getHolidayApplicationType(),
				appAbsenceStartInfoOutput.getAppDispInfoStartupOutput().getAppDispInfoWithDateOutput()
						.getOpEmploymentSet().isPresent()
								? Optional.ofNullable(appAbsenceStartInfoOutput.getAppDispInfoStartupOutput()
		...
		/*
		 * // 申請済み勤務種類の存在判定と取得 ApplyWorkTypeOutput applyWorkTypeOutput =
		 * commonAlgorithm.appliedWorkType(companyID, workTypeLst,
		 * appAbsence.getReflectFreeTimeApp().getWorkInfo().getWorkTypeCode().v());
		 */

		// 申請中の勤務種類・就業時間帯を取得する
		WorkInfoListOutput workInfos = commonAlgorithm.getWorkInfoList(companyID,
				appAbsence.getReflectFreeTimeApp().getWorkInfo().getWorkTypeCode().v(),
				appAbsence.getReflectFreeTimeApp().getWorkInfo().getWorkTimeCodeNotNull()
						.map(x -> x != null ? x.v() : null),
				workTypeLst, appAbsenceStartInfoOutput.getAppDispInfoStartupOutput().getAppDispInfoWithDateOutput()
						.getOpWorkTimeLst().get());
		...
		if (workTimeCD != null || workTypeCD != null) {
			workInformationForApplication = new WorkInformationForApplication(workTimeCD, workTypeCD);
		}

		// INPUT．「休暇申請起動時の表示情報」を更新する
		appAbsenceStartInfoOutput.setWorkTypeLst(workInfos.getWorkTypes());
		appAbsenceStartInfoOutput.getAppDispInfoStartupOutput().getAppDispInfoWithDateOutput()
				.setOpWorkTimeLst(Optional.of(workInfos.getWorkTimes()));
		appAbsenceStartInfoOutput.setWorkInformationForApplication(Optional.ofNullable(workInformationForApplication));
		appAbsenceStartInfoOutput.setSelectedWorkTypeCD(
				appAbsence.getReflectFreeTimeApp().getWorkInfo().getWorkTypeCode() == null ? Optional.empty()
						: Optional.of(appAbsence.getReflectFreeTimeApp().getWorkInfo().getWorkTypeCode().v()));
		appAbsenceStartInfoOutput.setSelectedWorkTimeCD(
				appAbsence.getReflectFreeTimeApp().getWorkInfo().getWorkTimeCode() == null ? Optional.empty()
						: Optional.of(appAbsence.getReflectFreeTimeApp().getWorkInfo().getWorkTimeCode().v()));
			...
			if (wtAfter.getDailyWork().getWorkTypeUnit() == WorkTypeUnit.OneDay) {
				...
				if (wtBefore.getDailyWork().getWorkTypeUnit().equals(wtAfter.getDailyWork().getWorkTypeUnit())
						&& (wtAfter.getDailyWork().getMorning().equals(WorkTypeClassification.SubstituteHoliday)
								|| wtAfter.getDailyWork().getAfternoon()
										.equals(WorkTypeClassification.SubstituteHoliday))) {
					return true;
				}
			}
		}
	...
	public ProcessResult registerAppAbsence(ApplyForLeave applyForLeave, List<String> appDates,
		...
		List<GeneralDate> listHolidayDates = appDates.stream().map(date -> GeneralDate.fromString(date, FORMAT_DATE))
				.collect(Collectors.toList());
		...
		for (ReflectionStatusOfDay item : applyForLeave.getApplication().getReflectionStatus()
				.getListReflectionStatusOfDay()) {
		...
		;
		this.applicationService.insertApp(appTemp, approvalRoot);
		...
		String reflectAppId = this.registerApproveReflectInfoService
				.newScreenRegisterAtApproveInfoReflect(applyForLeave.getApplication().getEmployeeID(), appNew);
		...
		this.interimRemainDataMngRegisterDateChange.registerDateChange(companyId,
				applyForLeave.getApplication().getEmployeeID(), listDates);

		// アルゴリズム「新規画面登録後の処理」を実行する
		ProcessResult result = this.afterRegisterService.processAfterRegister(
				Arrays.asList(applyForLeave.getApplication().getAppID()), appTypeSetting, mailServerSet, false);
	...
	public void registerVacationLinkManage(List<LeaveComDayOffManagement> leaveComDayOffMana,
		...
		if (!leaveComDayOffMana.isEmpty()) {
			...
			this.leaveComDayOffManaRepo.deleteByDigestTarget(leaveComDayOffMana.get(0).getSid(),
					leaveComDayOffMana.get(0).getAssocialInfo().getDateOfUse(), TargetSelectionAtr.REQUEST);
		...
		if (!payoutSubofHDManagements.isEmpty()) {
			...
			this.payoutHdManaRepo.deleteByDigestTarget(payoutSubofHDManagements.get(0).getSid(),
					payoutSubofHDManagements.get(0).getAssocialInfo().getDateOfUse(), TargetSelectionAtr.REQUEST);
	...
	public AppForLeaveStartOutput getAppForLeaveStartB(String companyID, String appID,
		...
		if (applyForLeave.isPresent()) {
			applyForLeave.get()
					.setApplication(appDispInfoStartupOutput.getAppDetailScreenInfo().get().getApplication());
		}
		...
		appAbsenceStartInfoOutput.setWorkTimeChange(
				applyForLeave.isPresent() ? applyForLeave.get().getReflectFreeTimeApp().getWorkChangeUse().isUse()
						: false);

		// 勤務種類・就業時間帯情報を取得する
		appAbsenceStartInfoOutput = this.getWorkTypeWorkTimeInfo(companyID,
				applyForLeave.isPresent() ? applyForLeave.get() : null, appAbsenceStartInfoOutput);

		// 休暇紐付管理情報を取得する
		WorkType workTypeParam = appAbsenceStartInfoOutput.getWorkTypeLst().stream()
				.filter(workType -> workType.getWorkTypeCode()
						.equals(applyForLeave.get().getReflectFreeTimeApp().getWorkInfo().getWorkTypeCode()))
		...
		VacationLinkManageInfo vacationLinkManageInfo = this.getVacationLinkManageInfo(
				...
				workTypeParam, appAbsenceStartInfoOutput.getAppDispInfoStartupOutput().getAppDispInfoWithDateOutput()
						.getOpActualContentDisplayLst().get());
	...
	@Override
	public VacationLinkManageInfo getVacationLinkManageInfo(String employeeID, String appStartDate, String appEndDate,
			WorkType workType, List<ActualContentDisplay> actualContentDisplayLst) {

		// 申請期間から休日の申請日を取得する
		List<GeneralDate> lstDatesHoliday = otherCommonAlgorithm.lstDateIsHoliday(employeeID,
				new DatePeriod(GeneralDate.fromString(appStartDate, FORMAT_DATE),
						GeneralDate.fromString(appEndDate, FORMAT_DATE)),
				actualContentDisplayLst);

		// 「対象年月日リスト」を作成する
		DatePeriod datePeriod = new DatePeriod(GeneralDate.fromString(appStartDate, FORMAT_DATE),
				GeneralDate.fromString(appEndDate, FORMAT_DATE));
	...
	public AbsenceCheckRegisterOutput checkBeforeUpdate(String companyID,
		...
		AbsenceCheckRegisterOutput result = new AbsenceCheckRegisterOutput();
		// 紐付けデータをチェックする
		if (appAbsenceStartInfoOutput.getWorkTypeLst().stream()
				.filter(x -> x.getWorkTypeCode().v()
						.equals(appAbsence.getReflectFreeTimeApp().getWorkInfo().getWorkTypeCode().v()))
				.findFirst().isPresent()) {
			this.checkLinkingData(
					appAbsenceStartInfoOutput.getWorkTypeLst().stream()
							.filter(x -> x.getWorkTypeCode().v()
									.equals(appAbsence.getReflectFreeTimeApp().getWorkInfo().getWorkTypeCode().v()))
							.findFirst().get(),
					appAbsenceStartInfoOutput.getRemainVacationInfo().getSubstituteLeaveManagement()
							.getLinkingManagement(),
					appAbsenceStartInfoOutput.getLeaveComDayOffManas(),
					appAbsenceStartInfoOutput.getPayoutSubofHDManagements(), isEmptyLeaveList, isEmptyPayoutList);
		}
		...
		if (appAbsenceStartInfoOutput.getWorkInformationForApplication().isPresent()) {
			...
			if (appAbsence.getReflectFreeTimeApp().getWorkInfo().getWorkTimeCodeNotNull().isPresent()
					...
					&& !appAbsence.getReflectFreeTimeApp().getWorkInfo().getWorkTimeCodeNotNull().get().v().equals(
							appAbsenceStartInfoOutput.getWorkInformationForApplication().get().getWorkTimeCode().v())) {
		...
		detailBeforeUpdate.processBeforeDetailScreenRegistration(companyID, appAbsence.getApplication().getEmployeeID(),
				appAbsence.getAppDate().getApplicationDate(), EmploymentRootAtr.APPLICATION.value,
				appAbsence.getApplication().getAppID(), appAbsence.getApplication().getPrePostAtr(),
				appAbsenceStartInfoOutput.getAppDispInfoStartupOutput().getAppDetailScreenInfo().get().getApplication()
						.getVersion(),
				workType, workTime, appAbsenceStartInfoOutput.getAppDispInfoStartupOutput(),
				Arrays.asList(appAbsence.getReflectFreeTimeApp().getWorkInfo().getWorkTypeCode().v()),
				Optional.of(timeDigestionParam), false,
				Optional.of(appAbsence.getReflectFreeTimeApp().getWorkInfo().getWorkTypeCode().v()),
				appAbsence.getReflectFreeTimeApp().getWorkInfo().getWorkTimeCodeNotNull().map(WorkTimeCode::v));

		// 休暇申請登録時チェック処理
		List<ConfirmMsgOutput> listConfirmMsg = this.checkAbsenceWhenRegister(true, companyID, appAbsence,
				appAbsenceStartInfoOutput, holidayDates);
	...
	public ProcessResult updateApplyForLeave(ApplyForLeave applyForLeave, List<String> holidayAppDates,
		...
		// ドメインモデル「休出代休紐付け管理」を取得する
		List<LeaveComDayOffManagement> leaveComDayOffManagementLst = leaveComDayOffManaRepository
				.getBycomDayOffID(application.getEmployeeID(), application.getAppDate().getApplicationDate()).stream()
				.filter(x -> x.getAssocialInfo().getTargetSelectionAtr() == TargetSelectionAtr.REQUEST)
				.collect(Collectors.toList());
		// ドメインモデル「振出振休紐付け管理」を取得する
		List<PayoutSubofHDManagement> payoutSubofHDManagementLst = payoutSubofHDManaRepository
				.getBySubId(application.getEmployeeID(), application.getAppDate().getApplicationDate()).stream()
				.filter(x -> x.getAssocialInfo().getTargetSelectionAtr() == TargetSelectionAtr.REQUEST)
				.collect(Collectors.toList());
		// 休暇発生日リストに追加する
		List<GeneralDate> holidayLst = new ArrayList<>();
		holidayLst.addAll(leaveComDayOffManagementLst.stream().map(x -> x.getAssocialInfo().getOutbreakDay())
				.collect(Collectors.toList()));
		holidayLst.addAll(payoutSubofHDManagementLst.stream().map(x -> x.getAssocialInfo().getOutbreakDay())
				.collect(Collectors.toList()));
		...
		List<GeneralDate> listHolidayDates = holidayAppDates.stream()
				.map(date -> GeneralDate.fromString(date, FORMAT_DATE)).collect(Collectors.toList());
		...
		return this.detailAfterUpdate.processAfterDetailScreenRegistration(companyID, application.getAppID(),
	...
	public AppAbsenceStartInfoOutput getChangeHolidayDates(String companyID, List<GeneralDate> holidayDates,
		...
		holidayDates = holidayDates.size() > 1 ? new DatePeriod(holidayDates.get(0), holidayDates.get(1)).datesBetween()
				: holidayDates;
		// 基準日として扱う日の取得
		GeneralDate refDate = appAbsenceStartInfoDto.getAppDispInfoStartupOutput().getAppDispInfoNoDateOutput()
				.getApplicationSetting()
				.getBaseDate(holidayDates.size() > 0 ? Optional.of(holidayDates.get(0)) : Optional.empty());

		// 12_承認ルートを取得
		ApprovalRootContentImport_New approvalRootContentImport = commonAlgorithm.getApprovalRoot(companyID,
				appAbsenceStartInfoDto.getAppDispInfoStartupOutput().getAppDispInfoNoDateOutput().getEmployeeInfoLst()
						.get(0).getSid(),
				EmploymentRootAtr.APPLICATION, appAbsenceStartInfoDto.getAppDispInfoStartupOutput()
						.getAppDetailScreenInfo().get().getApplication().getAppType(),
				refDate);
		...
		case NO_ERROR:
			...
			appAbsenceStartInfoDto.getAppDispInfoStartupOutput().getAppDispInfoWithDateOutput()
					.setOpListApprovalPhaseState(
							Optional.of(approvalRootContentImport.getApprovalRootState().getListApprovalPhaseState()));
			List<ActualContentDisplay> actualContentDisplays = collectAchievement.getAchievementContents(companyID,
					appAbsenceStartInfoDto.getAppDispInfoStartupOutput().getAppDispInfoNoDateOutput()
							.getEmployeeInfoLst().get(0).getSid(),
					holidayDates, appAbsenceStartInfoDto.getAppDispInfoStartupOutput().getAppDetailScreenInfo().get()
							.getApplication().getAppType());
			appAbsenceStartInfoDto.getAppDispInfoStartupOutput().getAppDispInfoWithDateOutput()
		...
		appAbsenceStartInfoDto.getAppDispInfoStartupOutput().getAppDispInfoWithDateOutput()
				.setOpMsgErrorLst(Optional.of(msgErrorLst));
	...
	public ProcessResult registerHolidayDates(String companyID, ApplyForLeave newApplyForLeave,
		...
		interimRemainDataMngRegisterDateChange.registerDateChange(companyID,
				originApplyForLeave.getApplication().getEmployeeID(),
				originApplyForLeave.getApplication().getReflectionStatus().getListReflectionStatusOfDay().stream()
						.map(x -> x.getTargetDate()).collect(Collectors.toList()));
		...
		this.updateLinkManage(
				...
				holidayDates, appAbsenceStartInfoDto.getLeaveComDayOffManas(),
				appAbsenceStartInfoDto.getPayoutSubofHDManagements());
		// 休暇申請（新規）登録処理
		ProcessResult processResult = this.registerAppAbsence(newApplyForLeave,
				...
				appAbsenceStartInfoDto.getAppDispInfoStartupOutput().getAppDispInfoWithDateOutput()
						.getOpListApprovalPhaseState().get(),
				appAbsenceStartInfoDto.getAppDispInfoStartupOutput().getAppDispInfoNoDateOutput()
						.getApplicationSetting().getAppTypeSettings().get(0));
	 ...
	 */
	public void updateLinkManage(String startDate, String endDate, List<GeneralDate> holidayDates,
			List<LeaveComDayOffManagement> leaveComDayOffMana, List<PayoutSubofHDManagement> payoutSubofHDManagements) {
		// 「対象年月日リスト」を作成する
		DatePeriod datePeriod = new DatePeriod(GeneralDate.fromString(startDate, FORMAT_DATE),
				GeneralDate.fromString(endDate, FORMAT_DATE));
		List<GeneralDate> listDates = datePeriod.datesBetween();

		listDates = listDates.stream().filter(date -> !holidayDates.contains(date))
				.sorted(Comparator.comparing(GeneralDate::localDate)).collect(Collectors.toList());
		...
		if (!leaveComDayOffMana.isEmpty()) {
			...
			if (listDates.size() == leaveComDayOffMana.size()) {
				...
				for (LeaveComDayOffManagement data : leaveComDayOffMana) {
					leaveComDayOffManaRepo.delete(data.getSid(), data.getAssocialInfo().getOutbreakDay(),
							data.getAssocialInfo().getDateOfUse());
				}
			}
		}
		...
		if (!payoutSubofHDManagements.isEmpty()) {
			...
			if (listDates.size() == payoutSubofHDManagements.size()) {
				...
				for (PayoutSubofHDManagement data : payoutSubofHDManagements) {
					payoutHdManaRepo.delete(data.getSid(), data.getAssocialInfo().getOutbreakDay(),
							data.getAssocialInfo().getDateOfUse());
				}
			}
		}
	...
	public void checkVacationTimeRequire(TimeDigestApplication timeDigestApplication,
		...
		if (totalTime < requiredVacationTime.valueAsMinutes()) {
			throw new BusinessException("Msg_2157",
					new TimeWithDayAttr(requiredVacationTime.v()).getRawTimeWithFormat());
		}
	}
	 ...
	 * 紐付けデータをチェックする
	 */
	public void checkLinkingData(WorkType workType, ManageDistinct linkingManagement,
			List<LeaveComDayOffManagement> leaveComDayOffManas, List<PayoutSubofHDManagement> payoutSubofHDManagements,
			boolean isEmptyLeaveList, boolean isEmptyPayoutList) {
		// 代休の紐付けをチェックする
	...
	public boolean isHolidayWorkType(WorkType workType) {
		...
		if (workTypeUnit.equals(WorkTypeUnit.OneDay)) {
			...
			return workType.getDailyWork().getMorning().equals(WorkTypeClassification.SubstituteHoliday)
	...
	public boolean isPauseWorkType(WorkType workType) {
		...
		if (workTypeUnit.equals(WorkTypeUnit.OneDay)) {
			...
			return workType.getDailyWork().getMorning().equals(WorkTypeClassification.Pause)
					|| workType.getDailyWork().getAfternoon().equals(WorkTypeClassification.Pause);
		}
	}
...
//    }
	...
	@AllArgsConstructor
	private class RequireM11Imp
			implements BreakDayOffMngInPeriodQuery.RequireM11, AbsenceReruitmentMngInPeriodQuery.RequireM11 {
		private ComDayOffManaDataRepository comDayOffManaDataRepo;
