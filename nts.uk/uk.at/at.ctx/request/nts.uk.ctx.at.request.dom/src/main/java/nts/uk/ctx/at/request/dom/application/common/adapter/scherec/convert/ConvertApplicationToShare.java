	...
	private static ApplicationShare appDetail(ApplicationShare appShare, Application application) {
		...
		try {
			...
			case WORK_CHANGE_APPLICATION:
				...
				return new AppWorkChangeShare(appWCh.getStraightGo(), appWCh.getStraightBack(),
						appWCh.getOpWorkTypeCD(), appWCh.getOpWorkTimeCD(), appWCh.getTimeZoneWithWorkNoLst(),
						appShare);
					...
					return new AppStampShare(appStamp.getListTimeStampApp().stream().map(x -> {
						return new TimeStampAppShare(converDesTimeApp(x.getDestinationTimeApp()), x.getTimeOfDay(),
							...
							appStamp.getListDestinationTimeZoneApp().stream().map(x -> {
								return new DestinationTimeZoneAppShare(TimeZoneStampClassificationShare
										.valueOf(x.getTimeZoneStampClassification().value), x.getEngraveFrameNo());
			...
			case ANNUAL_HOLIDAY_APPLICATION:
				...
				return new TimeLeaveApplicationShare(appShare,
						appTimeLeav.getLeaveApplicationDetails().stream().map(x -> {
			...
			case COMPLEMENT_LEAVE_APPLICATION:
				...
				if (typeAppHolidayShare == TypeApplicationHolidaysShare.Abs) {
					...
					return new AbsenceLeaveAppShare(absence.getWorkingHours(), absence.getWorkInformation(),
							absence.getWorkChangeUse(), absence.getChangeSourceHoliday(), typeAppHolidayShare,
							appShare);
				} else {
					// 振出申請
