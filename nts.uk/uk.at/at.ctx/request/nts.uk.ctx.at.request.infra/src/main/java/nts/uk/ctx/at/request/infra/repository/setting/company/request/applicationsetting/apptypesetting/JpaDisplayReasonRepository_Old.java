...
public class JpaDisplayReasonRepository_Old extends JpaRepository implements DisplayReasonRepository{
	...
	private KrqmtDisplayReason toEntity(DisplayReason domain){
		val entity = new KrqmtDisplayReason(new KrqstDisplayReasonPK(domain.getCompanyId(), domain.getTypeOfLeaveApp().value), 
	...
	public Optional<DisplayReason> findDpReasonHd(String companyId, int typeOfLeaveApp) {
		Optional<KrqmtDisplayReason> getData = this.queryProxy().find(new KrqstDisplayReasonPK(companyId, typeOfLeaveApp), KrqmtDisplayReason.class);
		if(getData != null){
	...
	public Optional<DisplayReason> findByHdType(String companyId, int hdType) {
		return this.queryProxy().find(new KrqstDisplayReasonPK(companyId, hdType), KrqmtDisplayReason.class)
				.map(c -> toDomain(c));
	}
}
