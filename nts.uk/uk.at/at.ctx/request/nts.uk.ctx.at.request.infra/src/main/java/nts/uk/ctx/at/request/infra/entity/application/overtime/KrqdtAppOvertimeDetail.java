...
public class KrqdtAppOvertimeDetail extends UkJpaEntity implements Serializable {
	...
	public AppOvertimeDetail toDomain() {
		return new AppOvertimeDetail(
				appOvertimeDetailPk.cid, 
				appOvertimeDetailPk.appId, 
				new YearMonth(yearMonth), 
				new Time36Agree(
						new AttendanceTimeMonth(appTimeAgree), 
						new Time36AgreeMonth(
								new AttendanceTimeMonth(actualTimeMonth), 
								new LimitOneMonth(limitAlarmTime), 
								new LimitOneMonth(limitErrorTime), 
								new NumberOfMonth(numOfYear36Over), 
								year36OverMonth.stream().map(x -> new YearMonth(x.year36OverMonthPk.overMonth)).collect(Collectors.toList()), 
								exceptionLimitAlarmTime == null ? Optional.empty() : Optional.of(new LimitOneMonth(exceptionLimitAlarmTime)), 
								exceptionLimitErrorTime == null ? Optional.empty() : Optional.of(new LimitOneMonth(exceptionLimitErrorTime))
						), 
	...
	public static KrqdtAppOvertimeDetail toEntity(Optional<AppOvertimeDetail> domainOtp) {
		...
		return new KrqdtAppOvertimeDetail(
				new KrqdtAppOvertimeDetailPk(domain.getCid(), domain.getAppId()), 
				domain.getYearMonth().v(), 
				domain.getTime36Agree().getApplicationTime().v(), 
				...
				domain.getTime36Agree().getAgreeMonth().getYear36OverMonth()
							.stream().map(x -> new KrqdtYear36OverMonth(new KrqdtYear36OverMonthPk(
									domain.getCid(), 
									domain.getAppId(), 
									x.v()))).collect(Collectors.toList()), 
