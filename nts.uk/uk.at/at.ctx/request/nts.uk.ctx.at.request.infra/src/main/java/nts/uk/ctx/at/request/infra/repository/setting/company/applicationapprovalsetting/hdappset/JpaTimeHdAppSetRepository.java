...
public class JpaTimeHdAppSetRepository extends JpaRepository implements TimeHdAppSetRepository{
	...
	private static KrqstTimeHdAppSet toEntity(TimeHdAppSet domain){
		...
		entity.nameAfter = domain.getNameAfter().v();
		entity.nameAfter2 = domain.getNameAfter2().v();
		entity.nameBefore = domain.getNameBefore().v();
		entity.nameBefore2 = domain.getNameBefore2().v();
		entity.privateName = domain.getPrivateName().v();
		entity.unionLeave = domain.getUnionLeave().value;
		entity.unionName = domain.getUnionName().v();
