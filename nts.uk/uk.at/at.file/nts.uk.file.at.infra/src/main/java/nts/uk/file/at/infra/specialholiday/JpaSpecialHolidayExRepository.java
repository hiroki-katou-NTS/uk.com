...
public class JpaSpecialHolidayExRepository extends JpaRepository implements SpecialHolidayExRepository {
	...
	StringBuilder specialHDData = new StringBuilder()
			...
			.append("	CASE WHEN TABLE_RESULT.ROW_NUMBER = 1 THEN CONCAT(TABLE_RESULT.abName, TABLE_RESULT.leaveName)")
			.append("	ELSE NULL")
			...
			.append("		CASE TABLE_RESULT.IS_SPECIFIED WHEN 0 THEN '－' WHEN 1 THEN '〇' END")
			.append("	ELSE NULL")
			...
			.append("	CASE WHEN  (TABLE_RESULT.TYPE_TIME = 1)  THEN ")
			.append("		CASE WHEN TABLE_RESULT.MONTHS IS NULL THEN NULL ELSE CONCAT(TABLE_RESULT.MONTHS,'ヵ月') END")
			.append("	ELSE NULL")
			.append("	END MONTHS,")
			.append("	CASE WHEN  (TABLE_RESULT.TYPE_TIME = 1)  THEN ")
			...
			.append("	END YEARS,")
			.append("	CASE WHEN  (TABLE_RESULT.ROW_NUMBER2 = 1 AND TABLE_RESULT.TYPE_TIME = 1)  THEN ")
			.append("		CASE TABLE_RESULT.FIXED_ASSIGN WHEN 0 THEN '－' WHEN 1 THEN '〇' END")
			.append("	ELSE NULL")
			...
			// Time_cls_method is 1 「有効期限を指定する」")
			.append("	CASE WHEN (TABLE_RESULT.ROW_NUMBER = 1 AND TABLE_RESULT.TIME_CSL_METHOD = 1) THEN ")
			.append("		CASE WHEN TABLE_RESULT.DEADLINE_MONTHS IS NULL THEN NULL ELSE CONCAT(TABLE_RESULT.DEADLINE_MONTHS,'月') END")
			.append("	ELSE NULL")
			...
			.append("	END END_DATE, ")
			.append("	CASE WHEN TABLE_RESULT.ROW_NUMBER = 1 THEN ")
			.append("		CASE TABLE_RESULT.GENDER_REST WHEN 1 THEN '－' WHEN 0 THEN '〇' END")
			.append("	ELSE NULL")
			...
			.append("	END GENDER, ")
			.append("	CASE WHEN TABLE_RESULT.ROW_NUMBER = 1 THEN ")
			.append("		CASE TABLE_RESULT.REST_EMP WHEN 1 THEN '－' WHEN 0 THEN '〇' END")
			.append("	ELSE NULL")
			.append("	END REST_EMP, ")
			...
			.append("	END empName,")
			.append("	CASE WHEN TABLE_RESULT.ROW_NUMBER = 1 THEN ")
			.append("		CASE TABLE_RESULT.RESTRICTION_CLS WHEN 1 THEN '－' WHEN 0 THEN '〇' END")
			.append("	ELSE NULL")
			...
			.append("	END clsName, ")
			.append("	CASE WHEN TABLE_RESULT.ROW_NUMBER = 1 THEN ")
			.append("		CASE TABLE_RESULT.AGE_LIMIT WHEN 1 THEN '－' WHEN 0 THEN '〇' END")
			.append("	ELSE NULL")
			...
			.append("					FROM KSHST_SPHD_SPEC_LEAVE spl1")
			.append("						LEFT JOIN KSHMT_SPHD_FRAME mspl1 ON spl1.SPHD_NO = mspl1.SPHD_FRAME_NO ")
			...
			.append("				LEFT JOIN KSHST_SPHD_SPEC_LEAVE spl ON sphd.SPHD_CD = spl.SPHD_CD AND sphd.CID = spl.CID ")
			.append("				LEFT JOIN KSHMT_SPHD_FRAME mspl ON spl.SPHD_NO = mspl.SPHD_FRAME_NO AND mspl.CID = spl.CID AND mspl.ABOLISH_ATR = 0")
			.append("				INNER JOIN KSHST_GRANT_REGULAR gra ON gra.CID = sphd.CID AND gra.SPHD_CD = sphd.SPHD_CD")
			.append("				INNER JOIN KSHST_GRANT_PERIODIC graPe ON graPe.CID = sphd.CID AND graPe.SPHD_CD = sphd.SPHD_CD")
			.append("				INNER JOIN KSHST_SPEC_LEAVE_REST leaRe ON leaRe.CID = sphd.CID AND leaRe.SPHD_CD = sphd.SPHD_CD")
			.append("				LEFT JOIN KSHST_SPEC_CLS cls ON cls.CID = sphd.CID AND cls.SPHD_CD = sphd.SPHD_CD")
			.append("				LEFT JOIN BSYMT_CLASSIFICATION mcls ON mcls.CID = cls.CID AND mcls.CLSCD = cls.CLS_CD")
			.append("				LEFT JOIN KSHST_SPEC_EMP emp ON emp.CID = sphd.CID AND emp.SPHD_CD = sphd.SPHD_CD")
			.append("				LEFT JOIN BSYMT_EMPLOYMENT memp ON memp.CID = emp.CID AND memp.CODE = emp.EMP_CD")
			.append("				LEFT JOIN KSHST_GRANT_DATE_TBL graDa ON graDa.CID = sphd.CID AND graDa.SPHD_CD = sphd.SPHD_CD")
			.append("				LEFT JOIN KSHST_ELAPSE_YEARS ely ON ely.CID = graDa.CID AND ely.SPHD_CD = graDa.SPHD_CD AND ely.GD_CD = graDa.GD_TBL_CD")
		...
		.append(" (SELECT ")
		.append("		CASE WHEN TABLE_RESULT.ROW_NUMBER = 1 THEN TABLE_RESULT.DENSE_NUMBER")
		.append("		ELSE NULL")
		.append("		END DENSE_NUMBER,")
		...
		.append("		CASE WHEN TABLE_RESULT.ROW_NUMBER = 1 THEN ")
		.append("			CASE TABLE_RESULT.MAKE_INVITATION_ATR WHEN 0 THEN '－' WHEN 1 THEN '〇' END")
		.append("		ELSE NULL")
		.append("		END MAKE_INVITATION_ATR,")
		.append("		CASE WHEN TABLE_RESULT.ROW_NUMBER = 1 THEN ")
		.append("			CASE TABLE_RESULT.INCLUDE_HOLIDAYS_ATR WHEN 0 THEN '－' WHEN 1 THEN '〇' END")
		.append("		ELSE NULL")
		...
		.append("		CASE WHEN (TABLE_RESULT.MAX_NUMBER_DAYS_TYPE = 2) THEN ")
		.append("			CASE WHEN (TABLE_RESULT.MORNING_HOUR IS NULL) THEN NULL ELSE CONCAT(TABLE_RESULT.MORNING_HOUR,'日') END")
		.append("		ELSE NULL")
		.append("		END MORNING_HOUR")
		.append("	FROM	")
		.append(" 		(SELECT hdframe.NAME,holidayevent.MAX_NUMBER_DAYS_TYPE, holidayevent.FIXED_DAY_GRANT,")
		.append("				holidayevent.MAKE_INVITATION_ATR, holidayevent.INCLUDE_HOLIDAYS_ATR,  grantdayrelp.RELATIONSHIP_CD, ")
		.append("				rel.RELATIONSHIP_NAME,  grantdayrelp.GRANTED_DAY, grantdayrelp.MORNING_HOUR, ")
	...
	private MasterData toSPHDEventData(NtsResultRecord r){
		Map<String,MasterCellData> data = new HashMap<>();
		 data.put(SpecialHolidayUtils.KMF004_143, MasterCellData.builder()
					...
					.value(r.getString("DENSE_NUMBER"))
					.style(MasterCellStyle.build().horizontalAlign(ColumnTextAlign.RIGHT))
					.build());
