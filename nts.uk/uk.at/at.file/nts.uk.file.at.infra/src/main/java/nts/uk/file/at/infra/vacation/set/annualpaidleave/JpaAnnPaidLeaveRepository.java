...
public class JpaAnnPaidLeaveRepository extends JpaRepository implements AnnPaidLeaveRepository {
	...
	private static final String GET_PAID_LEA_EXPORT = "SELECT pl.MANAGE_ATR, mas.IS_WORK_DAY_CAL, "
			+ " mas.HALF_MAX_GRANT_DAY, mas.REMAINING_MAX_DAY," 
			+ "		mas.RETENTION_YEAR, mas.YEARLY_OF_DAYS,"
			+ "		mas.HALF_MANAGE_ATR, mas.HALF_MAX_REFERENCE, mas.HALF_MAX_UNIFORM_COMP,"
			+ "		tas.TIME_MANAGE_ATR, tas.TIME_UNIT, tas.TIME_MAX_DAY_MANAGE_ATR, tas.TIME_MAX_DAY_REFERENCE, tas.TIME_MAX_DAY_UNIF_COMP"
			+ " FROM KALMT_ANNUAL_PAID_LEAVE pl " 
	...
	private List<MasterData> buildMasterListData(ResultSet rs) {
		...
		try {
			...
			// Row 1
			datas.add(buildARow("残数管理", "", "", "", rs.getInt(1) == 0 ? "管理する" : "管理しない"));
			// Row 2
			datas.add(buildARow("", "年休の扱い", "出勤日数として加算", "", rs.getInt(2) == 0 ? "管理する" : "管理しない"));
			// Row 3
			datas.add(buildARow("", "", "上限日数設定", "付与上限日数", rs.getString(3)));
			// Row 4
			datas.add(buildARow("", "", "", "残数上限日数", rs.getString(4)));
			// Row 5
			datas.add(buildARow("", "", "保持できる年数", "", rs.getString(5)));
			// Row 6
			datas.add(buildARow("", "", "年間所定労働日数", "", rs.getString(6)));
			// Row 7
			datas.add(buildARow("", "", "半休の上限管理", "", rs.getInt(7) == 0 ? "管理する" : "管理しない"));
			// Row 8
			datas.add(buildARow("", "", "上限回数参照先", "", rs.getInt(8) == 0 ? "会社一律" : "年休付与テーブルを参照"));
			// Row 9
			datas.add(buildARow("", "", "", "会社の上限回数", rs.getString(9)));
