...
public class JpaBasicWorkRegisterReportRepository extends JpaRepository implements BasicWorkRegisterReportRepository {
	...
	private static final String SELECT_COMPANY_BASIC_WORK = "SELECT a.WORK_DAY_ATR, ISNULL(a.WORK_TYPE_CD, ''), "
			...
			+ " FROM KSCMT_COMPANY_WORK_SET a "
			+ " LEFT JOIN KSHMT_WORKTYPE b ON b.CD = a.WORK_TYPE_CD AND b.CID = a.CID"
			+ " LEFT JOIN KSHMT_WORK_TIME_SET c ON c.WORKTIME_CD = a.WORKING_CD AND c.CID = a.CID AND c.ABOLITION_ATR = 1"
	...
	private static final String GET_WORKSPACE_BASIC_WORK = "SELECT a.WKPID, "
			...
			+ "	FROM KSCMT_WORKPLACE_WORK_SET a "
	...
	private static final String GET_CLASS_BASIC_WORK = "SELECT a.CLSCD, a.CLSNAME, b.WORK_DAY_ATR, "
			...
			+ "	INNER JOIN KSCMT_CLASSIFY_WORK_SET b ON b.CID = a.CID and b.CLSCD = a.CLSCD"
			+ "	INNER JOIN KSHMT_WORKTYPE c ON c.CD =b.WORK_TYPE_CD and c.CID = b.CID"
			+ "	INNER JOIN KSHMT_WORK_TIME_SET d ON d.WORKTIME_CD = b.WORKING_CD AND d.CID = b.CID AND d.ABOLITION_ATR = 1"
