		...
		return this.queryProxy().query(SEL_ERROR_SETTING, KwrmtErAlWorkRecord.class)
				...
				.getList().stream().map(s -> {
					return new DPErrorSettingDto(s.kwrmtErAlWorkRecordPK.companyId,
							...
							s.typeAtr, s.krcmtErAlCondition == null ? "" : s.krcmtErAlCondition.messageDisplay, s.boldAtr == 1 ? true : false, s.messageColor,
							s.cancelableAtr == 1 ? true : false, s.errorDisplayItem == null ? null : s.errorDisplayItem.intValue());
				}).collect(Collectors.toList());
	}
