 ...
 */
@Stateless
public class RegisterStampInputWithEmployeeIdCommandHandler extends
		CommandHandlerWithResult<RegisterStampInputCommandWithEmployeeId, RegisterStampInputResultWithEmployeeId> {
	...
	@Override
	protected RegisterStampInputResultWithEmployeeId handle(
			CommandHandlerContext<RegisterStampInputCommandWithEmployeeId> context) {
		RegisterStampInputCommandWithEmployeeId cmd = context.getCommand();
		...
		String employeeID = cmd.getEmployeeId();
		...
		// note: 作成する(@Require, 契約コード, 社員ID, 日時, 打刻ボタン位置NO, 実績への反映内容)
		TimeStampInputResult inputResult = EnterStampFromPortalService.create(require, contractCode, employeeID,
				datetime, buttonPositionNo, refActualResults);

		if (inputResult == null) {
			return null;
		}

		// note: not 打刻入力結果 empty
		...
		// note: 打刻入力から日別実績を作成する (Tạo thực tế hàng ngày từ input check tay)
		CreateDailyResultsStampsRequireImpl resultStampRequire = new CreateDailyResultsStampsRequireImpl(topPageRepo, empManaAdapter);

		CreateDailyResultsStamps.create(resultStampRequire, cmd.getCompanyId(), employeeID, refDateOpt);

		return new RegisterStampInputResultWithEmployeeId(refDateOpt.map(m -> m).orElse(null));
	}
