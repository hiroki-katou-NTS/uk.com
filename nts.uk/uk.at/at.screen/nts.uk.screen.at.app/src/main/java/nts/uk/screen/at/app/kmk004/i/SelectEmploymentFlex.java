 ...
 * @author sonnlb
 *
 *         UKDesign.UniversalK.就業.KDW_日別実績.KMK_計算マスタ.KMK004_法定労働時間の登録（New）.I：雇用別法定労働時間の登録（フレックス勤務）.メニュー別OCD.雇用を選択する（フレックス勤務）.雇用を選択する（フレックス勤務）
 */
@Stateless
public class SelectEmploymentFlex {
	...
	public SelectEmploymentFlexDto selectEmploymentFlex(String employmentCd, LaborWorkTypeAttr workTypeAttr) {
		...
		// 2. 雇用別年度リストを表示する
		result.setYears(this.yearListByEmployment.get(employmentCd, workTypeAttr));
