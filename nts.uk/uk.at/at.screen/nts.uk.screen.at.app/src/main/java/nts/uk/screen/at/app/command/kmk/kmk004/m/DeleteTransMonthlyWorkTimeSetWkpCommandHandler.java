...
/**
 * UKDesign.UniversalK.就業.KDW_日別実績.KMK_計算マスタ.KMK004_法定労働時間の登録（New）.M：職場別法定労働時間の登録（変形労働）.メニュー別OCD.職場別月単位労働時間（変形労働）を削除する
 * 
 * @author tutt
 *
 */
@Stateless
public class DeleteTransMonthlyWorkTimeSetWkpCommandHandler
		extends CommandHandlerWithResult<DeleteTransMonthlyWorkTimeSetWkpCommand, List<WorkplaceIdDto>> {
	...
	public List<WorkplaceIdDto> handle(CommandHandlerContext<DeleteTransMonthlyWorkTimeSetWkpCommand> context) {
		...
		this.deleteHandler.handle(
				new DeleteMonthlyWorkTimeSetWkpCommand(cmd.getWorkplaceId(), LaborWorkTypeAttr.DEFOR_LABOR.value,
						new YearMonthPeriodCommand(yearMonths.start().v(), yearMonths.end().v())));

		// 3: <call>
		// Input:勤務区分＝1：変形労働
		return this.workplaceList.get(LaborWorkTypeAttr.DEFOR_LABOR);
	}
