...
public class DisplayInWorkInformation {
	...
	public DisplayInWorkInfoResult getData(DisplayInWorkInfoParam param) {
		...
		for (int i = 0; i < listWorkTypeDto.size(); i++) {
			...
			// 1日半日出勤・1日休日系の判定 - (Thực hiện thuật toán [Kiểm tra hệ thống đi làm nửa ngày・ nghỉ cả ngày ])
		...
		for (String sid : sids) {
			for (int i = 1; i < 32; i++) {
				WorkScheduleWorkInforDto w1 = new WorkScheduleWorkInforDto(sid, GeneralDate.ymd(2020, 7, i), 
						false, false, false, false , 1, 
						"001 ", "出勤 ", new EditStateOfDailyAttdDto(1,0),
						"001 ", "テスト固定 ",new EditStateOfDailyAttdDto(1,0), 
						480, new EditStateOfDailyAttdDto(1,0),
						1020, new EditStateOfDailyAttdDto(1,0), 1);
