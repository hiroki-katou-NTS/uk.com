...
public class GetCompanyAndWorkInfo {
	...
	public CompanyAndWorkInfoOutput get(String contractCd,List<WorkplacePossibleCmd> listCidAndWorkplace,List<String> listCid) {
		...
		for(String cid :listCid) {
			List<String> listWorkplaceId = listCidAndWorkplace.stream().filter(c->c.getCompanyId().equals(cid)).map(c->c.getWorkpalceId()).collect(Collectors.toList());
			List<WorkplaceInfoImport> listWorkplaceInfo = workplaceAdapter.getWorkplaceInfoByWkpIds(cid,listWorkplaceId, GeneralDate.today());
