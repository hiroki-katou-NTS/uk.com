...
export class ApprovedComponent extends Vue {
    ...
    public mounted() {
        let self = this,
            buttons: Element[] = [].slice.call(self.$el.querySelectorAll('.row>.col:first-child>button'));
        ...
        if (buttons.length == 5) {
            buttons.forEach((btn: HTMLButtonElement, i: number) => {
                ...
                if (i !== 0) {
                    let cont: HTMLDivElement = self.$el.querySelector(`.row>.col:nth-child(${i + 1})`);
    ...
    public updated() {
        let self = this,
            buttons: Element[] = [].slice.call(self.$el.querySelectorAll('.row>.col button'));
