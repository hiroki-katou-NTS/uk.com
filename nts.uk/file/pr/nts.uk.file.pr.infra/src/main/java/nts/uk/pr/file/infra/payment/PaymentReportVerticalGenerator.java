 ...
 */
public class PaymentReportVerticalGenerator implements PaymentGenerator {
	...
	public void generate(AsposeCellsReportContext context, PaymentReportData data) {
		 ...
		 * 
		 * // Set worksheet name. Worksheet ws =
		 * context.getWorkbook().getWorksheets().get(0);
		 * ws.setName(data.getName());
		 * 
		 * // Set data. Cells cells = ws.getCells(); ElementSettingDto
		 * elementSettingDto = history.getElements().get(0); cells.get(0,
		 * 0).setValue(elementSettingDto.getDemensionName()); cells.get(0,
		 * 1).setValue("値");
		 * 
