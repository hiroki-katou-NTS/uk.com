...
public class PaymentSalaryReportService extends ExportService<PaymentSalaryQuery> {
	...
	//
	// PaymentSalaryPrintSettingDto configure = new
	// PaymentSalaryPrintSettingDto();
	// configure.setSelectedLevels(selectedLevels);
	// rawData.setConfigure(configure);
	//
	...
	private List<EmployeeDto> fakeEmployees(int numberOfEmployee) {
		...
		List<Integer> yearMonths = Arrays.asList(201601);
	...
	private Map<EmployeeKey, Double> fakeMapAmount(int numberOfEmployee, List<EmployeeDto> employees) {
		...
		Map<EmployeeKey, Double> mapPayment = fakeCategoryItem(SalaryCategory.Payment, numberOfEmployee,
			employees);
		mapEmployeeAmount.putAll(mapPayment);

		// Map<EmployeeKey, Double> mapDeduction =
		// fakeCategoryItem(SalaryCategory.Deduction, numberOfEmployee,
		// employees);
		// mapEmployeeAmount.putAll(mapDeduction);
		//
		// Map<EmployeeKey, Double> mapArticleOther =
		// fakeCategoryItem(SalaryCategory.ArticleOthers, numberOfEmployee,
		// employees);
