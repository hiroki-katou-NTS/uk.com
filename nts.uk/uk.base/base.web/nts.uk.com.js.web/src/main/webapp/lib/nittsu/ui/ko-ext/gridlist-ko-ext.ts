...
module nts.uk.ui.koExtentions {
    ...
    export module gridlist {
         ...
         */
        @handler({
            bindingName: 'ntsGridList'
        })
        export class NtsGridListBindingHandler implements KnockoutBindingHandler {
            init(element: HTMLElement, valueAccessor: () => any, allBindingsAccessor: () => any, viewModel: any, bindingContext: KnockoutBindingContext): void {
                ...
                const $grid = $(element).addClass("nts-gridlist");
                ...
                $grid.data("tabindex", _.isNil(tabIndex) ? "0" : tabIndex);
                    ...
                    if (c["controlType"] === "switch") {
                        ...
                        if (!_.isNil(switchF)) {
                            ...
                            c["formatter"] = function createButton(val, row) {
                                ...
                                let rVal = _.isNil(formatter) ? val : formatter(val, row);
                        ...
                        c.formatter = function (val, row) {
                            ...
                            return _.isNil(formatter) ? val : formatter(val, row);
                        };
                    }
                    return c;
                });

                let isDeleteButton = !_.isNil(deleteOptions) && !_.isNil(deleteOptions.deleteField)
                ...
                if (data.multiple) {
                    $grid.bind('iggridrowselectorscheckboxstatechanging', (eventObject: JQueryEventObject, data: any) => {
                        ...
                        let disables = $grid.data("selectionDisables");
                        if (disables && !_.isNil(_.find(disables, d => data.rowKey === d))) {
                            return false;
                        }
                        return true;
                    });
                ...
                $grid.bind('iggridselectionrowselectionchanging', (eventObject: JQueryEventObject, ui: any) => {
                    if (String($grid.data("enable")) === "false") return false;
                    let disables = $grid.data("selectionDisables");
                    if (disables && _.isNil(ui.startIndex)
                        && !_.isNil(_.find(disables, d => ui.row.id === d))) {
                        return false;
                    }
                    ...
                    if (data.multiple) {
                        ...
                        if (disables) {
                            _.forEach(selected, (s, i) => {
                                _.forEach(disables, (d) => {
                                    if (d === s.id && _.isNil(_.find(value, iv => iv === d))) {
                ...
                if (!isEqual) {
                    ...
                    if (!_.isNil(data.value())) {
                        ...
                        if (isSameSource && data.value().length == sources.length && $grid.igGridSelection('option', 'multipleSelection')) {
                            ...
                            clickCheckBox = !_.isNil(features.enableCheckBoxes) && features.enableCheckBoxes;
                        }
                    }
                    if (clickCheckBox) {
