...
public class JogmtGuideDispSetting extends UkJpaEntity implements Serializable {
	...
	public List<JogmtGuideMsg> guideMsgList;
	...
	public Guidance toDomain() {
		return Guidance.createFromJavaType(
				...
				this.guideMsgMaxNum, 
				this.guideMsgList.stream().map(c -> c.toDomain()).collect(Collectors.toList()));
	}
	...
	public JogmtGuideDispSetting(Guidance domain) {
		...
		this.guideMsgList = domain.getGuideMsg().stream().map(c -> new JogmtGuideMsg(domain.getCompanyId(), c)).collect(Collectors.toList());
	}
