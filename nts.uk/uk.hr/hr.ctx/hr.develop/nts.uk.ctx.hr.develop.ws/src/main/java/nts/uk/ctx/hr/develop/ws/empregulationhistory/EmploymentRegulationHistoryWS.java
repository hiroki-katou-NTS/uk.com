...
public class EmploymentRegulationHistoryWS {
	...
	public JavaTypeResult<String> saveDateHistoryItem(DateHistoryItemDto dto){
		return new JavaTypeResult<String>(command.add(dto));
	}
	
	@POST
	@Path("/updateDateHistoryItem")
	public void updateDateHistoryItem(DateHistoryItemDto dto){
		command.update(dto);
	}
	
	@POST
	@Path("/removeDateHistoryItem")
	public void removeDateHistoryItem(DateHistoryItemDto dto){
		command.remove(dto);
	}
