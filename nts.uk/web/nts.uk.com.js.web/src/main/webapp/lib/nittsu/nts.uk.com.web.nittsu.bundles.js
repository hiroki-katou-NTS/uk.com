    ...
    (function (uk) {
        ...
        (function (text_3) {
            ...
            function toOneByteAlphaNumberic(text) {
                return text.replace(/[??-?û„??]/g, function (s) {
                    if (s === "„Ä?") {
            ...
            function katakanaToHiragana(text) {
                text = text.replace(/[„Ç°-„É¥]/g, function (s) {
                    return String.fromCharCode(s.charCodeAt(0) - 0x60);
                }).replace(/??/g, '„Ç?').replace(/??/g, '„Ç?')
                    .replace(/(„Å??)/g, '„Ç?').replace(/„É∑/g, '„Çè„?')
                    .replace(/„É∏/g, '„Çê„?').replace(/„Éπ/g, '„Çë„?')
                    .replace(/„É∫/g, '„Çí„?').replace(/(„ÉΩ„Ç?)/g, '„Ç?')
                    .replace(/„ÉΩ/g, '„Ç?').replace(/„Éæ/g, '„Ç?');
                return text;
            }
            text_3.katakanaToHiragana = katakanaToHiragana;
            function hiraganaToKatakana(text, opt) {
                text = text.replace(/[„Å?-„Çî]/g, function (s) {
                    return String.fromCharCode(s.charCodeAt(0) + 0x60);
                }).replace(/??/g, '„Ç?').replace(/??/g, '„Ç?')
                    .replace(/(„Ç¶„Ç?)/g, '„É¥').replace(/(„ÉØ„Ç?)/g, '„É∑')
                    .replace(/(„É∞„Ç?)/g, '„É∏').replace(/(„É±„Ç?)/g, '„Éπ')
                    .replace(/(„É≤„Ç?)/g, '„É∫').replace(/(„Çù„?)/g, '„Éæ')
                    .replace(/„Ç?/g, '„ÉΩ').replace(/„Ç?/g, '„Éæ');
                if (opt !== false) {
                    text = text.replace(/„Ç?/g, '„Éµ').replace(/„Ç?/g, '„É∂');
                }
                return text;
            }
             ...
             *
             * @param {String} str Â§âÊèõ„Åó„Åü„Å?ñáÂ≠óÂ?
             */
            function oneByteKatakanaToTwoByte(text) {
                var katakanaMap = {
                    '?∂??': '„Ç¨', '?∑??': '„ÇÆ', '?∏??': '„Ç∞', '?π??': '„Ç≤', '?∫??': '„Ç¥',
                    '?ª??': '„Ç∂', '?º??': '„Ç∏', '?Ω??': '„Ç∫', '?æ??': '„Çº', '?ø??': '„Çæ',
                    '????': '„É?', '??æ?': '„É?', '?ÇÔæ?': '„É?', '??æ?': '„É?', '??æ?': '„É?',
                    '?äÔæ?': '„É?', '?ãÔæ?': '„É?', '?åÔæ?': '„É?', '?çÔæ?': '„É?', '?éÔæ?': '„É?',
                    '?äÔæ?': '„É?', '?ãÔæ?': '„É?', '?åÔæ?': '„É?', '?çÔæ?': '„É?', '?éÔæ?': '„É?',
                    '?≥??': '„É¥', '?úÔæ?': '„É∑', '?¶??': '„É∫',
                    '?±': '„Ç¢', '?≤': '„Ç§', '?≥': '„Ç¶', '?¥': '„Ç®', '?µ': '„Ç™',
                    '?∂': '„Ç´', '?∑': '„Ç≠', '?∏': '„ÇØ', '?π': '„Ç±', '?∫': '„Ç≥',
                    '?ª': '„Çµ', '?º': '„Ç∑', '?Ω': '„Çπ', '?æ': '„Çª', '?ø': '„ÇΩ',
                    '??': '„Çø', '??': '„É?', '??': '„É?', '??': '„É?', '??': '„É?',
                    '??': '„É?', '??': '„É?', '??': '„É?', '??': '„É?', '??': '„É?',
                    '??': '„É?', '??': '„É?', '??': '„É?', '??': '„É?', '??': '„É?',
                    '??': '„É?', '??': '„É?', '??': '„É?', '??': '„É°', '??': '„É¢',
                ...
                var StringValidator = (function () {
                    ...
                    StringValidator.prototype.validate = function (inputText, option) {
                        ...
                        if (this.charType !== null && this.charType !== undefined) {
                            if (this.charType.viewName === 'ÂçäËßíÊï∞Â≠?' || this.charType.viewName === 'ÂçäËßíËã±Êï∞Â≠?') {
                                inputText = uk.text.toOneByteAlphaNumberic(inputText);
                            }
                            else if (this.charType.viewName === '„Ç´„Çø„Ç´„É?') {
            ...
            (function (option_1) {
                ...
                var DelDialogOption = (function (_super) {
                    ...
                    function DelDialogOption(option) {
                        ...
                        // Add Cancel Button
                        _this.buttons.push({ text: "„Å?ÅÑ„Å?",
                            "class": "no ",
                            size: "large",
                            color: "",
                            click: function (viewmodel, ui) {
                ...
                var ErrorDialogOption = (function (_super) {
                    ...
                    function ErrorDialogOption(option) {
                        ...
                        _this.buttons = [];
                        // Add Close Button
                        _this.buttons.push({ text: "Èñâ„Åò„Ç?",
                            "class": "yes ",
                            size: "large",
                            color: "",
                            click: function (viewmodel, ui) {
                    ...
                    NtsSwapListBindingHandler.prototype.init = function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
                        ...
                        if (!uk.util.isNullOrUndefined(showSearchBox) && (showSearchBox.showLeft || showSearchBox.showEright)) {
                            var initSearchArea = function ($SearchArea, targetId) {
                                ...
                                $SearchArea.find(".ntsSearchInput").attr("placeholder", "„Ç≥„Éº„Éâ„?ÂêçÁß∞„ÅßÊ§úÁ¥¢„Éª„Éª„Éª");
                                $SearchArea.find(".ntsSearchButton").text("Ê§úÁ¥¢");
                            };
