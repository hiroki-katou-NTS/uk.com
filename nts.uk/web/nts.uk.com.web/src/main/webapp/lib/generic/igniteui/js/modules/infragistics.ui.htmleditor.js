...
/*!@license
 * Infragistics.Web.ClientUI HtmlEditor 16.1.20161.2145
 *
 * Copyright (c) 2011-2016 Infragistics Inc.
 * <Licensing info>
 *
    ...
    */
    $.widget("ui.igHtmlEditor", {
        ...
        css: {
            ...
            htmlEditor: "ui-widget ui-widget-content ui-corner-all " +
                "ui-ightmleditor ui-helper-clearfix",
        ...
        _render: function () {
            ...
            html += '<div id="' + this._id("_content") +
                '" class="' + this.css.htmlEditorContent + '">';
            html += '<iframe class="' + this.css.workspaceIframe +
                '" id="' + this._id("_editor") + '"></iframe>';
        ...
        _initFontsCombos: function () {
            ...
            fontFamiliesCombo.igCombo("option", {
                ...
                // Combo bug #113720
                itemTemplate: '<span style="font-family: ${value}" ' +
        ...
        _initFormatsListCombo: function () {
            var formatsListCombo =
                this._getToolbar("textToolbar").igToolbar("getItem", "formatsList");
        ...
        _bindToToolbarEvents: function () {
            var self = this,
                events = "igtoolbartoolbarbuttonclick igtoolbartoolbarcomboselected " +
                    ...
                    "igtoolbarexpanding igtoolbarexpanded",
        ...
        _addTableRowPlg: function () {
            ...
            if (selectedItem.is("table")) {
                ...
                selectedItem.parents("table").
                    first().tableManipulator("addRow", selectedItem.parent().index());
            }
        },
        _removeTableRowPlg: function () {
            ...
            if (selectedItem.is("table")) {
                ...
                selectedItem.parents("table").
                    first().tableManipulator("removeRow", selectedItem.parent().index());
            }
        },
        _addTableColumnPlg: function () {
            ...
            if (selectedItem.is("table")) {
                ...
                selectedItem.parents("table").
                    first().tableManipulator("addColumn", selectedItem.index());
            }
        },
        _removeTableColumnPlg: function () {
            ...
            if (selectedItem.is("table")) {
                ...
                selectedItem.parents("table").
                    first().tableManipulator("removeColumn", selectedItem.index());
            }
        },
        _fontColorPlg: function (workspace, ui) {
            // R.K. August 25th, 2016 Bug #224051: Colored text with <font color="rgb(X, X, X)"> on IE doesn't work on Chrome
    ...
    $.widget("ui.igPathFinder", {
        ...
        css: {
            button: "ui-button ui-widget ui-state-default ui-corner-all " +
                "ui-button-text-only ui-igbutton ui-widget-content",
        ...
        _updateToolbar: function () {
            ...
            var i,
                ...
                maxWidth = viewSrcBtn.parent().width() -
                    viewSrcBtn.outerWidth(true) - overflowMarkerWidth,
                maxWidthReached = false;

            if (o.items.length) {
                o.items.each(function (index, element) {
                    html = '<div aria-disabled="false" style="z-index:' + index +
                        '" class="' + self.css.button + '">' +
   ...
   ************************************/
    $.widget("ui.igLinkPropertiesDialog", $.ui.igHtmlEditorPopover, {
        ...
        _createForm: function () {
            ...
            html += "<div>" +
                '<ol class="layoutList">' +
                    "<li>" +
                        '<label for="' + this._id("_linkHref") + '">' +
                            $.ig.HtmlEditor.locale.linkNavigateToUrlDialogText + "</label>" +
                        '<input autocomplete="off" id="' +
                            this._id("_linkHref") + '" name="href" type="text" value="" />' +
                    "</li>" +
                    "<li>" +
                        '<label for="' + this._id("_linkDisplayText") + '">' +
                            $.ig.HtmlEditor.locale.linkDisplayTextDialogText + "</label>" +
                        '<input autocomplete="off" id="' +
                            this._id("_linkDisplayText") + '" name="href" type="text" value="" />' +
                    "</li>" +
                    "<li>" +
                        '<label for="' + this._id("_cmbOpenIn") + '">' +
                            $.ig.HtmlEditor.locale.linkOpenInDialogText + "</label>" +
                        '<select id="' + this._id("_cmbOpenIn") + '" name="target">' +
                            '<option value="_blank">' +
                                $.ig.HtmlEditor.locale.linkTargetNewWindowDialogText + "</option>" +
                            '<option value="_self">' +
                                    $.ig.HtmlEditor.locale.linkTargetSameWindowDialogText +
                                "</option>" +
                            '<option value="_parent">' +
                                    $.ig.HtmlEditor.locale.linkTargetParentWindowDialogText +
                                "</option>" +
                            '<option value="_top">' +
                                    $.ig.HtmlEditor.locale.linkTargetTopmostWindowDialogText +
                                "</option>" +
                        "</select>" +
                    "</li>" +
                    '<li style="text-align:right">' +
                        '<button aria-disabled="false" id="' +
                            this._id("_btnApply") + '" name="insertLink" role="button" title="' +
                            $.ig.HtmlEditor.locale.applyButtonTitle + '" type="button">' +
                                $.ig.HtmlEditor.locale.applyButtonTitle + "</button>" +
                        '<button aria-disabled="false" id="' +
                            this._id("_btnCancel") + '" role="button" title="' +
                            $.ig.HtmlEditor.locale.cancelButtonTitle + '" type="button">' +
                                $.ig.HtmlEditor.locale.cancelButtonTitle + "</button>" +
                    "</li>" +
                "</ol>" +
            "</div>";
        ...
        _dataBind: function (anchor) {
            this._txtUrl = this.poContent.find("#" +
                this._id("_linkHref")).val(anchor.attr("href"));
            this._cbTarget = this.poContent.find("#" +
                this._id("_cmbOpenIn")).val(anchor.attr("target"));
            this._displayText = this.poContent.find("#" +
                this._id("_linkDisplayText")).val(anchor.html());
        }
    });
    ...
    ************************************/
    $.widget("ui.igTablePropertiesDialog", $.ui.igHtmlEditorPopover, {
        ...
        _createForm: function () {
                        ...
                        "<li>" +
                            '<div class="ui-igtablepropertiesdialog-cols-rows-num">' +
                                '<input type="hidden" id="' +
                                    this._id("_tableRows") +
                                        '" name="tableRows" readonly="readonly" value="">' +
                                '<input type="hidden" id="' +
                                    this._id("_tableColumns") +
                                        '" name="tableColumns" readonly="readonly" value="">' +
                            "</div>" +
                        "</li>" +
                    "</ol>" +
                "</div>";
            ...
            this._sampleTable = $('<table "' +
                this._id("_tableModel") +
        ...
        _attachFormEvents: function () {
            ...
            tablePreview.delegate("td", "mouseover", function (e) {
                ...
                for (i = 0; i < currentRowNumber; i++) {
                    $(rows.get(i)).find("td:lt(" +
                        currentColumnNumber + ")").addClass("ui-state-hover");
                }
            ...
            tablePreview.delegate("td", "click", function (e) {
                ...
                self._item.tableManipulator({ rows: currentRowNumber,
                        columns: currentColumnNumber, addSpacingChar: true });
    ...
    ************************************/
    $.widget("ui.igImagePropertiesDialog", $.ui.igHtmlEditorPopover, {
        ...
        _createForm: function () {
            ...
            html += "			<li>";
            html += '				<label for="' + this._id("_imgSrc") + '">' +
                            $.ig.HtmlEditor.locale.imageUrlDialogText + "</label>";
            html += '				<input autocomplete="off" id="' + this._id("_imgSrc") +
                                        '" name="src" type="text" value="" />';
            html += "			</li>";
            html += "			<li>";
            html += '				<label for="' + this._id("_imgAlt") + '">' +
                                        $.ig.HtmlEditor.locale.imageAlternativeTextDialogText +
                                        "</label>";
            html += '				<input autocomplete="off" id="' + this._id("_imgAlt") +
                                        '" name="altText" type="text" value="" />';
            html += "			</li>";
            html += '			<li style="text-align:right">';
            html += '				<button aria-disabled="false" id="' +
                                        this._id("_btnApply") +
                                        '" name="insertLink" role="button" title="' +
                                        $.ig.HtmlEditor.locale.applyButtonTitle +
                                        '" type="button">' +
                                        $.ig.HtmlEditor.locale.applyButtonTitle + "</button>";
            html += '				<button aria-disabled="false" id="' +
                                        this._id("_btnCancel") +
                                        '" role="button" title="' +
                                        $.ig.HtmlEditor.locale.cancelButtonTitle +
                                        '" type="button">' +
                                        $.ig.HtmlEditor.locale.cancelButtonTitle + "</button>";
            html += "			</li>";
            html += "		</ol>";
    ...
    ************************************/
    $.ig.SelectionWrapper = $.ig.SelectionWrapper || Class.extend({
        ...
        _document: null,
        _commands: {
            insertunorderedlist: { name: "_insertList",
                browsers: null, args: [ "insertunorderedlist" ] },
            insertorderedlist: { name: "_insertList",
                browsers: null, args: [ "insertorderedlist" ] }
        },
        _css: {
    ...
    $.ig.ToolbarHelper = $.ig.ToolbarHelper || Class.extend({
        ...
        analyse: function (el) {
            ...
            $.each(this._callbackMap, function (isTrueFunc, callback) {
                ...
                if ($.isFunction(self[ isTrueFunc ]) &&
                    isTrueRes && $.isFunction(self[ callback ])) {
        ...
        _onLineThrough: function () {
            this._toolbars.textToolbar.
                igToolbar("getItem", "Strikethrough").igToolbarButton("toggle");
        },
        ...
        _onList: function (el, listType) {
            this._toolbars.formattingToolbar.
                igToolbar("getItem", listType).igToolbarButton("toggle");
        },
        ...
        _onHeader: function (element) {
            ...
            this._setComboValue(combo,
                $.ig.HtmlEditor.locale.formatsList, element[ 0 ].nodeName.toLowerCase());
        },
        _onTable: function () {
    ...
    ************************************/
    ...
    $.extend($.ui.igHtmlEditor, { version: "16.1.20161.2145" });
}(jQuery));
