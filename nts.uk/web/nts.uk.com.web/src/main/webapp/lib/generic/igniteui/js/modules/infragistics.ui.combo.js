...
/*!@license
* Infragistics.Web.ClientUI Combo 16.1.20161.2145
*
* Copyright (c) 2011-2016 Infragistics Inc.
*
* http://www.infragistics.com/
...
(function ($) {
	...
	*/
    $.widget("ui.igCombo", {
        ...
        _render: function () {
            var css = this.css,
				...
				$input = (_options.$input || $('<input type="text">')).addClass(css.field)
        ...
        _convertToArrayOfObjects: function (options) {
            ...
            if ($.type(options.dataSource) === "object" &&
                (options.dataSource._data !== null && options.dataSource._data !== undefined)) {
            ...
            if (this._options.dropDownOpened) {
                if ($item.length > 0) {
                    ...
                    if (!(itemTop > listContTop &&
                        ...
                        this.listScrollTop(itemTop + listContScrollTop +
                            itemHeight - listContTop - listContHeight);
                    }
        ...
        _handleKeyNavigation: function (event) {
            ...
            if (event.keyCode === $.ui.keyCode.ENTER ||
                ...
                if (event.shiftKey && _options.$itemsToSelectOnShiftUpDown.length === 1) {
                    ...
                    multiSelect = multiSelection && (!options.multiSelection.addWithKeyModifier ||
                        event.ctrlKey || isAutoSelectedActive);
                ...
                if ((event.keyCode === $.ui.keyCode.ENTER &&
                    options.preventSubmitOnEnter) || event.keyCode === $.ui.keyCode.SPACE) {
                    event.preventDefault();
                }
            }
        ...
        _inputBlur: function (event) {
            ...
            if (!_options.preventInputBlur) {
                ...
                if (this.options.mode === "editable") {
                    ...
                    // R.K. 15th September 2016, Bug #225401: igCombo allows you to keep unlisted value
        ...
        _handleDropDownModeKeypress: function (event) {
            ...
            if (options.multiSelection.enabled) {
                ...
                startValue =
                    _options.selectedData[ _options.selectedData.length - 1 ][ options.valueKey ];
            }
        ...
        _attachEvents: function () {
            ...
            if (_options.$dropDownScrollCont) {
                ...
                _options.$dropDownListCont.on("touchmove", function (event) {
                    ...
                    // R.K. 22/08/2016 #221998: igCombo scrolling not working properly on ipad when virtualization is on
                    // Scrolling direction was reversed compared to normal select scroll on touch devices
                    if (currentY < lastY) {
                        self.listScrollTop(currentScrollTop + itemHeight);
        ...
        _offsetItems: function (dataView, itemHeight) {
            ...
            containerRatio =
                this.listScrollTop() / this._options.$dropDownScrollCont.prop("scrollHeight");
        ...
        _initDataSource: function () {
            ...
            if (!(options.dataSource && this._isInstanceOfDataSource(options.dataSource))) {
                ...
                if (!schema && options.dataSource &&
                    ($.isArray(options.dataSource) || $.isFunction(options.dataSource))) {
                ...
                options.dataSource._responseData = function (data) {
                    ...
                    if (count) {
                        data.pop();
                        count = count.split(":");
                        options.dataSource.totalRecordsCount(_aNull(len =
                            parseInt(count[ 0 ], 10), true) ? 0 : len);
        ...
        _generateExpressions: function (texts) {
            ...
            if ($.type(texts) === "string") {
                ...
                if (texts.length > 0 ||
                    (this._options.expression && this._options.expression.length > 0)) {
                    // R.K. July 13th 2016 Bug #220867 Only lowercase items returned by igCombo filtering when filtering by turkish symbol ı
                ...
                highlighMatches = function () {
                    return (this.nodeValue || "")
                        .replace(regex, function (match) {
                            return "<" + highlightElement + " class=\"" +
                                highlightCssClass + "\">" + match + "</" + highlightElement + ">";
                        });
                };

                for (i = 0, len = $items.length; i < len; i++) {
                    if (this._checkBoxesEnabled()) {
                        $curContents = $items.eq(i).
                            children("." + this.css.listItemTextWithCheckbox).contents();
                    } else {
        ...
        _handleLocalFiltering: function (dataSource) {
            var $curItem, curKey, filterItem, dataLen, i, j, unwrappedDataViewItem,
                unwrappedDataViewValue, $noMatchFoundItem,
                _options = this._options,
                valKey = this.options.valueKey,
        ...
        _dropDownOrientation: function () {
            ...
            dropDownAndComboHeight =
                parseInt((comboTopOffset + comboOuterHeight + dropDownContainerHeight), 10);
                        ...
                        .animate(newAnimationStyle, {
                            ...
                            complete: function () {
                                ...
                                if (self.options.virtualization) {
                                    itemHeight = self._itemHeight();
                                    offset = self._offsetItems(self.options.dataSource.dataView(),
                                        itemHeight);
        ...
        closeDropDown: function (callback, event) {
            ...
            */
            ...
            if (_options.dropDownOpened &&
                (!_options.dataBinding || _options.closingDropDownOnBlur)) {
        ...
        isIndexSelected: function (index) {
            /* Verifies whether the li representing the data source's record at the specified index is selected
                ...
                returnType="bool" Returns boolean representing whether the item is selected
            */
            return this.isValueSelected(this.options.dataSource.
                data()[ index ][ this.options.valueKey ]);
        },
        ...
        _selectData: function (data, options, event) {
            /* Selects list item/items from the drop down list by specified value or array of values. When called witout params will return the value of the selected item or if multiple selection is enabled array of selected values.
                ...
                returnType="object" Returns reference to this igCombo.
            */
            var items, itemsLen, selectedValues, newSelItems, selAutoSelectedItem, selChanged,
                additive, prevSelValues, newSelData, skipEventTrigger, noCancel, i,
            ...
            if (additive) {
				...
				// Z.K. June 30, 2016 #17 not() behavior is changed in jQuery 3.0.0, #225719 - Can't select an item from  drop down list when using jQuery 3
            ...
            if (selChanged || selAutoSelectedItem) {
                ...
                if (noCancel) {
                    ...
                    if (prevSelItems && prevSelItems.length > 0 && !additive) {
                        this._removeItemSelectionStyles(this._$elementsFromItems(this.
                            _filterItems(prevSelItems, items)));
                    }
        ...
        _deselectData: function (data, options, event) {
            ...
            */
            ...
            if (selChanged) {
                ...
                // Z.K March 14, 2016 - Fixing Bug #215857 originalEvent and keyCode are missing from event parameter on selectionChanging event
                noCancel = event ?
                    this._triggerSelectionChanging(newSelItems, prevSelItems, event) : true;
    ...
    $.extend($.ui.igCombo, { version: "16.1.20161.2145" });
}(jQuery));
