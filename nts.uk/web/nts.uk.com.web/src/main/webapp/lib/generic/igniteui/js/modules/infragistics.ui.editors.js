...
/*!@license
 * Infragistics.Web.ClientUI CountDown 16.1.20161.2145
 *
 * Copyright (c) 2011-2016 Infragistics Inc.
 *
 * http://www.infragistics.com/
...
(function ($) {
	...
	$.widget("ui.igBaseEditor", {
		...
		_setFocus: function (event) {
			// D.P. 26th Aug 2016 Bug 221854: Can't right-click paste in Edge, double focus event on menu closing
			if (this._focused) {
				return;
			}
	...
	$.extend($.ui.igBaseEditor, { version: "16.1.20161.2145" });
			...
			if (noCancel) {
				...
				if (e.keyCode === 13) {
					if (event.altKey && this.options.textMode === "multiline") {
						...
						// N.A. 7/8/2016 Bug #221300: Carry over the word on new line and move the cursor there.
	...
	$.widget("ui.igDatePicker", $.ui.igDateEditor, {
		...
		_toggleDropDown: function () {
			...
			if (this._dropDownList.is(":visible") &&
				(!!this._focused || this.options.readOnly) && !!this._dropDownOpened) {
					// Proceed with hiding
					this._hideDropDownList();
			} else {
