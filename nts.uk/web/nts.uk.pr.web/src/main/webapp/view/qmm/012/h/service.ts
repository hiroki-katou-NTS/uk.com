module qmm012.h {
    export module service {
        ...
        export function findItemDeductPeriod(itemCode): JQueryPromise<model.ItemDeductPeriod> {
            var dfd = $.Deferred<model.ItemDeductPeriod>();
            nts.uk.request.ajax(paths.findItemDeductPeriod + "/" + itemCode)
                .done(function(res: model.ItemDeductPeriod) {
                    dfd.resolve(res);
                })
                .fail(function(res) {
                    dfd.reject(res);
