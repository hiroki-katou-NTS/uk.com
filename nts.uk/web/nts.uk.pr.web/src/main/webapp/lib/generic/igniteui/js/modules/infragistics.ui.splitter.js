/*!@license
 * Infragistics.Web.ClientUI Splitter 16.1.20161.2145
 *
	...
	*/
    $.widget("ui.igSplitter", {
        _const: {
            ...
            properties: [ "max", "_max", "min", "_min",
                "size", "collapsed", "collapsible", "resizable" ],
            step: 10,
            touchEvents: {
        ...
        css: {
            ...
            horizontalCollapseButtonRightCollapsed:
        ...
        _render: function () {
            ...
            var panels =
                $(this.element.children("div")), panel, self =
                    this, reducedSize, defaultSize = 0, j, elementSize;
            reducedSize = this._reducedSize();
            if (this.options[ this._getOrientation("size") ]) {
                //D.U. 8st July 2014 default size always NaN while width or height was set with percentage.
                elementSize =
                    this._getOrientation("size") === "width" ?
                        this.element.width() :
                        this.element.height();
                ...
                panels.each(function (i, element) {
                    ...
                    for (j = 0; j < self._const.properties.length; j++) {
                        if (self.options.panels[ i ] &&
                            self.options.panels[ i ][ self._const.properties[ j ]] !== undefined &&
                            self.options.panels[ i ][ self._const.properties[ j ]] !== null) {
                            panel.options[ self._const.properties[ j ]] =
                                self.options.panels[ i ][ self._const.properties[ j ]];
                        } else {
                            if (self._const.properties[ j ] === "size") {
                                if (panel[ 0 ].style[ self._getOrientation("size") ] !== "auto" &&
                                    panel[ 0 ].style[ self._getOrientation("size") ] !== "") {
                ...
                if (this.options.panels[ i ].size !== undefined) {
                    if (/%/.test(this.options.panels[ i ].size)) {
                        this.options.panels[ i ].size =
                            this.options.panels[ i ].size.replace("%", "") *
                ...
                if (this.options.panels[ i ].min !== undefined) {
                    ...
                    if (/%/.test(this.options.panels[ i ].min)) {
                        ...
                        this.options.panels[ i ].min =
                            this.options.panels[ i ].min.replace("%", "") *
                            this._getSize(this._getOrientation("size")) / 100;
                    }
                }
        ...
        _getSize: function (size) {
            ...
            if (window.getComputedStyle !== undefined) {
                ...
                // R.K. August 26th, 2016 #222513 The splitter can't calculate width correctly on Firefox and Microsoft Edge with "box-sizing: border-box"
        ...
        _createSplitter: function () {
            ...
            $("<span></span>")[ this._getOrientation("size") ](
                    this._splitter.bar[ this._getOrientation("size") ]).attr("title", "").
                    appendTo(div);
        ...
        _removeClasses: function () {
            ...
            buttonLeft.removeClass(
                this.css[ this.options.orientation +
                    "CollapseButtonLeftExpanded" ]);
            buttonRight.removeClass(
                this.css[ this.options.orientation +
            ...
            $(resizeHandler.children()[ 0 ]).
                removeClass(
                    this.css.resizeHandlerInner + "-" + this.options.orientation);
        },
        _addClasses: function () {
            ...
            buttonLeft.addClass(
                this.css[ this.options.orientation +
                    "CollapseButtonLeftExpanded" ]);
            $(buttonLeft.children()).addClass(
                this.css[ this.options.orientation +
                    "CollapseButtonLeftExpandedIcon" ]);
            buttonRight.addClass(
                this.css[ this.options.orientation +
                    "CollapseButtonRightExpanded" ]);
            $(buttonRight.children()).addClass(
                this.css[ this.options.orientation +
                    "CollapseButtonRightExpandedIcon" ]);
            resizeHandler = $(this._splitter.bar.children()[ 2 ]);
            resizeHandler.addClass(
                this.css.resizeHandler + "-" +
                    this.options.orientation);
            $(resizeHandler.children()[ 0 ]).addClass(
                this.css.resizeHandlerInner + "-" +
                    this.options.orientation);
        ...
        _removeEventHandlers: function () {
            ...
            if (!this._isTouch()) {
                ...
                $(this._splitter.bar.children()[ 0 ]).
                    unbind(this._getEvent("mouseenter"), this._getEvent("mouseleave"));
                $(this._splitter.bar.children()[ 1 ]).
                    unbind(this._getEvent("mouseenter"), this._getEvent("mouseleave"));
            }
        },
                    ...
                    if ((!left.options.collapsed && !right.options.collapsed) &&
                        ...
                        if (!splitter._capturedElement) {
                            splitter._lastMove = splitter._isTouch(event) ?
                                event.originalEvent.
                                    touches[ 0 ][ splitter._getOrientation("page") ] :
                                        event[ splitter._getOrientation("page") ];

                            if ($(event.target).is("span")) {
                                splitter._capturedElement =
                                splitter._clone($($(event.target).parent()).parent(), splitter);
                            } else {
                            ...
                            splitter._kbLockLeft = false;
                            splitter._kbLockRight = false;
                            splitter._mouseStartX =
                                splitter._isTouch(event) ?
                                    event.originalEvent.touches[ 0 ].pageX : event.pageX;
                            splitter._mouseStartY =
                                splitter._isTouch(event) ?
        ...
        _clone: function (bar, splitter) {
            ...
            clonedBar.mouseup(function () {
                ...
                if (splitter.options.orientation === "vertical") {
                    if (opt.barMouseDown &&
                        Math.round(offset.left) === Math.round($.ig.util.offset(clonedBar).left)) {
                        $bar.trigger("click");
                    }
                } else {
                    if (opt.barMouseDown &&
                        Math.round(offset.top) === Math.round($.ig.util.offset(clonedBar).top)) {
                        $bar.trigger("click");
                    }
                }
        ...
        _performDrag: function (self, ev) {
            ...
            if (self._capturedElement) {
                if (ev.type === "keydown") {
                    ...
                    bar =
                        (page - self[ self._getOrientation("mouse") ]) +
        ...
        _validatePosition: function (bar) {
            var resizeArea = this._resizeArea,
				...
				min =
                    Math.min(rightBoundary,
                    rightBoundary - resizeArea.right.options.min,
                    getPreviousBoundary + resizeArea.left.options.max),
				max =
                    Math.max(getPreviousBoundary,
                        getPreviousBoundary + resizeArea.left.options.min,
                        rightBoundary - resizeArea.right.options.max),
				pos;
            if (max > min) {
                pos =
                    resizeArea.right.offset()[ this._getOrientation("dimention") ] -
        ...
        _getNextBoundary: function (panel) {
            var size =
                panel.right.offset()[
                    this._getOrientation("dimention") ] +
				...
				offset =
                    $.ig.util.offset(
                        this._capturedElement)[
                            this._getOrientation("dimention") ] -
                            this[ this._getOrientation("start") ],
        ...
        */
        _panelsLayout: function (parameterObject) {
                ...
                if (parameterObject && parameterObject.isWindowResize) {
                    ...
                    if (this._isPercentLayout || !this.options.panels.length ||
                        ...
                        cloneObjPanels[ 1 ].options.size =
                            this.options.panels[ 1 ] &&
                                this.options.panels[ 1 ].size && this._panels[ 1 ].options.size;
                    }
            ...
            outerOppositeSizeKey =
                "outer" + oppositeSizeKey.charAt(0).toUpperCase() +
                oppositeSizeKey.slice(1);
            if (oppositeSize === this._splitter.bar[ outerOppositeSizeKey ](true)) {
                oppositeSize -=
                    this._splitter.bar[ outerOppositeSizeKey ](true) -
        ...
        _calculateSizeWithClone: function () {
            var $splitterClone, sizeWithoutBarSize, panel2RecalculatedSize, min, max,
                ...
                panel2Size = cloneObjPanels[ 1 ].options.size &&
                            ...
                            size *
                            cloneObjPanels[ 1 ].options.size.replace("%", "") / 100 - barSize :
                            cloneObjPanels[ 1 ].options.size,
            ...
            cssObj[ sizeKey ] =
                isPanel1Collapsed ?
                0 : isPanel2Collapsed ? size - barSize + "px" : panel1Size;
            $panel1.css(cssObj);
            ...
            if ($panel1[ outerSizeKey ]() + $panel2[ outerSizeKey ]() + barSize !==
                cloneObj.element[ sizeKey ]()) {
                this._resolveSizeConflictsOfCloneObject(
                    $panel1, $panel2, cloneObj.element, sizeKey);
            }

            // P.P. 1 Sep 2015 #201087 - Splitter cannot be resized if size is in percentage and window is resized
            this._resolveRoundingConflictsOfCloneObject(
                $panel1, $panel1.css(minSize), $panel2, size, sizeKey);
            this._resolveRoundingConflictsOfCloneObject(
                $panel2, $panel2.css(minSize), $panel1, size, sizeKey);
        ...
        _resolveSizeConflictsOfCloneObject: function (
            clonePanel1, clonePanel2, cloneSplitter, sizeKey) {
            ...
            if (size1 > clonePanel1NeededArea && (clonePanel1Min || clonePanel2Min)) {
                if (clonePanel1Min && !clonePanel2Min) {
                    ...
                    if (clonePanel2NeededArea > clonePanel2Min) {
                        ...
                        clonePanel2.css(
                            "min-" + sizeKey, size - barSize - Math.floor(clonePanel1Min));
                    }
                }
        ...
        // P.P. 1 Sep 2015 #202984 - On window resize of splitter, panel drops down, if min/max limitations of the panels are conflicted
        _resolveRoundingConflictsOfCloneObject: function (
            checkedPanel, checkedPanelMin, oppositePanel, sizeSplitter, sizeKey) {
            // P.P. 1 Sep 2015 #201087 - Splitter cannot be resized if size is in percentage and window is resized
        ...
        _handlerPanelSize: function (panel, outerSize, size) {
            ...
            if (this._isPercentLayout) {
                if (panel.options._min !== undefined) {
                    panel.options.min =
                        panel.options._min.replace("%", "") *
                        this._getSize(this._getOrientation("size")) / 100;
                }
                if (panel.options._max !== undefined) {
                    panel.options.max =
                        panel.options._max.replace("%", "") *
                        this._getSize(this._getOrientation("size")) / 100;
                }
            }
            ...
            if (outerSize + panel.options.size >= size && !panel.options.collapsed) {
                ...
                if (!panel.options.collapsed) {
                    if (this._isPercentLayout) {
                        panel[
                            this._getOrientation("size") ]
                            ((newSize / this._getSize(this._getOrientation("size"))) * 100 + "%");
                    } else {
        ...
        _setPanelSize: function (panel, size) {
            if (!panel.options.collapsed) {
                ...
                if (panel.options.size === 0 || (this._isPercentLayout &&
                    panel.options.size <= $.ig.util.getScrollWidth())) {
                ...
                if (this._isPercentLayout) {
                    ...
                    panel[
                        this._getOrientation("size") ]
                        ((size / this._getSize(this._getOrientation("size"))) * 100 + "%");
                } else {
            ...
            if (index <= this._panels.length) {
                if (panel.options.collapsed) {
                    ...
                    if (this._isPercentLayout) {
                        panel[
                            this._getOrientation("size") ]
                            ((maxSize / this._getSize(this._getOrientation("size"))) * 100 + "%");
                    } else {
        ...
        _panelHelper: function (outerSize, size) {
            ...
            panel[
                this._getOrientation("size") ]
                (size - outerSize + panel[ this._getOrientation("size") ]());
        },
        expandAt: function (index) {
			...
			*/
            ...
            if (index <= this._panels.length && index >= 0 && panel.options.collapsed) {
                ...
                panel.options.size =
                    Math.min(panel.options.size,
                    neighborPanel[ this._getOrientation("size") ]());
                neighborPanel.options.size =
                    neighborPanel[ this._getOrientation("size") ]() - panel.options.size;
                this._splitter.bar.removeClass(this.css.barCollapsed);
                ...
                $(this._splitter.bar.children()[ index ]).
                    removeClass(this.css[
                        this.options.orientation + "CollapseButton" +
        ...
        collapseAt: function (index) {
			...
			*/
            ...
            //animationDuration = 0;
            if (index < this._panels.length && index >= 0 &&
                !panel.options.collapsed && panel.options.collapsible) {
                ...
                $(this._splitter.bar.children()[ index ]).
                    addClass(this.css[ this.options.orientation + "CollapseButton" +
                        (index % 2 === 0 ? "Left" : "Right") + "Collapsed" ]);
                $($(this._splitter.bar.children()[ index ]).
                    children()).addClass(this.css[ this.options.orientation + "CollapseButton" +
                        (index % 2 === 0 ? "Left" : "Right") + "CollapsedIcon" ]);
                neighborPanel.options.size =
        ...
        _getPanelsSizeInPercentages: function (sizeKey) {
            var size = this._getSize(sizeKey),
                ...
                panel1CalculatedSize =
                    size - barSize === this._panels[ 0 ].options.size ?
                        100 : this._panels[ 0 ].options.size / size * 100,
                panel1Size = panel1CalculatedSize < 0 ?
                    0 : panel1CalculatedSize > 100 ? 100 : panel1CalculatedSize,
                panel2CalculatedSize = 100 - panel1Size,
                panel2Size = panel2CalculatedSize < 0 ?
                    0 : panel2CalculatedSize > 100 ? 100 : panel2CalculatedSize;
    ...
    $.extend($.ui.igSplitter, { version: "16.1.20161.2145" });
}(jQuery));
