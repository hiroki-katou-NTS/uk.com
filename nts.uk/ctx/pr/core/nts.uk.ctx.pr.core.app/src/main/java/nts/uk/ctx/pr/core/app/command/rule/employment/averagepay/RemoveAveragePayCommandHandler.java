 ...
 */
@RequestScoped
public class RemoveAveragePayCommandHandler extends CommandHandler<RemoveAveragePayCommand>{
	@Inject
	private AveragePayRepository averagePayRepository; 
	
	@Override
	protected void handle(CommandHandlerContext<RemoveAveragePayCommand> context) {
		RemoveAveragePayCommand command = context.getCommand();
		...
		averagePayRepository.remove(averagePay);
