...
public class DeleteLayoutHistoryCommandHandler extends CommandHandler<DeleteLayoutHistoryCommand> {
	...
	protected void handle(CommandHandlerContext<DeleteLayoutHistoryCommand> context) {
		...
		// データベース削除
		Optional<LayoutHistory> layoutHist = this.layoutHistRepository.getHistoryBefore(command.getCompanyCode(),
				command.getStmtCode(), command.getStartYm());
		if (layoutHist.isPresent()) {
			if (layoutHist.get().getEndYm().v() == 999912) {
				deleteObject(command);
				Optional<LayoutHistory> layoutHistBefore = this.layoutHistRepository.getHistoryByCodeAndEndYear(
						command.getCompanyCode(), command.getStmtCode(), command.getStartYm() - 1);
				if (!layoutHistBefore.isPresent()) {
					this.layoutRepository.remove(command.getCompanyCode(), command.getStmtCode());
				} else {
					updateObject(layoutHistBefore.get());
				}
			}
		}
	}
	...
	private void deleteObject(DeleteLayoutHistoryCommand command) {
		...
		// List<LayoutMasterCategory> lstCategories =
		// this.categoryRepository.getCategories(command.getCompanyCode(),
		// command.getStmtCode(),
		// command.getStartYm());
		List<LayoutMasterCategory> lstCategories = this.categoryRepository.getCategoriesBefore(command.getCompanyCode(),
		...
		List<LayoutMasterLine> lstLineOfCategory = this.lineRepository.getLinesBefore(command.getCompanyCode(),
				command.getHistoryId());
		this.lineRepository.remove(lstLineOfCategory);

		// データベース削除[明細書マスタ明細.DEｌ-1] を実施する
		List<LayoutMasterDetail> lstDetails = this.detailRepository.getDetailsBefore(command.getCompanyCode(),
				command.getStmtCode(), command.getHistoryId());
		this.detailRepository.remove(lstDetails);
	}

	private void updateObject(LayoutHistory layoutHistBefore) {
		...
		// }
		// Optional<LayoutHistory> layoutHistBefore =
		// layoutHistRepository.getHistoryBefore(command.getCompanyCode(),
		// command.getStmtCode(), command.getStartYm() - 1);
		// if (layoutHistBefore != null && layoutHistBefore.isPresent()) {
		// LayoutHistory layoutHist = layoutHistBefore.get();
		// }
		String companyCode = AppContexts.user().companyCode();
		String stmtCode = layoutHistBefore.getStmtCode().v();
		String historyId = layoutHistBefore.getHistoryId();
		layoutHistBefore.setEndYm(new YearMonth(999912));
		layoutHistRepository.update(layoutHistBefore);
		// データベース更新[明細書マスタカテゴリ.UPD-2] を実施する
		List<LayoutMasterCategory> lstCategoryBefore = this.categoryRepository.getCategoriesBefore(companyCode,
				stmtCode, historyId);
		if (!lstCategoryBefore.isEmpty()) {
			List<LayoutMasterCategory> lstCategoryUP = lstCategoryBefore.stream().map(category -> {
				return LayoutMasterCategory.createFromDomain(category.getCompanyCode(), category.getStmtCode(),
						category.getCtAtr(), category.getCtgPos(), category.getHistoryId());
			}).collect(Collectors.toList());
			this.categoryRepository.update(lstCategoryUP);
		}

		// データベース更新[明細書マスタ行.UPD-2] を実施する
		// List<LayoutMasterLine> lstLineBefore =
		// this.lineRepository.getLinesBefore(command.getCompanyCode(),
		// command.getStmtCode(),
		// command.getStartYm() - 1);
		List<LayoutMasterLine> lstLineBefore = this.lineRepository.getLinesBefore(companyCode, stmtCode, historyId);
		if (!lstLineBefore.isEmpty()) {
			List<LayoutMasterLine> lstLineUp = lstLineBefore.stream().map(line -> {
				return LayoutMasterLine.createFromDomain(line.getCompanyCode(), line.getStmtCode(),
						line.getAutoLineId(), line.getCategoryAtr(), line.getLineDisplayAttribute(),
						line.getLinePosition(), line.getHistoryId());
			}).collect(Collectors.toList());
			this.lineRepository.update(lstLineUp);
		}

		// データベース更新[明細書マスタ明細.UPD-2] を実施する
		// List<LayoutMasterDetail> detailsBefore =
		// detailRepository.getDetailsBefore(command.getCompanyCode(),
		// command.getStmtCode(),
		// command.getStartYm() - 1);
		List<LayoutMasterDetail> detailsBefore = detailRepository.getDetailsBefore(companyCode, stmtCode, historyId);
