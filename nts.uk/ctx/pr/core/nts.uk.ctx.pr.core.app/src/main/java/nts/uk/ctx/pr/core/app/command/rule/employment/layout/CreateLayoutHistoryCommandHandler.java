...
public class CreateLayoutHistoryCommandHandler extends CommandHandler<CreateLayoutHistoryCommand> {
	...
	protected void handle(CommandHandlerContext<CreateLayoutHistoryCommand> context) {
		...
		// found layout")));
		LayoutHistory layoutHeadOrigin = layoutHisRepo
		...
		LayoutMaster layoutNew = command.toDomain(command.getStartYm(), 999912, command.getLayoutAtr(),
				layoutOrigin.getStmtName().v(), IdentifierUtil.randomUniqueId());
		...
		updateData(command, companyCode, layoutOrigin, layoutNew, categoriesOrigin, linesOrigin, detailsOrigin);
	}
	...
	private void copyFromPreviousHistory(CreateLayoutHistoryCommand command, String companyCode,
		...
		List<LayoutMasterCategory> categoriesNew = categoriesOrigin.stream().map(org -> {
			return LayoutMasterCategory.createFromDomain(org.getCompanyCode(), new YearMonth(command.getStartYm()),
					org.getStmtCode(), org.getCtAtr(), new YearMonth(999912), org.getCtgPos(),
		...
		List<LayoutMasterLine> linesNew = linesOrigin.stream().map(org -> {
			return LayoutMasterLine.createFromDomain(org.getCompanyCode(), new YearMonth(command.getStartYm()),
					org.getStmtCode(), new YearMonth(999912), org.getAutoLineId(), org.getCategoryAtr(),
					org.getLineDispayAttribute(), org.getLinePosition(), IdentifierUtil.randomUniqueId());
		}).collect(Collectors.toList());
		...
		List<LayoutMasterDetail> detailsNew = detailsOrigin.stream().map(org -> {
			return LayoutMasterDetail.createFromDomain(org.getCompanyCode(), org.getLayoutCode(),
					new YearMonth(command.getStartYm()), new YearMonth(999912), org.getCategoryAtr(), org.getItemCode(),
					...
					org.getSetOffItemCode(), org.getCommuteAtr(), org.getPersonalWageCode(),
					IdentifierUtil.randomUniqueId());
		}).collect(Collectors.toList());
	...
	private void updateData(CreateLayoutHistoryCommand command, String companyCode, LayoutMaster layoutOrigin,
			LayoutMaster layoutNew, List<LayoutMasterCategory> categoriesOrigin, List<LayoutMasterLine> linesOrigin,
			List<LayoutMasterDetail> detailsOrigin) {
		...
		// command.getStmtCode(), command.getStartPrevious());
		List<LayoutMasterLine> linesNew = linesOrigin.stream().map(org -> {
			return LayoutMasterLine.createFromDomain(org.getCompanyCode(), org.getStartYM(), org.getStmtCode(), endYm,
					org.getAutoLineId(), org.getCategoryAtr(), org.getLineDispayAttribute(), org.getLinePosition(),
					org.getHistoryId());
		}).collect(Collectors.toList());
		...
		List<LayoutMasterDetail> detailsNew = detailsOrigin.stream().map(org -> {
			return LayoutMasterDetail.createFromDomain(org.getCompanyCode(), org.getLayoutCode(), org.getStartYm(),
					endYm, org.getCategoryAtr(), org.getItemCode(), org.getAutoLineId(), org.getItemPosColumn(),
