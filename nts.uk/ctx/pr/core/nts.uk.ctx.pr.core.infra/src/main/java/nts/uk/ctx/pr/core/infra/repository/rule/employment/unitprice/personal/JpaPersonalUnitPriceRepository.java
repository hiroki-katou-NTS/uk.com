...
public class JpaPersonalUnitPriceRepository extends JpaRepository implements PersonalUnitPriceRepository {
	...
	public Optional<PersonalUnitPrice> find(String companyCode, String personalUnitPriceCode) {
		...
		return this.queryProxy().find(primaryKey, QupmtPUnitprice.class)
				.map(x -> PersonalUnitPrice.createFromJavaType(x.qupmtPUnitpricePK.companyCode,
						x.qupmtPUnitpricePK.personalUnitPriceCode, x.personalUnitPriceName,
						x.personalUnitPriceShortName, x.uniteCode, x.memo, x.fixPaymentAtr, x.fixPaymentMonthly,
						x.fixPaymentDayMonth, x.fixPaymentDaily, x.fixPaymentHoursly, x.displaySet,
						x.paymentSettingType, x.unitPriceAtr));
	}

	@Override
	public List<PersonalUnitPrice> findAll(String companyCode) {
		return this.queryProxy().query(SEL_1, QupmtPUnitprice.class).setParameter("companyCode", companyCode)
				.getList(x -> PersonalUnitPrice.createFromJavaType(x.qupmtPUnitpricePK.companyCode,
						x.qupmtPUnitpricePK.personalUnitPriceCode, x.personalUnitPriceName,
						x.personalUnitPriceShortName, x.uniteCode, x.memo, x.fixPaymentAtr, x.fixPaymentMonthly,
						x.fixPaymentDayMonth, x.fixPaymentDaily, x.fixPaymentHoursly, x.displaySet,
						x.paymentSettingType, x.unitPriceAtr));
	...
	public List<PersonalUnitPrice> findAll(String companyCode, List<String> unitPriceCodeList) {
		return this.queryProxy().query(SEL_1_1, QupmtPUnitprice.class).setParameter("companyCode", companyCode)
	...
	private static QupmtPUnitprice toEntity(PersonalUnitPrice domain) {
		...
		QupmtPUnitprice entity = new QupmtPUnitprice(key, domain.getPersonalUnitPriceName().v(),
				domain.getPersonalUnitPriceAbName().v(), domain.getDisplaySet().value, domain.getUniteCode().v(),
				domain.getPaymentSettingType().value, domain.getFixPaymentAtr().value,
				domain.getFixPaymentMonthly().value, domain.getFixPaymentDayMonth().value,
				domain.getFixPaymentDaily().value, domain.getFixPaymentHoursly().value, domain.getUnitPriceAtr().value,
				domain.getMemo().v());
		return entity;
	}
}
