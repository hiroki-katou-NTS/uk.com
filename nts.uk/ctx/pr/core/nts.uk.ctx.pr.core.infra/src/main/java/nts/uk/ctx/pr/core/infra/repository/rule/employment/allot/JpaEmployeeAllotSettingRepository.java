...
public class JpaEmployeeAllotSettingRepository extends JpaRepository implements EmployeeAllotSettingRepository {
	...
	private final String ALL_EMPLOYEE_SETTING = "SELECT e.cmnmtEmpPk.companyCode, a.QstmtStmtAllotEmPK.histId,  e.cmnmtEmpPk.employmentCode, e.employmentName, q.qstmtStmtLayoutHeadPK.stmtCd, q.stmtName"
			...
			// + " ON e.cmnmtEmpPk.companyCode =
			// a.qstmtStmtAllotEmPK.companyCode AND e.cmnmtEmpPk.employmentCode
			// = a.qstmtStmtAllotEmPK.employeeCd"
			+ " , QstmtStmtLayoutHead q"
			// + " ON a.QstmtStmtAllotEmPK.companyCode =
			// q.qstmtStmtLayoutHeadPK.companyCd AND"
			// + " a.paymentDetailCode = q.qstmtStmtLayoutHeadPK.stmtCd AND
			// a.bonusDetailCode = q.qstmtStmtLayoutHeadPK.stmtCd"
	...
	private final String EMP_SET_SEL = "SELECT a FROM QstmtStmtAllotEm a WHERE a.QstmtStmtAllotEmPK.histId = :histId ";
	...
	public List<EmployeeAllSetting> findAllEm(String companyCode, String historyID) {
		List<EmployeeAllSetting> result = this.queryProxy().query(ALL_EMPLOYEE_SETTING, Object[].class)
				.setParameter("companyCode", companyCode).setParameter("histId", historyID)// .getList();
	...
	public List<EmployeeAllotSetting> findEmpDetail(String companyCode, String historyID) {
		List<EmployeeAllotSetting> result = this.queryProxy().query(EMP_SET_SEL, QstmtStmtAllotEm.class)
				.setParameter("histId", historyID).getList(a -> toDomain(a));
		return result;
	}

	@Override
	public void delete(EmployeeAllotSetting domain) {
		String companyCode = AppContexts.user().companyCode();
		QstmtStmtAllotEmPK qstmtStmtAllotEmPK = new QstmtStmtAllotEmPK(companyCode, domain.getHistoryId(),
				domain.getEmployeeCode().v());
		this.commandProxy().remove(QstmtStmtAllotEm.class, qstmtStmtAllotEmPK);
	}
}
