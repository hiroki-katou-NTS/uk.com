...
public class JpaRetirementPaymentRepository extends JpaRepository implements RetirementPaymentRepository {
	...
	private final String SEL_1 = "Select a From QredtRetirementPayment a "
	...
	public List<RetirementPayment> findByCompanyCodeAndPersonId(CompanyCode companyCode, PersonId personId) {
		return this.queryProxy().query(SEL_1, QredtRetirementPayment.class).setParameter("companyCode", companyCode.v())
				.setParameter("personId", personId.v()).getList(x -> convertToDomain(x));
	}

	@Override
	public Optional<RetirementPayment> findRetirementPaymentInfo(CompanyCode companyCode, PersonId personId,
			GeneralDate dateTime) {
		return this.queryProxy().find(new QredtRetirementPaymentPK(companyCode.v(), personId.v(), dateTime),
				QredtRetirementPayment.class).map(x -> convertToDomain(x));
	}
	...
	@Override
	public void remove(String companyCode, String personId, GeneralDate payDate) {
		this.commandProxy().remove(QredtRetirementPayment.class,
				new QredtRetirementPaymentPK(companyCode,
						personId, payDate));
	}
	...
	private QredtRetirementPayment convertToEntity(RetirementPayment retirementPayment) {
		return new QredtRetirementPayment(
				new QredtRetirementPaymentPK(retirementPayment.getCompanyCode().v(),
						retirementPayment.getPersonId().v(), retirementPayment.getPayDate()),
				retirementPayment.getTrialPeriodSet().value, retirementPayment.getExclusionYears().v(),
				retirementPayment.getAdditionalBoardYears().v(), retirementPayment.getBoardYears().v(),
				retirementPayment.getTotalPaymentMoney().v(), retirementPayment.getDeduction1Money().v(),
				retirementPayment.getDeduction2Money().v(), retirementPayment.getDeduction3Money().v(),
				retirementPayment.getRetirementPayOption().value, retirementPayment.getTaxCalculationMethod().value,
				retirementPayment.getIncomeTaxMoney().v(), retirementPayment.getCityTaxMoney().v(),
				retirementPayment.getPrefectureTaxMoney().v(), retirementPayment.getTotalDeclarationMoney().v(),
				retirementPayment.getActualRecieveMoney().v(), retirementPayment.getBankTransferOption1().value,
				retirementPayment.getOption1Money().v(), retirementPayment.getBankTransferOption2().value,
				retirementPayment.getOption2Money().v(), retirementPayment.getBankTransferOption3().value,
				retirementPayment.getOption3Money().v(), retirementPayment.getBankTransferOption4().value,
				retirementPayment.getOption4Money().v(), retirementPayment.getBankTransferOption5().value,
				retirementPayment.getOption5Money().v(), retirementPayment.getWithholdingMeno().v(),
	 ...
	 * 
	 * @param qredtRetirementPayment
	 *            Entity Item
	 * @return Domain Item
	 */
	private RetirementPayment convertToDomain(QredtRetirementPayment qredtRetirementPayment) {
		return new RetirementPayment(new CompanyCode(qredtRetirementPayment.qredtRetirementPaymentPK.companyCode),
				...
				new PaymentMoney(qredtRetirementPayment.option5Money), new Memo(qredtRetirementPayment.withholdingMeno),
