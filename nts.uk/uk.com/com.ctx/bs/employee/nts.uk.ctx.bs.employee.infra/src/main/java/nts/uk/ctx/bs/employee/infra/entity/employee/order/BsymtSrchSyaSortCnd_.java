/******************************************************************
 * Copyright (c) 2017 Nittsu System to present.                   *
 * All right reserved.                                            *
 *****************************************************************/
package nts.uk.ctx.bs.employee.infra.entity.employee.order;

import javax.annotation.Generated;
import javax.persistence.metamodel.SingularAttribute;
import javax.persistence.metamodel.StaticMetamodel;
import nts.uk.ctx.bs.employee.infra.entity.employee.order.BsymtSrchSyaSortCndPK;

/**
 * The Class BsymtSrchSyaSortCnd_.
 */
@Generated(value="EclipseLink-2.5.2.v20140319-rNA", date="2018-02-12T17:45:39")
@StaticMetamodel(BsymtSrchSyaSortCnd.class)
public class BsymtSrchSyaSortCnd_ { 

    /** The condition order. */
    public static volatile SingularAttribute<BsymtSrchSyaSortCnd, Integer> conditionOrder;
    
    /** The id. */
    public static volatile SingularAttribute<BsymtSrchSyaSortCnd, BsymtSrchSyaSortCndPK> id;

}