...
public class JpaTemporaryAbsence extends JpaRepository implements TemporaryAbsenceRepository {
	...
	private BsymtTemporaryAbsence toEntity(TemporaryAbsence domain){
		...
		return new BsymtTemporaryAbsence(key, domain.getEmployeeId(), domain.getStartDate(), domain.getEndDate(), domain.getTempAbsenceType().value, domain.getTempAbsenceReason(), domain.getFamilyMemberId(), domain.getBirthDate(), domain.getMulPregnancySegment());
	}
	/**
	 * Update entity
	 * @param domain
