...
public class JpaTemporaryAbsenceHistRepository extends JpaRepository implements TemporaryAbsenceHistRepository{
	...
	private void updateItemBefore(TempAbsenceHistory domain, DateHistoryItem item){
		...
		Optional<BsymtTempAbsHistory> histItem = this.queryProxy().find(beforeItem.get().identifier(), BsymtTempAbsHistory.class);
		if (histItem.isPresent()){
	...
	private void updateItemAfter(TempAbsenceHistory domain, DateHistoryItem item){
		...
		Optional<BsymtTempAbsHistory> histItem  = this.queryProxy().find(aferItem.get().identifier(), BsymtTempAbsHistory.class);
		if (histItem.isPresent()){
	...
	@Override
	public Optional<TempAbsenceHistory> getTemporaryAbsenceHistByHistId(String employeeId, String histId) {
	...
	@Override
	public Optional<TempAbsenceHistory> getTemporaryAbsenceHistByEmployeeId(String employeeId) {
		List<BsymtTempAbsHistory> listHist = this.queryProxy().query(QUERY_GET_TEMPORARYABSENCE_BYSID,BsymtTempAbsHistory.class)
