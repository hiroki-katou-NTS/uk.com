...
public class JpaAffJobTitleHistoryRepository_v1 extends JpaRepository implements AffJobTitleHistoryRepository_ver1{
	...
	private final String QUERY_GET_AFFJOBTITLEHIST_BYSID = "select jb "
			+ "from BsymtAffJobTitleHist jb "
	...
	public Optional<AffJobTitleHistory_ver1> getListBySid(String sid) {
		...
		List<BsymtAffJobTitleHist> listHist = this.queryProxy().query(QUERY_GET_AFFJOBTITLEHIST_BYSID,BsymtAffJobTitleHist.class).setParameter("sid", sid).getList();
		if (!listHist.isEmpty()){
	...
	@Override
	public void addJobTitleMain(AffJobTitleHistory_ver1 domain) {
	...
	@Override
	public void updateJobTitleMain(AffJobTitleHistory_ver1 domain, DateHistoryItem item) {
		Optional<BsymtAffJobTitleHist> itemToBeUpdated = this.queryProxy().find(item.identifier(), BsymtAffJobTitleHist.class);
	...
	@Override
	public void deleteJobTitleMain(AffJobTitleHistory_ver1 domain, DateHistoryItem item) {
		Optional<BsymtAffJobTitleHist> itemToBeDeleted = this.queryProxy().find(item.identifier(), BsymtAffJobTitleHist.class);
		...
		if (domain.getHistoryItems().size() >0){
			...
			Optional<BsymtAffJobTitleHist> histItem  = this.queryProxy().find(lastItem.identifier(), BsymtAffJobTitleHist.class);
