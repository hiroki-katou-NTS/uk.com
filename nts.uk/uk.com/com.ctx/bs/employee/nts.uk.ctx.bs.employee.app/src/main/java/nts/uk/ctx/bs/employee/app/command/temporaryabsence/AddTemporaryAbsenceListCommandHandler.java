...
@Stateless
public class AddTemporaryAbsenceListCommandHandler extends CommandHandlerWithResult<List<AddTemporaryAbsenceCommand>, List<PeregAddCommandResult>>
implements PeregAddListCommandHandler<AddTemporaryAbsenceCommand>{
	...
	@Override
	protected List<PeregAddCommandResult> handle(CommandHandlerContext<List<AddTemporaryAbsenceCommand>> context) {
		...
		command.stream().forEach(c ->{
			...
			result.add(new PeregAddCommandResult(newHistID));
		});
