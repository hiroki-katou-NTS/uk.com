...
public class JpaCurrentAddressRepository extends JpaRepository implements CurrentAddressRepository {
	...
	private static BpsmtCurrentaddress toEntity(CurrentAddress domain) {
		...
		return new BpsmtCurrentaddress(key, domain.getPid(), domain.getPeriod().start(), domain.getPeriod().end(),
				domain.getAddress1().getAddress1().v(), domain.getAddress2().getAddress2().v(),domain.getAddress1().getAddressKana1().v(), domain.getAddress2().getAddressKana2().v(), domain.getCountryId(), domain.getHomeSituationType().v(), domain.getHouseRent().v(), 
				domain.getPhoneNumber().v(), domain.getPostalCode().v(), domain.getPrefectures().v(), domain.getPersonMailAddress().v(), domain.getHouseType().v(), domain.getNearestStation().v());
