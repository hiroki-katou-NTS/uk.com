...
public class RoleSetGrantedEmployeePubImpl implements RoleSetGrantedEmployeePub {
	...
	public List<String> findEmpGrantedInWorkplace(String workplaceId, DatePeriod period) {
		...
		List<RoleSet> roleSets = roleSetRepo.findByCompanyId(companyId).stream()
				.filter(item -> item.getApprovalAuthority() == ApprovalAuthority.HasRight).collect(Collectors.toList());
		...
		List<String> empIds2 = roleSetPersons.stream().filter(item -> empIds.contains(item.getEmployeeID()))
