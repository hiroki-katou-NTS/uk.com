...
public class UpdateRoleSetCommandHandler extends CommandHandlerWithResult<RoleSetCommand, String> {
	...
	protected String handle(CommandHandlerContext<RoleSetCommand> context) {
		...
		if (!StringUtils.isNoneEmpty(companyId)) {
			...
			// validate
			roleSetDom.validateForUpdate();
			
			// register to DB - ドメインモデル「ロールセット」を新規登録する
			this.roleSetRepository.update(roleSetDom);
	
			// register/update to web menu link - ドメインモデル「ロールセット別紐付け」を新規登録する
			...
			roleSetAndWebMenuAdapter.updateListOfRoleSetAndWebMenu(lstRoleSetAndWebMenu);
