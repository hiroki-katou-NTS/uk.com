...
@NoArgsConstructor
@AllArgsConstructor
public class CcgmtTopPagePartDataCopyHandler extends JpaRepository implements DataCopyHandler {
	 ...
	 */
	public CcgmtTopPagePartDataCopyHandler(CopyMethod copyMethod, String companyId) {
	...
	public void doCopy() {
		...
		List<CcgmtTopPagePart> zeroCidEntities = this.findAllByCid(zeroCid);
		List<CcgmtTopPagePart> oldDatas = this.findAllByCid(companyId);
		switch (copyMethod) {
			case REPLACE_ALL:
				// Delete all old data
			...
			case ADD_NEW:
				// Insert Data
				List<CcgmtTopPagePart> dataCopy = new ArrayList<>();
				zeroCidEntities.stream().forEach(e-> {
					CcgmtTopPagePart cloneEntity = SerializationUtils.clone(e);
					cloneEntity.getCcgmtTopPagePartPK().setCompanyID(companyId);
					dataCopy.add(cloneEntity);
				});
				List<CcgmtTopPagePart> addEntites = dataCopy;
	 ...
	 * @return the list
	 */
	public List<CcgmtTopPagePart> findAllByCid(String cid){
		return this.queryProxy().query(SELECT_BY_CID, CcgmtTopPagePart.class).setParameter("cid", cid).getList();
	}
