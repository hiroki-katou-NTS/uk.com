...
//		// get employee code
//		List<EmployeeInfoDtoImport> listEmployee = this.employeeInfoAdapter
//				.getEmployeesAtWorkByBaseDate(companyId, baseDate);
//
//		List<String> listPersonId = new ArrayList<>();
//		List<UserDto> listUserMap = new ArrayList<>();
//		List<UserImport> listUser = new ArrayList<>();
...
//	}
	...
	public List<UserDto> loadUserSetting(List<UserDto> listUserMap, Boolean isScreenC) {
		...
		if (isScreenC == true) {
			List<String> listUserIDs = listUserMap.stream().map(w -> w.getUserId())
					.collect(Collectors.toList());
			List<OtherSysAccount> listOtherSysAccs = otherSysAccountRepository
					.findAllOtherSysAccount(listUserIDs);
			...
			listUserMap.forEach(w -> {
				...
				if (otherSysAcc != null
						&& otherSysAcc.getAccountInfo().getUseAtr().value == UseAtr.Use.value) {
					w.setIsSetting(true);
				} else if (otherSysAcc == null
						|| otherSysAcc.getAccountInfo().getUseAtr().value == UseAtr.NotUse.value) {
					w.setIsSetting(false);
				}
			});

		} else {
			List<String> listUserID = listUserMap.stream().map(w -> w.getUserId())
					.collect(Collectors.toList());

			List<WindowsAccount> lstWindowAccount = windowAccountRepository
					.findByListUserId(listUserID);

			listUserMap.forEach(w -> {
				List<WindowsAccount> winAcc = lstWindowAccount.stream()
						.filter(w2 -> w2.getUserId().equals(w.getUserId()))
						.collect(Collectors.toList());
