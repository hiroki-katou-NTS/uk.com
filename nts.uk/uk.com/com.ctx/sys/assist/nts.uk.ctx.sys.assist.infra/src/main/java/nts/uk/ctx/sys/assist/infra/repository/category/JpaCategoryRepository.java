...
public class JpaCategoryRepository extends JpaRepository implements CategoryRepository
{
    ...
    private static final String SELECT_BY_ATTENDANCE_SYSTEM_AND_CODENAME_CATEIGNORE = SELECT_ALL_QUERY_STRING + " WHERE ( f.categoryId like :keySearch OR f.categoryName LIKE :keySearch ) and f.timeStore <> :timeStore  and f.attendanceSystem = :attendanceSystem ORDER BY f.attendanceSystem,f.categoryId";
    private static final String SELECT_BY_PAYMENT_AVAIABILITY_AND_CODENAME_CATEIGNORE = SELECT_ALL_QUERY_STRING + " WHERE ( f.categoryId like :keySearch OR f.categoryName LIKE :keySearch ) and f.timeStore <> :timeStore  and f.paymentAvailability = :paymentAvailability ORDER BY f.paymentAvailability,f.categoryId";
