...
public class JpaCategoryRepository extends JpaRepository implements CategoryRepository
{
    ...
    private static final String SELECT_BY_ATTENDANCE_SYSTEM_AND_CODENAME = SELECT_ALL_QUERY_STRING + " WHERE f.categoryId like %:keySearch% OR f.categoryName LIKE '%:keySearch%' and f.timeStore <> 1 and f.categoryId NOT IN :categoriesIgnore and f.attendanceSystem = 1 ORDER BY f.attendanceSystem,f.categoryId";
