...
public class ProcessRecoverOneEmpHandle {
	...
	public DataRecoveryOperatingCondition recoverDataOneEmp(String dataRecoveryProcessId,
			List<EmployeeDataReInfoImport> empsOrderByScd, List<DataRecoveryTable> tableList, 
		...
		for (EmployeeDataReInfoImport employeeData : empsOrderByScd) {
			...
			if (errorCode.equals(SETTING_EXCEPTION) || errorCode.equals(SETTING_DATE_EXCEPTION)) {
		...
		for (DataRecoveryTable dataRecoveryTable : listTableOrder) {
			...
			if (table.isPresent() && table.get().getHistoryCls() == HistoryDiviSion.HAVE_HISTORY) {
				try {
					...
					saveLogDataRecoverServices.saveErrorLogDataRecover(dataRecoveryProcessId, target, errorContent, targetDate, processingContent, contentSql);
					// #9005_1
					LOGGER.error("deleteDataEmpTableHistory rollBack transaction");
			...
			try {
				...
				saveLogDataRecoverServices.saveErrorLogDataRecover(dataRecoveryProcessId, target, errorContent, targetDate, processingContent, contentSql);
				// #9005_3
				LOGGER.error("crud error rollBack transaction");
				throw new Exception(SQL_EXCEPTION);
			}
					...
					if (!records.isEmpty()) {
						...
						for (int k = 0; k < records.size(); k++) {
							...
							try {
								...
								try {
									throw new Exception(SETTING_DATE_EXCEPTION);
								} catch (Exception e1) {
									e1.printStackTrace();
								}
							}
