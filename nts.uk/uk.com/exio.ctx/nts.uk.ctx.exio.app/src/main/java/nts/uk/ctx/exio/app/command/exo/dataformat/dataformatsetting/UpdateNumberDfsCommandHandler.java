...
public class UpdateNumberDfsCommandHandler extends CommandHandler<NumberDfsCommand> {
	...
	protected void handle(CommandHandlerContext<NumberDfsCommand> context) {
		...
		repository.update(new NumberDataFmSetting(updateCommand.getCid(), updateCommand.getNullValueReplace(),
				...
				updateCommand.getFixedLengthOutput(), updateCommand.getFixedLengthIntegerDigit(),
				updateCommand.getFixedLengthEditingMethod(), updateCommand.getDecimalDigit(),
				updateCommand.getDecimalPointClassification(), updateCommand.getDecimalFraction(),
