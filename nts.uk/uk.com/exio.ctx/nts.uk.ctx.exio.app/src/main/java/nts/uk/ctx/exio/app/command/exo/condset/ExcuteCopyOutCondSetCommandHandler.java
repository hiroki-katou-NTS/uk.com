...
public class ExcuteCopyOutCondSetCommandHandler
		extends CommandHandlerWithResult<StdOutputCondSetCommand, ExcuteCopyOutCondSet> {
	
	@Inject
	private StdOutputCondSetRepository stdOutputCondSetRepository;

	@Override
	protected ExcuteCopyOutCondSet handle(CommandHandlerContext<StdOutputCondSetCommand> context) {
		...
		if(stdOutputCondSet.isPresent()){
			if(command.isOverWrite()){
				return new ExcuteCopyOutCondSet(true,command.getDestinationName(),
