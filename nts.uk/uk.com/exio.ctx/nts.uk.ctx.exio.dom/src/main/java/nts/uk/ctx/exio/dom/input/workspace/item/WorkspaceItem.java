...
public class WorkspaceItem {
	...
	private class AnnotationValues{
		...
		public AnnotationValues(Optional<DomainConstraint> domainContraint) {
			domainContraint.ifPresent(constraint ->{
			...
			switch(name) {
				...
				case "max":
					...
					if(value.contains(".")) {
						...
						val vTotalLength = Integer.toString(Math.abs(Integer.valueOf(value.split("\\.")[0]))).length() + vMantissaMaxLength;
					...
					//時分・時刻はここで
						...
						val vLength = Integer.toString(Math.abs((Integer.parseInt(parts[0]) * 60 + Integer.parseInt(parts[1])))).length();
		...
		public boolean isSamesLengths(int maxLength, int mantissaMaxLength) {
			return isSame(this.maxLength, maxLength)
				&&   isSame(this.mantissaMaxLength, mantissaMaxLength);
		}
		
		private boolean isSame(Optional<Integer> v ,int length) {
			return v.map(value -> v.get().intValue() == length)
					.orElse(length == 0);
		}
