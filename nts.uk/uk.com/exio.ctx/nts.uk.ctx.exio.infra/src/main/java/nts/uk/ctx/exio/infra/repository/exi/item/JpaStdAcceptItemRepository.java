...
public class JpaStdAcceptItemRepository extends JpaRepository implements StdAcceptItemRepository {
	...
	private static final String SELECT_BY_KEY_STRING = SELECT_ALL_QUERY_STRING
			+ " WHERE  f.stdAcceptItemPk.cid =:cid AND  f.stdAcceptItemPk.conditionSetCd =:conditionSetCd AND  f.stdAcceptItemPk.categoryId =:categoryId AND  f.stdAcceptItemPk.acceptItemNumber =:acceptItemNumber ";
	...
	@Override
	public Optional<StdAcceptItem> getStdAcceptItemById(String cid, String conditionSetCd, String categoryId,
			int acceptItemNumber) {
		return this.queryProxy().query(SELECT_BY_KEY_STRING, OiomtStdAcceptItem.class).setParameter("cid", cid)
				.setParameter("conditionSetCd", conditionSetCd).setParameter("categoryId", categoryId)
				.setParameter("acceptItemNumber", acceptItemNumber).getSingle(c -> toDomain(c));
	}
	...
	public void update(StdAcceptItem domain) {
		...
		OiomtStdAcceptItem updateStdAcceptItem = this.queryProxy()
				.find(newStdAcceptItem.stdAcceptItemPk, OiomtStdAcceptItem.class).get();
	...
	public void remove(String cid, String conditionSetCd, String categoryId, int acceptItemNumber) {
		this.commandProxy().remove(OiomtStdAcceptItemPk.class,
				new OiomtStdAcceptItemPk(cid, conditionSetCd, categoryId, acceptItemNumber));
	}

	private static StdAcceptItem toDomain(OiomtStdAcceptItem entity) {
		return StdAcceptItem.createFromJavaType(entity.version, entity.stdAcceptItemPk.cid,
				entity.stdAcceptItemPk.conditionSetCd, entity.stdAcceptItemPk.categoryId,
				entity.stdAcceptItemPk.acceptItemNumber, entity.systemType, entity.csvItemNumber, entity.csvItemName,
				entity.itemType, entity.categoryItemNo);
	}

	private OiomtStdAcceptItem toEntity(StdAcceptItem domain) {
		return new OiomtStdAcceptItem(domain.getVersion(),
				new OiomtStdAcceptItemPk(domain.getCid(), domain.getConditionSetCd(), domain.getCategoryId(),
						domain.getAcceptItemNumber()),
				domain.getSystemType(), domain.getCsvItemNumber(), domain.getCsvItemName(), domain.getItemType(),
				domain.getCategoryItemNo());
	}

	@Override
	public List<StdAcceptItem> getStdAcceptItem(String cid, int systemType, String conditionSetCd) {
		String sql = SELECT_ALL_QUERY_STRING
				+ " WHERE  f.stdAcceptItemPk.cid =:cid AND  f.systemType =:systemType AND f.stdAcceptItemPk.conditionSetCd =:conditionSetCd ";
		return this.queryProxy().query(sql, OiomtStdAcceptItem.class).setParameter("cid", cid)
