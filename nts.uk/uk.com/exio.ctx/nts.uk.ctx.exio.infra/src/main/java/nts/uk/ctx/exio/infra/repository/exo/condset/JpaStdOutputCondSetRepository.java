...
public class JpaStdOutputCondSetRepository extends JpaRepository implements StdOutputCondSetRepository
{
    ...
    private static final String SELECT_BY_KEY_STRING = SELECT_ALL_QUERY_STRING + " WHERE  f.stdOutputCondSetPk.cid =:cid AND  f.stdOutputCondSetPk.conditionSetCd =:conditionSetCd ";
    ...
    public void update(StdOutputCondSet domain){
        ...
        OiomtStdOutputCondSet updateStdOutputCondSet = this.queryProxy().find(newStdOutputCondSet.stdOutputCondSetPk, OiomtStdOutputCondSet.class).get();
    ...
    private static StdOutputCondSet toDomain(OiomtStdOutputCondSet entity) {
        return StdOutputCondSet.createFromJavaType(entity.stdOutputCondSetPk.cid, entity.stdOutputCondSetPk.conditionSetCd, entity.categoryId, entity.delimiter, entity.itemOutputName, entity.autoExecution, entity.conditionSetName, entity.conditionOutputName, entity.stringFormat);
    }

    private OiomtStdOutputCondSet toEntity(StdOutputCondSet domain) {
        return new OiomtStdOutputCondSet(new OiomtStdOutputCondSetPk(domain.getCid(), domain.getConditionSetCd()), domain.getCategoryId(), domain.getDelimiter(), domain.getItemOutputName(), domain.getAutoExecution(), domain.getConditionSetName(), domain.getConditionOutputName(), domain.getStringFormat());
    }
