...
public class JpaCanonicalizedDataRecordRepository extends JpaRepository implements CanonicalizedDataRecordRepository {
	...
	@Override
	public List<CanonicalizedDataRecord> findCanonicalizedDomainDataByCriteria(Require require, ExecutionContext context, ImportingDomainId domainId, int criteriaItemNo, String criteriaValue){
//		//実行コンテキストを偽装
		val impersonatedContext = new ExecutionContext(context.getCompanyId(), 
																						   context.getSettingCode(),
																						   domainId,
																						   context.getMode());
		return findByCriteria(require, impersonatedContext, criteriaItemNo, criteriaValue);
	}
	
	@Override
	public List<CanonicalizedDataRecord> findByCriteria(Require require, ExecutionContext context, int criteriaItemNo, String criteriaValue){
		return WorkspaceSql.create(require, context, jdbcProxy(), this.database().product())
