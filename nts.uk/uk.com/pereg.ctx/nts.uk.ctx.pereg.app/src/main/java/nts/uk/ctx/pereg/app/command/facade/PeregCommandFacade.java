						...
						if(ctgType == CategoryType.CONTINUOUSHISTORY || ctgType == CategoryType.CONTINUOUS_HISTORY_FOR_ENDDATE
							...
							if(historyLst.size() == 1) {
								if (item.itemCode().equals(dateRange.getEndDateCode())) {
									item.setValueAfter(isContinuousHistory && !input.getCategoryCd().equals(category21)? valueEndate: item.valueAfter());
									item.setContentAfter(isContinuousHistory && !input.getCategoryCd().equals(category21)? valueEndate: item.valueAfter());
								}
								...
								for (ComboBoxObject c : historyLst) {
									if (c.getOptionValue() != null) {
										...
										case ADD:
											...
											if (item.itemCode().equals(dateRange.getEndDateCode())) {
												item.setValueAfter(isContinuousHistory && !input.getCategoryCd().equals("CS00021")? valueEndate: item.valueAfter());
												item.setContentAfter(isContinuousHistory && !input.getCategoryCd().equals("CS00021")? valueEndate: item.contentAfter());
											}else {
												if(ctgType == CategoryType.CONTINUOUSHISTORY || ctgType == CategoryType.CONTINUOUS_HISTORY_FOR_ENDDATE) {
