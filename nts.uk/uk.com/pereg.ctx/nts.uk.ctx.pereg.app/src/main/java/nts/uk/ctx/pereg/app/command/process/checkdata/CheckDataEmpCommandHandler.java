...
public class CheckDataEmpCommandHandler extends AsyncCommandHandler<CheckDataFromUI>{
	 ...
	 */
	public List<PersonInfoCategory> getAllCategory(String cid) {
		...
		List<PersonInfoCategory> result = new ArrayList<>();
		...
		filterCategory(lstCtgCheck, result);
	...
	public void filterCategory(List<PerInfoValidateCheckCategory> listCtgSetting, List<PersonInfoCategory> listCtg) {
		listCtg.forEach(ctg -> {
			PerInfoValidateCheckCategory ctgSetting = listCtgSetting.stream()
					.filter(i -> i.getCategoryCd().toString().equals(ctg.getCategoryCode().toString())).findFirst()
					.get();
