...
public class WorkPlaceSelectionImpl implements WorkPlaceSelectionRepository {
	...
	private static final String GET_EXPORT_EXCEL = "SELECT "
			...
			+" 						INNER JOIN SACMT_WORKPLACE_MANAGER wi ON wm.WKPID = wi.WKP_ID "
			+" 						INNER JOIN BSYMT_EMP_DTA_MNG_INFO edm ON wi.SID = edm.SID  AND edm.CID = ?cid"
			+" 						INNER JOIN BPSMT_PERSON ps ON edm.PID = ps.PID "
			+" 						INNER JOIN KASMT_WORKPLACE_AUTHORITY kwa ON wi.WKP_MANAGER_ID = kwa.ROLE_ID AND wm.CID = kwa.CID AND wm.CID = ?cid"
