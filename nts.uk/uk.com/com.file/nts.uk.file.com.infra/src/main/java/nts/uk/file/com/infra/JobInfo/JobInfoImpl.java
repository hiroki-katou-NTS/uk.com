...
public class JobInfoImpl extends JpaRepository implements JobInfoRepository {
	...
	private static final String GET_EXPORT_EXCEL = 
			"SELECT CASE WHEN TBL.ROW_NUMBER = 1 THEN TBL.APPLY_CONCURRENT_PERSON ELSE NULL END APPLY_CONCURRENT_PERSON,"
			+" TBL.JOB_CD, TBL.JOB_NAME, TBL.ROLESET_NAME, TBL.START_DATE, TBL.END_DATE FROM "
			+" (SELECT job.APPLY_CONCURRENT_PERSON, info.JOB_CD, info.JOB_NAME, d.ROLESET_CD +' '+ s.ROLE_SET_NAME AS ROLESET_NAME,"
			+" his.START_DATE, his.END_DATE, ROW_NUMBER() OVER (PARTITION BY job.APPLY_CONCURRENT_PERSON "
			+" ORDER BY info.JOB_CD ASC) AS ROW_NUMBER "
			...
			+" WHERE info.CID = ?cid AND  his.START_DATE <= CONVERT(DATETIME, ?date, 127) AND his.END_DATE >= CONVERT(DATETIME, ?date, 127)) TBL";
	...
	private static final String GET_EXPORT_DATA = 
			...
			+ "(SELECT per.ROLESET_CD, "
			+" rs.ROLE_SET_NAME, em.SCD, p.BUSINESS_NAME, per.START_DATE, per.END_DATE,"
				+" ROW_NUMBER() OVER (PARTITION BY per.ROLESET_CD "
				+" ORDER BY  rs.ROLE_SET_CD ASC, em.SCD) AS ROW_NUMBER "
	...
	@Override
	public List<MasterData> getDataExport_Per(){
		String cid = AppContexts.user().companyId();
		List<MasterData> datas = new ArrayList<>();
		...
		for (Object[] objects : data) {
			datas.add(new MasterData(dataContent2(objects), null, ""));
		}
		return datas;
	}
	
	private Map<String, Object> dataContent2(Object[] object) {
		Map<String, Object> data = new HashMap<>();
		// R9_1
		...
		// R9_5
		data.put(JobInfoColumn.CAS014_49, object[4] != null ? checkValue1(object[4]) : null);
		// R9_6
		data.put(JobInfoColumn.CAS014_50, object[5] != null ? checkValue1(object[5]) : null);
		return data;
	}
