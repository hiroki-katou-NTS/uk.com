...
public class AutoExecutionServerExternalOutputQuery {
	...
	public Optional<String> processAutoExecution(String cid, String conditionCd, DatePeriod period, GeneralDate baseDate, Integer categoryId,
			String execId, List<String> empIds) {
		// 外部出力処理IDを採番する
		String processingId = execId;
		ExOutSetting exOutSetting = new ExOutSetting(conditionCd, AppContexts.user().userId(), categoryId,
				period.start(), period.end(), baseDate, processingId, true, empIds);
		...
		Optional<ExterOutExecLog> optResult = this.exterOutExecLogRepository
				.getExterOutExecLogById(cid, processingId);
		optResult.ifPresent(result -> {
			result.getFileId().ifPresent(fileId -> {
				fileStorage.getInfo(fileId).ifPresent(fileInfo -> {
					this.exterOutExecLogRepository.update(cid, processingId,
							fileInfo.getOriginalSize());
				});
			});			
		});
		// Check whether errors have occured or not
		List<ExternalOutLog> logList = this.externalOutLogRepository
				.getExternalOutLogById(cid, execId, ProcessingClassification.ERROR.value);
		return logList.stream()
				.map(item -> item.getErrorContent().orElse(null))
