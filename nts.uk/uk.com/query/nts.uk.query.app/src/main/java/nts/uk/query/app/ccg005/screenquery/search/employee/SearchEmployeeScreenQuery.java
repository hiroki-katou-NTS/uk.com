...
public class SearchEmployeeScreenQuery {
	...
	public void searchForEmployee(String keyWorks, GeneralDate baseDate, boolean emojiUseage) {
		...
		//1: 社員コードで検索する() 
		List<String> sidsByCode = employeeFinder.searchByEmployeeCode(keyWorks, 2, baseDate); // システム区分　＝　就業  ＝  2
		sidsSearch.addAll(sidsByCode);
		//2: 社員名で検索する()
		List<String> sidsByName = employeeFinder.searchByEmployeeName(keyWorks, 2, baseDate);  // システム区分　＝　就業  ＝ 2 
		...
		List<String> empList = DetermineEmpIdListDomainService.determineReferenced(deterEmpLstDsRequireImpl, sidsSearch, baseDate, loginCid, loginRoleid, loginSid);
