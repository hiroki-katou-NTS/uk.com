...
public class JpaEmployeeInformationRepository extends JpaRepository implements EmployeeInformationRepository {
	...
	// 社員の所属部門を取得する
	private static final String DEPARTMENT_QUERY = "SELECT dph.sid, di.pk.company, di.deleteFlag, di.pk.departmentHistoryId,"
													+ " di.pk.departmentHistoryId, di.departmentId,"
			...
			+ " LEFT JOIN BsymtDepartmentInfor di ON di.pk.departmentHistoryId = dh.bsymtDepartmentHistPK.histId AND di.pk.cid =:cid"
