...
public class ApprovalPhaseOutput {
	...
	public static ApprovalPhaseOutput convertToOutputData(ApprovalPhase phase) {
		return new ApprovalPhaseOutput(phase.getCompanyId(), phase.getBranchId(), phase.getApprovalPhaseId(),
				phase.getApprovalForm().value, phase.getBrowsingPhase(), phase.getOrderNumber(), null);
	}
	...
	public boolean containsAny(List<Approver> masterApprovers) {
		Set<String> masterApproverIds = masterApprovers.stream().map(a -> a.getApproverId())
