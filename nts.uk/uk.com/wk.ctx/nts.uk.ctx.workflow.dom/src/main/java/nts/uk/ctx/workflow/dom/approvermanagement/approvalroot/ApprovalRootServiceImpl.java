...
public class ApprovalRootServiceImpl implements ApprovalRootService {
	...
	public List<ApprovalRootOutput> adjustmentData(String cid, String sid, GeneralDate baseDate,
		...
		appDatas.stream().forEach(x -> {
			...
			appPhase.stream().filter(f -> f.getBrowsingPhase() == 0).collect(Collectors.toList());
			...
			if (!CollectionUtil.isEmpty(approvers)) {
				...
				approvers.stream().forEach(x -> {
					...
					if (x.getApprovalAtr() == ApprovalAtr.PERSON) {
						String t = employeeAdapter.getEmployeeName(x.getEmployeeId());
						approversResult.add(new ApproverInfo(x.getEmployeeId(), x.getApprovalPhaseId(), true,
								x.getOrderNumber(), t));
					} else {
						// 職位の場合
				...
				if (!CollectionUtil.isEmpty(approversResult)) {
					List<String> approverIds = approversResult.stream().map(x -> x.getSid())
							.collect(Collectors.toList());
					// 3-1.承認代行情報の取得処理
					ApprovalAgencyInfoOutput agency = this.appAgencyInfoService.getApprovalAgencyInformation(cid,
							approverIds);
