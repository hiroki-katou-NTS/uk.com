...
@AllArgsConstructor
@Entity
@Table(name="WWFDT_APP_INST_FRAME_DAY")
@Builder
public class WwfdtAppInstFrameDay extends ContractUkJpaEntity {
	...
	@OneToMany(targetEntity=WwfdtApproverDay.class, cascade = CascadeType.ALL, mappedBy = "wwfdtAppInstFrameDay", orphanRemoval = true, fetch = FetchType.EAGER)
	...
	public static WwfdtAppInstFrameDay fromDomain(String companyID, String rootId, int phaseOrder, ApprovalFrame frame){
		ApproverInfor approver = frame.getLstApproverInfo().get(0);
		return WwfdtAppInstFrameDay.builder()
