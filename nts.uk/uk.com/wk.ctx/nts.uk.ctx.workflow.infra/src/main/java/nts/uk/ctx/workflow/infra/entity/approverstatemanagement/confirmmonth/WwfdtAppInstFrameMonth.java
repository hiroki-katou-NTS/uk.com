...
@AllArgsConstructor
@Entity
@Table(name="WWFDT_APP_INST_FRAME_MONTH")
@Builder
public class WwfdtAppInstFrameMonth extends ContractUkJpaEntity {
	...
	@OneToMany(targetEntity=WwfdtApproverMonth.class, cascade = CascadeType.ALL, mappedBy = "wwfdtAppInstFrameMonth", orphanRemoval = true, fetch = FetchType.EAGER)
	...
	public static WwfdtAppInstFrameMonth fromDomain(String companyID, String rootId, int phaseOrder, ApprovalFrame frame){
		ApproverInfor approver = frame.getLstApproverInfo().get(0);
		return WwfdtAppInstFrameMonth.builder()
