 ...
 */
@Stateless
public class UpdateWorkAppApprovalRByHistCommandHandler extends CommandHandler<List<UpdateWorkAppApprovalRByHistCommand>>{
	@Inject
	private WorkAppApprovalRootRepository repo;
	
	@Override
	protected void handle(CommandHandlerContext<List<UpdateWorkAppApprovalRByHistCommand>> context) {
		String companyId = AppContexts.user().companyId();
		List<UpdateWorkAppApprovalRByHistCommand>  lstUpdateItem = context.getCommand();
		for (UpdateWorkAppApprovalRByHistCommand updateItem : lstUpdateItem) {
			//history current
			...
			if(updateItem.getCheck()==2){
				...
				if(lstOld.isEmpty()){// history previous is not exist
					if(updateItem.getEditOrDelete()==1){//TH: edit
						...
						repo.deleteWpApprovalRoot(companyId, updateItem.getApprovalId(), updateItem.getWorkplaceId(), updateItem.getHistoryId());
					}
				}else{// history previous is exist
			...
			if(updateItem.getCheck()==3){
				...
				if(lstOld.isEmpty()){// history previous is not exist
					if(updateItem.getEditOrDelete()==1){//TH: edit
						...
						repo.deletePsApprovalRoot(companyId, updateItem.getApprovalId(), updateItem.getEmployeeId(), updateItem.getHistoryId());
					}
				}else{// history previous is exist
