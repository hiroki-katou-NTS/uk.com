...
public class DisplayInformationScreenQuery {
	...
	private DisplayInformationDto getDisplayInfoAfterSelect(List<String> wkspIds, GeneralDate baseDate, boolean emojiUseage) {
		...
		List<PersonalInfomationObj> listPersonalInfo = PersonalInfomationDomainService.getPersonalInfomation(rq, getDeterEmployeeIdList, baseDate);
		// TODO: 4:在席情報を取得する(社員ID, 年月日, するしない区分): List<在席情報DTO>
		
		return DisplayInformationDto.builder().build();
	}
}
